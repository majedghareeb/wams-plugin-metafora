...
registerBlockType('um-block/um-forms', {
	edit: function (props) {
		...
		const posts = useSelect((select) => {
			return select('core').getEntityRecords('postType', 'wams_form', {
				per_page: -1,
				_fields: ['id', 'title']
			});
		});
		...
		return (
			<div {...blockProps}>
				...
				<InspectorControls>
					<PanelBody title={wp.i18n.__('Select Forms', 'wams')}>
						<SelectControl
							label={wp.i18n.__('Select Forms', 'wams')}
							className="wams_select_forms"
							value={form_id}
							options={get_post}
