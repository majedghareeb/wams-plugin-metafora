...
registerBlockType('um-block/um-member-directories', {
	edit: function (props) {
		...
		const posts = useSelect((select) => {
			return select('core').getEntityRecords('postType', 'wams_directory', {
				per_page: -1,
				_fields: ['id', 'title']
			});
		});
		...
		return (
			<div {...blockProps}>
				...
				<InspectorControls>
					<PanelBody title={wp.i18n.__('Select Directories', 'wams')}>
						<SelectControl
							label={wp.i18n.__('Select Directories', 'wams')}
							className="wams_select_directory"
							value={member_id}
							options={get_post}
	...
	var observer = new MutationObserver(function(mutations) {
		mutations.forEach(function(mutation) {
			...
			jQuery(mutation.addedNodes).find('.um.um-directory').each(function() {
				...
				wams_ajax_get_members( directory );
				wams_slider_filter_init( directory );
				wams_datepicker_filter_init( directory );
				wams_timepicker_filter_init( directory );
			});
		});
	});
