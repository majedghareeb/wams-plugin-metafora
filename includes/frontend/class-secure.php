...
if (!class_exists('wams\frontend\Secure')) {
	...
	class Secure
	{
		...
		public function __construct()
		{
			...
			add_action('wams_before_login_fields', array($this, 'reset_password_notice'), 1);

			add_action('wams_before_login_fields', array($this, 'under_maintenance_notice'), 1);

			add_action('wams_submit_form_register', array($this, 'block_register_forms'));

			add_action('wams_user_login', array($this, 'login_validate_expired_pass'), 1);
		...
		{
			if (!WAMS()->options()->get('lock_register_forms')) {
				return;
			}
		...
		/**
		 * Block all WAMS Register form submissions.
		 *
		 * @since 2.6.8
		 */
		...
		{
			if (WAMS()->options()->get('lock_register_forms')) {
				$login_url = add_query_arg('notice', 'maintenance', wams_get_core_page('login'));
				nocache_headers();
		...
		{
			if (WAMS()->options()->get('display_login_form_notice')) {
				$expired_password_reset = get_user_meta(wams_user('ID'), 'wams_secure_has_reset_password', true);
				if (!$expired_password_reset) {
					$login_url = add_query_arg('notice', 'expired_password', wams_get_core_page('login'));
					// Not `wams_safe_redirect()` because predefined login page is situated on the same host.
					wp_safe_redirect($login_url);
					exit;
				}
		...
		/**
		 * Prevent users from using Old Passwords on WAMS Password Reset form.
		 *
		...
		{
			if (empty($_POST['_wams_password_change'])) { // phpcs:ignore WordPress.Security.NonceVerification
				return;
			}
			...
			if (isset($_REQUEST['user_password'])) { // phpcs:ignore WordPress.Security.NonceVerification
				...
				if (wp_check_password($new_user_pass, $user->data->user_pass, $user->ID)) {
					WAMS()->form()->add_error('user_password', __('Your new password cannot be same as old password.', 'wams'));
					$errors->add('wams_block_old_password', __('Your new password cannot be same as old password.', 'wams'));
				} else {
					update_user_meta($user->ID, 'wams_secure_has_reset_password', true);
					update_user_meta($user->ID, 'wams_secure_has_reset_password__timestamp', current_time('mysql', true));
				}
			}
		}
		...
		public function secure_user_capabilities($user_id)
		{
			...
			$has_admin_cap   = false;
			$arr_banned_caps = WAMS()->options()->get('banned_capabilities');
			...
			if (!$has_admin_cap) {
				 ...
				 */
				$user_level = wams_user($wpdb->get_blog_prefix() . 'user_level');
			...
			if (!$has_admin_cap) {
				 ...
				 */
				if (!empty(WAMS()->form()->post_form['submitted']) && is_array(WAMS()->form()->post_form['submitted'])) {
					foreach (array_keys(WAMS()->form()->post_form['submitted']) as $submitted_metakey) {
						if (WAMS()->user()->is_metakey_banned($submitted_metakey)) {
							$has_admin_cap = true;
							break;
						}
					}
			...
			if ($has_admin_cap) {
				WAMS()->common()->secure()->revoke_caps($user);
				/**
				 * Notify Administrators Immediately
				 */
				if (WAMS()->options()->get('secure_notify_admins_banned_accounts')) {
					$interval = WAMS()->options()->get('secure_notify_admins_banned_accounts__interval');
					if ('instant' === $interval) {
						WAMS()->common()->secure()->send_notification(array($user_id));
					}
				}
				...
				wp_logout();
				session_unset();
				$redirect = apply_filters('wams_secure_blocked_user_redirect_immediately', true);
				if ($redirect) {
					$login_url = add_query_arg('err', 'inactive', wams_get_core_page('login'));
					// Not `wams_safe_redirect()` because predefined login page is situated on the same host.
					wp_safe_redirect($login_url);
					exit;
				}
