...
class ExportToWord
{
	...
	private static $imgRef = WAMS_PATH . 'temp/';							// path to the images directory in 'ref' format
	private static $imgDir = WAMS_PATH . 'temp/img/';					// path to the images directory in absolute format
	...
	private static function htmlEntities($html)
	{
		...
		foreach ($matches[0] as $imgTag) {
			...
			$imageExt = '';
			if (isset(pathinfo($matches[2][$i])['extension'])) {
