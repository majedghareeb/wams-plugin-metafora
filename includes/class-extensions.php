...
if (!class_exists('wams\Extensions')) {
	...
	class Extensions
	{
		...
		function check_dependencies()
		{
			...
			foreach ($extensions as $slug) {
				...
				list($run, $slug, $message) = apply_filters_ref_array('wams_extension_custom_dependencies', array(true, $slug, ''));

				if ($run) {
					$compare_version_result = WAMS()->dependencies()->compare_versions($extension['min_core_version'], $extension['version'], $slug, $extension['title']);

					if (true !== $compare_version_result) {
						WAMS()->admin()->notices()->add_notice("{$slug}_dependencies", array(
							'class'     => 'error',
				...
				if ($run) {
					WAMS()->call_class("wams_ext\wams_{$slug}\Init");
				}
			}
		}
		...
		function license_options($settings)
		{
			...
			foreach ($extensions as $slug) {
				...
				$settings['licenses']['fields'][] = array(
					'id'        => "wams_{$slug}_license_key",
					// translators: %s: extension name.
					'label'     => sprintf(__('%s License Key', 'wams'), $extension['title']),
		...
		function activate($slug)
		{
			...
			//if extension wasn't inited, init it firstly via "wams_{$slug}_add" function
			//"wams_{$slug}_add" must be in the preset structure of WAMS extension
			if (empty($plugin_data) && function_exists("wams_{$slug}_add")) {
				call_user_func("wams_{$slug}_add");
				$plugin_data = $this->get_info($slug);
			}

			//first install
			$version = get_option("wams_{$slug}_version");
			if (!$version) {
				update_option("wams_{$slug}_last_version_upgrade", $plugin_data['version']);
			}

			if ($version != $plugin_data['version']) {
				update_option("wams_{$slug}_version", $plugin_data['version']);
			}


			//start setup
			WAMS()->extension($slug)->setup()->start();
		}
		...
		function get_packages($slug)
		{
			...
			if ($handle) {
				...
				usort($update_versions, array(WAMS()->admin_upgrade(), 'version_compare_sort'));
			}
