...
if (!class_exists('wams\core\Validation')) {
	...
	class Validation
	{
		...
		{
			add_filter('wams_user_pre_updating_files_array', array($this, 'validate_files'));
			add_filter('wams_before_save_filter_submitted', array($this, 'validate_fields_values'), 10, 3);
		}
		...
		public function validate_fields_values($changes, $args, $form_data)
		{
			...
			foreach ($changes as $key => $value) {
				...
				if (in_array($fields[$key]['type'], array('select', 'multiselect'), true) && $has_custom_source) {
					...
					$fields[$key] = apply_filters("wams_get_field__{$key}", $fields[$key]);
				...
				if (
					in_array($fields[$key]['type'], array('select', 'multiselect'), true) &&
					...
					if (!WAMS()->fields()->is_source_blacklisted($fields[$key]['custom_dropdown_options_source'])) {
				...
				if (
					in_array($fields[$key]['type'], array('select')) &&
				...
				if (
					in_array($fields[$key]['type'], array('multiselect', 'checkbox', 'radio')) &&
		...
		function safe_username($string)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_validation_safe_username_regex
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_validation_safe_username_regex', 'function_name', 10, 1 ); ?>
			 * @example
			 * <?php
			 * add_filter( 'wams_validation_safe_username_regex', 'my_validation_safe_username', 10, 1 );
			 ...
			 * ?>
			 */
			$regex_safe_username = apply_filters('wams_validation_safe_username_regex', $this->regex_username_safe);
		...
		function safe_string($string)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_validation_safe_string_regex
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_validation_safe_string_regex', 'function_name', 10, 1 ); ?>
			 * @example
			 * <?php
			 * add_filter( 'wams_validation_safe_string_regex', 'my_validation_safe_string', 10, 1 );
			 ...
			 * ?>
			 */
			$regex_safe_string = apply_filters('wams_validation_safe_string_regex', $this->regex_safe);
