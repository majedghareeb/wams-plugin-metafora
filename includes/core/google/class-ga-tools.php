...
if (!class_exists('wams\core\google\GA_Tools')) {
    ...
    class GA_Tools
    {
        ...
        public static function set_cache($name, $value, $expiration = 0)
        {
            ...
            update_option('wams_ga_cache_' . $name, $option, 'no');
        }

        public static function delete_cache($name)
        {
            delete_option('wams_ga_cache_' . $name);
        }

        public static function get_cache($name)
        {
            $option = get_option('wams_ga_cache_' . $name);
        ...
        {
            global $wpdb;
            $sqlquery = $wpdb->query("DELETE FROM $wpdb->options WHERE option_name LIKE 'wams_ga_cache_qr%%'");
        }
        ...
        public static function anonymize_options($options)
        {
            ...
            $options['aiwp_version'] = WAMS_VERSION;
