...
if (!class_exists('wams\core\Date_Time')) {
	...
	class Date_Time
	{
		...
		public function time_diff($from, $to = '')
		{
			...
			if ($diff < 60) {
				...
				$since = sprintf(_n('%s min', '%s mins', $mins, 'wams'), $mins);
				...
				// translators: %s: hours.
				$since = sprintf(_n('%s hr', '%s hrs', $hours, 'wams'), $hours);
				...
				if (1 === $days) {
					// translators: %s: time.
					$since = sprintf(__('Yesterday at %s', 'wams'), date_i18n(get_option('time_format'), $from));
				} else {
					// translators: %1$s is a date; %2$s is a time.
					$since = sprintf(__('%1$s at %2$s', 'wams'), date_i18n('F d', $from), date_i18n(get_option('time_format'), $from));
				}
				...
				// translators: %1$s is a date; %2$s is a time.
				$since = sprintf(__('%1$s at %2$s', 'wams'), date_i18n(get_option('date_format'), $from), date_i18n(get_option('time_format'), $from));
			}
			 ...
			 *
			 * @type filter
			 * @title wams_human_time_diff
			 * @description Change human time string
			 * @input_vars
			 * [{"var":"$since","type":"string","desc":"Disable WAMS Cron?"},
			 * {"var":"$diff","type":"int","desc":"Difference in seconds"},
			 * {"var":"$from","type":"int","desc":"From Timestamp"},
			 * {"var":"$to","type":"int","desc":"To Timestamp"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_human_time_diff', 'function_name', 10, 4 );
			 * @example
			 * <?php
			 * add_filter( 'wams_human_time_diff', 'my_human_time_diff', 10, 4 );
			 ...
			 * ?>
			 */
			return apply_filters('wams_human_time_diff', $since, $diff, $from, $to);
		}
		...
		public function get_age($then)
		{
			...
			if (0 === $age) {
				return __('Less than 1 year old', 'wams');
			}

			// translators: %s: age.
			return sprintf(_n('%s year old', '%s years old', $age, 'wams'), $age);
		}
