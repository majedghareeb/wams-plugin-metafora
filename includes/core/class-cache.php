...
if (!class_exists('wams\core\Cache')) {
	class Cache
	{
		/**
		 * Instance.
		...
		public function setup()
		{
			...
			self::$instance->is_cache = defined('CACHE') ? CACHE : $this->is_cache_enabled();

			// weekly delete all expired cache.
			// Give_Cron::add_weekly_event(array($this, 'delete_all_expired'));

			add_action('save_post_wams_vendors', array($this, 'delete_vendor_related_cache'));

			add_action('wams_save_settings_wams_settings', array(__CLASS__, 'flush_cache'));
...
<?php
		...
		public static function get_key($action, $query_args = null, $is_prefix = true)
		{
			...
			// Set cache key.
			$cache_key = $is_prefix ? "cache_{$action}" : $action;
		...
		public static function get($cache_key, $custom_key = false, $query_args = array())
		{
			if (!self::is_valid_cache_key($cache_key)) {
				if (empty($cache_key)) {
					...
					return new WP_Error('give_invalid_cache_key', __('Cache key format should be cache_*', 'give'));
				}
		...
		public static function delete($cache_keys)
		{
			...
			if (!$result) {
				$result = new WP_Error(
					...
					__('Cache key format should be cache_*', 'give'),
					$invalid_keys
				);
			}
		...
		{
			$is_valid = (false !== strpos($cache_key, 'cache_'));
		...
		public static function flush_cache($force = false)
		{
			if (
				...
				|| (wp_doing_ajax() && isset($_GET['action']) && 'cache_flush' === give_clean($_GET['action']))
			) {
				self::$instance->get_incrementer(true);
		...
		private function filter_group_name($group)
		{
			 ...
			 * @since 2.1.0
			 */
			$filtered_group = apply_filters('cache_filter_group_name', '', $group);
