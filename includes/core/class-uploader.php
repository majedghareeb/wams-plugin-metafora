...
if (!class_exists('wams\core\Uploader')) {
	...
	class Uploader
	{
		...
		{
			$this->core_upload_dir = DIRECTORY_SEPARATOR . 'wams' . DIRECTORY_SEPARATOR;
			$this->core_upload_url = '/wams/';
			...
			// add_filter('wams_upload_image_result', array($this, 'rotate_uploaded_image'), 10, 1);
			// add_filter('wams_upload_image_process__profile_photo', array($this, 'profile_photo'), 10, 7);
			// add_filter('wams_upload_image_process__cover_photo', array($this, 'cover_photo'), 10, 7);
			// add_action('wams_upload_stream_image_process', array($this, 'stream_photo'), 10, 7);

			add_action('init', array($this, 'init'));

			//remove user old files
			// add_action('wams_after_move_temporary_files', array($this, 'remove_unused_uploads'), 10, 3);
		}
		...
		public function set_upload_directory($args)
		{
			...
			list($this->upload_user_baseurl, $this->upload_user_basedir) = apply_filters('wams_change_upload_user_path', array($this->upload_user_baseurl, $this->upload_user_basedir), $this->field_key, $this->upload_type);
		...
		public function upload_image($uploadedfile, $user_id = null, $field_key = '', $upload_type = 'stream_photo')
		{
			...
			if (!empty($field_data['allowed_types'])) {
				...
				$field_allowed_file_types = apply_filters('wams_uploader_image_default_filetypes', array('JPG', 'JPEG', 'PNG', 'GIF'));
			}
			...
			$upload_overrides = array(
				'test_form'                 => false,
				'mimes'                     => apply_filters('wams_uploader_allowed_image_mimes', $allowed_image_mimes),
				'unique_filename_callback'  => array($this, 'unique_filename'),
			);

			$upload_overrides = apply_filters("wams_image_upload_handler_overrides__{$field_key}", $upload_overrides);
			...
			if (isset($movefile['error'])) {
				 ...
				 *
				 * @type        filter
				 * @title       wams_upload_image_result
				 ...
				 * @example
				  <?php
				  add_filter( 'wams_upload_image_result', 'custom_wams_upload_image_result', 10, 3 );
				  function custom_wams_upload_image_result( $movefile, $user_id, $field_data ) {
						// your code here
						return $movefile;
				  }
				  ?>
				 */
				$movefile = apply_filters('wams_upload_image_result', $movefile, $user_id, $field_data);
				 ...
				 *
				 * @type action
				 * @title wams_before_upload_db_meta
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_action( 'wams_before_upload_db_meta', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_action( 'wams_before_upload_db_meta', 'my_before_upload_db_meta', 10, 2 );
				 * function my_before_upload_db_meta( $user_id, $field_key ) {
				 *     // your code here
				 * }
				 * ?>
				 */
				do_action('wams_before_upload_db_meta', $this->user_id, $field_key);
				/**
				 * WAMS hook
				 *
				 * @type action
				 * @title wams_before_upload_db_meta_{$key}
				 * @description Update user's meta before upload
				 * @input_vars
				 * [{"var":"$user_id","type":"int","desc":"User ID"}]
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_action( 'wams_before_upload_db_meta_{$key}', 'function_name', 10, 1 );
				 * @example
				 * <?php
				 * add_action( 'wams_before_upload_db_meta_{$key}', 'my_before_upload_db_meta', 10, 1 );
				 * function my_before_upload_db_meta( $user_id ) {
				 *     // your code here
				 * }
				 * ?>
				 */
				do_action("wams_before_upload_db_meta_{$field_key}", $this->user_id);

				/**
				 * WAMS hook
				 *
				 * @type action
				 * @title wams_after_upload_db_meta
				 * @description Update user's meta before upload
				 * @input_vars
				 * [{"var":"$user_id","type":"int","desc":"User ID"},
				 * {"var":"$key","type":"string","desc":"Meta key"}]
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_action( 'wams_after_upload_db_meta', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_action( 'wams_after_upload_db_meta', 'my_after_upload_db_meta', 10, 2 );
				 * function my_after_upload_db_meta( $user_id, $field_key ) {
				 *     // your code here
				 * }
				 * ?>
				 */
				do_action('wams_after_upload_db_meta', $this->user_id, $field_key);
				/**
				 * WAMS hook
				 *
				 * @type action
				 * @title wams_after_upload_db_meta_{$key}
				 * @description Update user's meta after upload
				 * @input_vars
				 * [{"var":"$user_id","type":"int","desc":"User ID"}]
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_action( 'wams_after_upload_db_meta_{$key}', 'function_name', 10, 1 );
				 * @example
				 * <?php
				 * add_action( 'wams_after_upload_db_meta_{$key}', 'my_after_upload_db_meta', 10, 1 );
				 * function my_after_upload_db_meta( $user_id ) {
				 *     // your code here
				 * }
				 * ?>
				 */
				do_action("wams_after_upload_db_meta_{$field_key}", $this->user_id);

				$transient = set_transient("wams_{$movefile['file']}", $movefile['file_info'], 2 * HOUR_IN_SECONDS);
				if (empty($transient)) {
			...
			WAMS()->files()->remove_old_files(WAMS()->files()->upload_temp);
		...
		public function upload_file($uploadedfile, $user_id = null, $field_key = '')
		{
			...
			if (isset($movefile['error'])) {
				...
				$transient = set_transient("wams_{$filename}", $movefile['file_info'], 2 * HOUR_IN_SECONDS);
				if (empty($transient)) {
					update_user_meta($this->user_id, "{$field_key}_metadata_temp", $movefile['file_info']);
				}
			}
		...
		public function validate_image_data($file, $field_key)
		{
			...
			if (is_wp_error($image)) {
				$error = sprintf(__('Your image is invalid!', 'wams'));
				return $error;
			}
			...
			if ($data == null) {
				 ...
				 *
				 * @type filter
				 * @title wams_custom_image_handle_{$field}
				 * @description Custom image handle
				 * @input_vars
				 * [{"var":"$data","type":"array","desc":"Image Data"}]
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_custom_image_handle_{$field}', 'function_name', 10, 1 );
				 * @example
				 * <?php
				 * add_filter( 'wams_custom_image_handle_{$field}', 'my_custom_image_handle', 10, 1 );
				 ...
				 * ?>
				 */
				$data = apply_filters("wams_custom_image_handle_{$field_key}", array());
				if (!$data) {
					$error = __('This media type is not recognized.', 'wams');
				}
			}
			 ...
			 *
			 * @type filter
			 * @title wams_image_handle_global__option
			 * @description Custom image global handle
			 * @input_vars
			 * [{"var":"$data","type":"array","desc":"Image Data"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_image_handle_global__option', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_filter( 'wams_image_handle_global__option', 'my_image_handle_global', 10, 1 );
			 ...
			 * ?>
			 */
			$data = apply_filters("wams_image_handle_global__option", $data);
			/**
			 * WAMS hook
			 *
			 * @type filter
			 * @title wams_image_handle_{$field}__option
			 * @description Custom image handle for each $field
			 * @input_vars
			 * [{"var":"$data","type":"array","desc":"Image Data"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_image_handle_{$field}__option', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_filter( 'wams_image_handle_{$field}__option', 'my_image_handle', 10, 1 );
			 ...
			 * ?>
			 */
			$data = apply_filters("wams_image_handle_{$field_key}__option", $data);

			if (isset($image_info['invalid_image']) && $image_info['invalid_image'] == true) {
				$error = sprintf(__('Your image is invalid or too large!', 'wams'));
				...
				$error = sprintf(__('Your photo is too small. It must be at least %spx wide.', 'wams'), $data['min_width']);
			} elseif (isset($data['min_height']) && ($image_info['height'] < $data['min_height'])) {
				// translators: %s: min height.
				$error = sprintf(__('Your photo is too small. It must be at least %spx high.', 'wams'), $data['min_height']);
			}
		...
		public function profile_photo($response, $image_path, $src, $key, $user_id, $coord, $crop)
		{
			...
			$quality = WAMS()->options()->get('image_compression');
			...
			if (!is_wp_error($image)) {
				...
				$max_w = WAMS()->options()->get('image_max_width');
				if ($src_w > $max_w) {
				...
				wp_send_json_error(esc_js(__("Unable to crop image file: {$src}", 'wams')));
			}
		...
		public function stream_photo($response, $image_path, $src, $key, $user_id, $coord = '', $crop = array())
		{
			...
			if (!is_wp_error($image)) {
				$quality = (int) WAMS()->options()->get('image_compression');
				$max_w = (int) WAMS()->options()->get('image_max_width');
				...
				wp_send_json_error(esc_js(__("Unable to crop stream image file: {$image_path}", 'wams')));
			}
		...
		public function resize_image($image_path, $src, $key, $user_id, $coord)
		{
			...
			$response = apply_filters("wams_upload_image_process__{$key}", $response, $image_path, $src, $key, $user_id, $coord, $crop);

			if (!in_array($key, array('profile_photo', 'cover_photo'))) {
				$response = apply_filters('wams_upload_stream_image_process', $response, $image_path, $src, $key, $user_id, $coord, $crop);
			}
		...
		{
			$image_fix_orientation = WAMS()->options()->get('image_orientation_by_exif');
			if ($image_fix_orientation && $movefile['type'] == 'image/jpeg') {
				$image = imagecreatefromjpeg($movefile['file']);
				if ($image) {
					$image = WAMS()->files()->fix_image_orientation($image, $movefile['file']);
					$quality = WAMS()->options()->get('image_compression');
		...
		function move_temporary_files($user_id, $files, $move_only = false)
		{
			...
			$user_basedir = WAMS()->uploader()->get_upload_user_base_dir($user_id, true);
			...
			foreach ($files as $key => $filename) {
				...
				if (empty($filename) || 'empty_file' == $filename) {
					...
					if (!empty($old_filename)) {
						...
						if ($valid) {
							if (file_exists($file) && wams_is_file_owner($file, $user_id)) {
								unlink($file);
							}
						}
					}
					...
					delete_transient("wams_{$filename}");

					continue;
				}

				//move temporary file from temp directory to the correct user directory
				$temp_file_path = WAMS()->uploader()->get_core_temp_dir() . DIRECTORY_SEPARATOR . $filename;
				if (file_exists($temp_file_path)) {
					...
					if ($move_only) {
						...
						if (rename($temp_file_path, $file)) {
							...
							$file_info = get_transient("wams_{$filename}");
							if (!$file_info) {
			...
			/**
			 * @hooked WAMS()->uploader()->remove_unused_uploads() - 10
			 */
			do_action('wams_after_move_temporary_files', $user_id, $new_files, $old_files);
		}
		...
		function remove_unused_uploads($user_id, $new_files, $old_files = array())
		{
			...
			$files = glob(WAMS()->uploader()->get_upload_base_dir() . $user_id . DIRECTORY_SEPARATOR . '*');
			if (!empty($files)) {
				foreach ($files as $file) {
					...
					$can_unlink = apply_filters('wams_can_remove_uploaded_file', true, $user_id, $str);
					if ($can_unlink) {
						unlink($file);
					}
				}
