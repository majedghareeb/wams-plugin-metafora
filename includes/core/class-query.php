...
if (!class_exists('wams\core\Query')) {
	...
	class Query
	{
		...
		public function ajax_paginate()
		{
			...
			if (!isset($_REQUEST['hook'])) {
				wp_send_json_error(__('Invalid hook.', 'wams'));
			}
		...
		public function wp_pages()
		{
			...
			// 	return 'reached_maximwams_limit';
			// }
			...
			if ($wpdb->num_rows > 0) {
				foreach ($pages as $page_data) {
					$array[$page_data->post_name] = $page_data->post_title;
				}
			}
		...
		public function make($args)
		{
			...
			if ('comment' === $args['post_type']) {
				 ...
				 *
				 * @since 1.3.x
				 * @hook  wams_excluded_comment_types
				 *
				 ...
				 *     return $types;
				 * }
				 * add_filter( 'wams_excluded_comment_types', 'my_excluded_comment_types' );
				 */
				$args['type__not_in'] = apply_filters('wams_excluded_comment_types', array(''));
		 ...
		 *
		 * @since 2.4.2 $status = 'unassigned' is unused. Please use `WAMS()->setup()->set_default_user_status()` instead. Will be deprecated since 3.0
		 *
		 * @param $status
		 *
		 * @return int
		...
		function count_users_by_status($status)
		{
			...
			$users_count = get_transient("wams_count_users_{$status}");
			if (false === $users_count) {
				...
				set_transient("wams_count_users_{$status}", $users_count);
			}
		...
		{
			$users_count = get_transient('wams_count_users_pending_dot');
			if (false === $users_count) {
				$args = array(
					'fields'               => 'ids',
					'number'               => 1,
				 ...
				 *
				 * @type filter
				 * @title wams_admin_pending_queue_filter
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_admin_pending_queue_filter', 'function_name', 10, 1 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_admin_pending_queue_filter', 'my_admin_pending_queue', 10, 1 );
				 ...
				 * ?>
				 */
				$args = apply_filters('wams_admin_pending_queue_filter', $args);
				...
				set_transient('wams_count_users_pending_dot', $users_count);
			}
		...
		function update_attr($key, $post_id, $new_value)
		{
			...
			update_post_meta($post_id, '_wams_' . $key, $new_value);
		}

		/**
		 * Get postmeta related to WAMS.
		 *
		 * @param string $key
		 * @param int    $post_id
		 *
		...
		{
			return get_post_meta($post_id, '_wams_' . $key, true);
		}
		...
		{
			$meta = delete_post_meta($post_id, '_wams_' . $key);
			return $meta;
		}
		...
		public function post_data($post_id)
		{
			...
			foreach ($meta as $k => $v) {
				if (strstr($k, '_wams_' . $mode . '_')) {
					$k           = str_replace('_wams_' . $mode . '_', '', $k);
					$array[$k] = $v[0];
				} elseif ('_wams_mode' === $k) {
					$k           = str_replace('_wams_', '', $k);
					$array[$k] = $v[0];
				} elseif (strstr($k, '_wams_')) {
					$k           = str_replace('_wams_', '', $k);
					$array[$k] = $v[0];
				}
			}
		...
		{
			$is_core = get_post_meta($post_id, '_wams_core', true);
			if ($is_core != '') {
				return $is_core;
			} else {
				return false;
