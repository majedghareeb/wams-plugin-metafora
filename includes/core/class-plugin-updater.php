...
if (!class_exists('wams\core\Plugin_Updater')) {
	...
	class Plugin_Updater
	{
		...
		{
			//cron request to WAMS()->store_url;
			// add_action('wams_weekly_scheduled_events', array(&$this, 'wams_checklicenses'));
		...
		function prevent_dangerous_auto_updates($should_update, $plugin)
		{
			...
			if ('wams/wams.php' !== $plugin->plugin) {
				return $should_update;
			}
				...
				foreach ($sites as $site) {
					...
					foreach ($the_plugs as $key => $value) {
						...
						if (in_array($value, array_keys($paid_extensions))) {
							$license = WAMS()->options()->get("wams_{$paid_extensions[$value]['key']}_license_key");

							if (empty($license)) {
								continue;
							}

							$active_wams_plugins[$value] = $paid_extensions[$value];
							$active_wams_plugins[$value]['license'] = $license;
						}
					}
		 ...
		 */
		function wams_checklicenses()
		{
			Logger::info('wams_as_analytics_import is called from admin class using cron');
		...
		function extend_download_url($download_url, $slug, $data)
		{
			...
			$api_params = array(
				...
				'item_name'     => str_replace('WAMS - ', '', $plugin_data['Name']),
