...
if (!class_exists('wams\core\Form')) {
	...
	class Form
	{
		...
		/**
		 * Whitelisted usermeta that can be stored when WAMS Form is submitted.
		 *
		 * @since 2.6.7
		 *
		 * @var array
		...
		public function ajax_muted_action()
		{
			...
			if (!isset($_REQUEST['hook'])) {
				die(esc_html__('Invalid hook', 'wams'));
			}

			if (isset($_REQUEST['user_id'])) {
				$user_id = absint($_REQUEST['user_id']);
			}
			if (!isset($user_id) || !WAMS()->roles()->wams_current_user_can('edit', $user_id)) {
			 ...
			 *
			 * @since 1.3.x
			 * @hook  wams_run_ajax_function__{$hook}
			 *
			 ...
			 *     // your code here
			 * }
			 * add_action( 'wams_run_ajax_function__{$hook}', 'my_run_ajax_function', 10, 1 );
			 */
			do_action("wams_run_ajax_function__{$hook}", $_REQUEST);
		...
		public function ajax_select_options()
		{
			...
			if (empty($_POST['child_callback'])) {
				...
				$arr_options['message'] = __('Wrong callback.', 'wams');
			...
			if (empty($allowed_callbacks)) {
				$arr_options['status']  = 'error';
				$arr_options['message'] = __('This is not possible for security reasons.', 'wams');
				wp_send_json($arr_options);
			}
			...
			if (WAMS()->fields()->is_source_blacklisted($ajax_source_func)) {
				$arr_options['status']  = 'error';
				$arr_options['message'] = __('This is not possible for security reasons.', 'wams');
			...
			if (isset($_POST['form_id'])) {
				WAMS()->fields()->set_id = absint($_POST['form_id']);
			}
			WAMS()->fields()->set_mode = 'profile';
			$form_fields             = WAMS()->fields()->get_fields();
			...
			if (isset($arr_options['post']['members_directory']) && 'yes' === $arr_options['post']['members_directory']) {
				 ...
				 *
				 * @type filter
				 * @title wams_ajax_select_options__debug_mode
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_ajax_select_options__debug_mode', 'function_name', 10, 1 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_ajax_select_options__debug_mode', 'my_ajax_select_options__debug_mode', 10, 1 );
				 ...
				 * ?>
				 */
				$debug = apply_filters('wams_ajax_select_options__debug_mode', false);
				if ($debug) {
				...
				if (!empty($_POST['child_callback']) && isset($form_fields[$_POST['child_name']])) {
					...
					if (
						isset($form_fields[$_POST['child_name']]['custom_dropdown_options_source']) &&
		...
		public function add_error($key, $error)
		{
			if (!isset($this->errors[$key])) {
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_submit_form_error', 'function_name', 10, 2 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_submit_form_error', 'my_submit_form_error', 10, 2 );
				 ...
				 * ?>
				 */
				$this->errors[$key] = apply_filters('wams_submit_form_error', $error, $key);
			}
		}
		...
		public function add_notice($key, $notice)
		{
			if (!isset($this->notices[$key])) {
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_submit_form_notice', 'function_name', 10, 2 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_submit_form_notice', 'my_submit_form_notice', 10, 2 );
				 ...
				 * ?>
				 */
				$this->notices[$key] = apply_filters('wams_submit_form_notice', $notice, $key);
			}
		}
		...
		{
			if (isset(WAMS()->builtin()->custom_fields)) {
				$this->all_fields = WAMS()->builtin()->custom_fields;
			} else {
		...
		public function clean_submitted_data($submitted)
		{
			foreach ($submitted as $metakey => $value) {
				if (WAMS()->user()->is_metakey_banned($metakey/*, 'submission'*/)) {
		...
		public function form_init()
		{
			...
			if ($http_post && !is_admin() && isset($_POST['form_id']) && is_numeric($_POST['form_id'])) {
				...
				if ('wams_form' !== get_post_type($this->form_id)) {
					return;
				}
				...
				// Verified that form_id is right and WAMS form is published. Then get form data.

				$this->form_data = WAMS()->query()->post_data($this->form_id);
				...
				$field_types_without_metakey = WAMS()->builtin()->get_fields_without_metakey();
				foreach ($custom_fields as $cf_k => $cf_data) {
					...
					if ('profile' === $this->form_data['mode']) {
						...
						if (!wams_can_edit_field($cf_data) || !wams_can_view_field($cf_data)) {
							$ignore_keys[] = $cf_k;
						}
					}
				...
				// The '_wams_last_login' cannot be updated through WAMS form.
				$cf_metakeys = array_values(array_diff($cf_metakeys, array('role_select', 'role_radio', 'role', '_wams_last_login', 'user_pass', 'user_password', 'confirm_user_password')));
				...
				if ('profile' === $this->form_data['mode']) {
					...
					$cf_metakeys = array_values(array_diff($cf_metakeys, WAMS()->fields()->get_restricted_fields_for_edit()));
					...
					if (!empty($this->form_data['use_custom_settings']) && !empty($this->form_data['show_bio'])) {
						$cf_metakeys[] = WAMS()->profile()->get_show_bio_key($this->form_data);
					} else {
				...
				/**
				 * Filters whitelisted usermeta keys that can be stored inside DB after WAMS Form submission.
				 *
				 * @param {array} $whitelisted_metakeys Whitelisted usermeta keys.
				 * @param {array} $form_data            WAMS form data.
				 *
				 * @return {array} Whitelisted usermeta keys.
				 *
				 * @since 2.6.7
				 * @hook wams_whitelisted_metakeys
				 *
				 * @example <caption>Extends whitelisted usermeta keys.</caption>
				 * function my_wams_whitelisted_metakeys( $metakeys, $form_data ) {
				 *     $metakeys[] = 'some_key';
				 *     return $metakeys;
				 * }
				 * add_filter( 'wams_whitelisted_metakeys', 'my_wams_whitelisted_metakeys', 10, 2 );
				 */
				$cf_metakeys = apply_filters('wams_whitelisted_metakeys', $cf_metakeys, $this->form_data);

				// Important variable to prevent save unnecessary data to wp_usermeta.
				$this->usermeta_whitelist = $cf_metakeys;

				/**
				 * Fires before WAMS login, registration or profile form submission.
				 *
				 * @since 1.3.x
				 * @hook wams_before_submit_form_post
				 *
				 * @param {array}  $post $_POST submission array. Deprecated since 2.0.
				 * @param {object} $this WAMS()->form() class instance. Since 2.6.7
				 *
				 * @example <caption>Make any custom action before WAMS login, registration or profile form submission.</caption>
				 * function my_custom_before_submit_form_post( $wams_form_obj ) {
				 *     // your code here
				 * }
				 * add_action( 'wams_before_submit_form_post', 'my_custom_before_submit_form_post' );
				 */
				do_action('wams_before_submit_form_post', $this);

				/* save entire form as global */
				/**
				 * Filters $_POST submitted data by the WAMS login, registration or profile form.
				 *
				 ...
				 *     return $_post;
				 * }
				 * add_filter( 'wams_submit_post_form', 'my_submit_post_form' );
				 */
				$this->post_form = apply_filters('wams_submit_post_form', wp_unslash($_POST));

				// Validate form submission by honeypot.
				if (isset($this->post_form[WAMS()->honeypot]) && '' !== $this->post_form[WAMS()->honeypot]) {
					// High level escape if hacking.
					wp_die(esc_html__('Hello, spam bot!', 'wams'));
				}
				 ...
				 * 9  - `wams_submit_form_data_trim_fields()` maybe over-functionality and can be removed.
				 * 10 - `wams_submit_form_data_role_fields()` important for conditional logic based on role fields in form.
				 *
				 ...
				 *
				 * @since 1.3.x
				 * @hook wams_submit_form_data
				 *
				 * @example <caption>Extends WAMS form submitted data.</caption>
				 * function my_submit_form_data( $_post, $mode, $all_cf_metakeys ) {
				 *     $_post['some_key'] = 'some value';
				 *     return $_post;
				 * }
				 * add_filter( 'wams_submit_form_data', 'my_submit_form_data', 10, 3 );
				 */
				$this->post_form = apply_filters('wams_submit_form_data', $this->post_form, $this->form_data['mode'], $all_cf_metakeys);
				/* Continue based on form mode - pre-validation */
				/**
				 * Fires for validation WAMS login, registration or profile form submission.
				 *
				 * Internal WAMS callbacks (Priority -> Callback name -> Excerpt):
				 * 10 - `wams_submit_form_errors_hook()` All form validation handlers.
				 * 20 - `wams_recaptcha_validate()`      reCAPTCHA form validation handlers. um-recaptcha extension.
				 *
				 * @since 1.3.x
				 * @hook wams_submit_form_errors_hook
				 *
				 ...
				 *     // your code here
				 * }
				 * add_action( 'wams_submit_form_errors_hook', 'my_custom_submit_form_errors_hook', 10, 2 );
				 */
				do_action('wams_submit_form_errors_hook', $this->post_form, $this->form_data);
				/* Continue based on form mode - store data. */
				/**
				 * Fires for make main actions on WAMS login, registration or profile form submission.
				 *
				 * Internal WAMS callbacks (Priority -> Callback name -> Excerpt):
				 * ### wams_submit_form_login:
				 * * 1 - `WAMS()->login()->verify_nonce()` Verify nonce.
				 * * 10 - `wams_submit_form_login()`       Login form main handler.
				 * ### wams_submit_form_register:
				 * * 1  - `WAMS()->register()->verify_nonce()`                 Verify nonce.
				 * * 9  - `WAMS()->agreement_validation()`                     GDPR Agreement.
				 * * 9  - `WAMS()->terms_conditions()->agreement_validation()` Terms & Conditions Agreement.
				 * * 10 - `wams_submit_form_register()`                        Register form main handler.
				 * ### wams_submit_form_profile:
				 * * 10 - `wams_submit_form_profile()` Profile form main handler.
				 *
				 * @since 1.3.x
				 * @hook wams_submit_form_errors_hook
				 *
				 ...
				 * add_action( 'wams_submit_form_errors_hook', 'my_custom_submit_form_errors_hook', 10, 2 );
				 */
				do_action("wams_submit_form_{$this->form_data['mode']}", $this->post_form, $this->form_data);
			}
		}
		...
		public function sanitize($form)
		{
			if (isset($form['form_id'])) {
						...
						foreach ($custom_fields as $k => $field) {
							...
							if (isset($field['type'])) {
								if (isset($form[$k])) {
									...
									switch ($field['type']) {
										default:
											$form[$k] = apply_filters('wams_sanitize_form_field', $form[$k], $field);
											break;
										case 'number':
											$form[$k] = '' !== $form[$k] ? (int) $form[$k] : '';
											break;
										case 'textarea':
											if (!empty($field['html']) || (WAMS()->profile()->get_show_bio_key($form) === $k && WAMS()->options()->get('profile_show_html_bio'))) {
												$allowed_html = WAMS()->get_allowed_html('templates');
				...
				$description_key = WAMS()->profile()->get_show_bio_key($this->form_data);
		...
		public function display_form_type($mode, $post_id)
		{
			...
			switch ($mode) {
				case 'login':
					$output = __('Login', 'wams');
					break;
				case 'profile':
					$output = __('Profile', 'wams');
					break;
				case 'register':
					$output = __('Register', 'wams');
					break;
			}
			return $output;
		}
		...
		public function assigned_role($post_id)
		{
			...
			$wams_global_role = WAMS()->options()->get('register_role'); // WAMS Settings Global settings
			if (!empty($wams_global_role)) {
				$global_role = $wams_global_role; // Form Global settings
			}
			 ...
			 */
			$use_custom = get_post_meta($post_id, "_wams_{$mode}_use_custom_settings", true);
			if ($use_custom) { // Custom Form settings
				$role = get_post_meta($post_id, "_wams_{$mode}_role", true);
			}
		...
		{
			$mode = get_post_meta($post_id, '_wams_mode', true);
			return $mode;
		}
			...
			foreach ($fields as $field_key => $field_settings) {
				...
				if (strstr($field_key, 'role_') && array_key_exists('options', $field_settings) && is_array($field_settings['options'])) {
					...
					if (isset($this->post_form['mode']) && 'profile' === $this->post_form['mode']) {
						...
						if ((array_key_exists('editable', $field_settings) && empty($field_settings['editable'])) || !wams_can_edit_field($field_settings)) {
							continue;
						}
					}

					if (!wams_can_view_field($field_settings)) {
						continue;
					}
