...
if (!class_exists('wams\core\Fields')) {
	...
	class Fields
	{
		...
		function checkbox($id, $title, $checked = true)
		{
			...
			$class = $checked ? 'wams-icon-android-checkbox-outline' : 'wams-icon-android-checkbox-outline-blank';

?>


			<div class="wams-field wams-field-c">
				<div class="wams-field-area">
					<label class="wams-field-checkbox<?php echo $checked ? ' active' : '' ?>">
						<input type="checkbox" name="<?php echo esc_attr($id); ?>" value="1" <?php checked($checked) ?> />
						<span class="wams-field-checkbox-state"><i class="<?php echo esc_attr($class) ?>"></i></span>
						<span class="wams-field-checkbox-option"> <?php echo esc_html($title); ?></span>
					</label>
				</div>
			</div>
		...
		public function show_social_urls()
		{
			...
			$fields = WAMS()->builtin()->get_all_user_fields();
			...
			?>
					...
					<a href="<?php echo esc_url(wams_filtered_social_link($k, $match)); ?>" style="background: <?php echo esc_attr($arr['color']); ?>;" target="<?php echo esc_attr($arr['url_target']); ?>" class="wams-tip-n" title="<?php echo esc_attr($arr['title']); ?>"><i class="<?php echo esc_attr($arr['icon']); ?>"></i></a>
		...
		function add_hidden_field($field)
		{
			...
			// $fields = WAMS()->builtin()->get_specific_fields( $field );
		...
		{
			$fields = WAMS()->builtin()->saved_fields;
			...
			if (array_key_exists('custom_dropdown_options_source', $args)) {
				if (function_exists(wp_unslash($args['custom_dropdown_options_source']))) {
					if (!$this->is_source_blacklisted($args['custom_dropdown_options_source'])) {
						$allowed_callbacks = WAMS()->options()->get('allowed_choice_callbacks');
						...
						WAMS()->options()->update('allowed_choice_callbacks', $allowed_callbacks);
					}
				}
			}
			...
			unset($fields[$id]['in_colwamsn']);
			unset($fields[$id]['in_group']);
			unset($fields[$id]['position']);

			do_action('wams_add_new_field', $id, $args);

			update_option('wams_fields', $fields);
		}
		...
		function update_field($id, $args, $form_id)
		{
			...
			WAMS()->query()->update_attr('custom_fields', $form_id, $fields);
		}
		...
		public function delete_field_from_form($id, $form_id)
		{
			...
			if (isset($fields[$id])) {
				$condition_fields = get_option('wams_fields', array());
				...
				update_option('wams_fields', $condition_fields);
				unset($fields[$id]);
				WAMS()->query()->update_attr('custom_fields', $form_id, $fields);
			}
		}
		...
		public function delete_field_from_db($id)
		{
			...
			if (isset($fields[$id])) {
				...
				do_action('wams_delete_custom_field', $id, $args);

				update_option('wams_fields', $fields);

				global $wpdb;
				$forms = $wpdb->get_col("SELECT ID FROM {$wpdb->posts} WHERE post_type = 'wams_form'");
				foreach ($forms as $form_id) {
					$form_fields = get_post_meta($form_id, '_wams_custom_fields', true);
					unset($form_fields[$id]);
					update_post_meta($form_id, '_wams_custom_fields', $form_fields);
				}

				$directories = $wpdb->get_col("SELECT ID FROM {$wpdb->posts} WHERE post_type = 'wams_directory'");
				foreach ($directories as $directory_id) {
					// Frontend filters
					$directory_search_fields = get_post_meta($directory_id, '_wams_search_fields', true);
					...
					update_post_meta($directory_id, '_wams_search_fields', $directory_search_fields);

					// Admin filtering
					$directory_search_filters = get_post_meta($directory_id, '_wams_search_filters', true);
					if (isset($directory_search_filters[$id])) {
						unset($directory_search_filters[$id]);
					}
					update_post_meta($directory_id, '_wams_search_filters', $directory_search_filters);

					// display in tagline
					$directory_reveal_fields = get_post_meta($directory_id, '_wams_reveal_fields', true);
					...
					update_post_meta($directory_id, '_wams_reveal_fields', $directory_reveal_fields);

					// extra user information section
					$directory_tagline_fields = get_post_meta($directory_id, '_wams_tagline_fields', true);
					...
					update_post_meta($directory_id, '_wams_tagline_fields', $directory_tagline_fields);

					// Custom fields selected in "Choose field(s) to enable in sorting"
					$directory_sorting_fields = get_post_meta($directory_id, '_wams_sorting_fields', true);
					...
					update_post_meta($directory_id, '_wams_sorting_fields', $directory_sorting_fields);

					// If "Default sort users by" = "Other (Custom Field)" is selected when delete this custom field and set default sorting
					$directory_sortby_custom = get_post_meta($directory_id, '_wams_sortby_custom', true);
					if ($directory_sortby_custom === $id) {
						$directory_sortby = get_post_meta($directory_id, '_wams_sortby', true);
						if ('other' === $directory_sortby) {
							update_post_meta($directory_id, '_wams_sortby', 'user_registered_desc');
						}
						update_post_meta($directory_id, '_wams_sortby_custom', '');
						update_post_meta($directory_id, '_wams_sortby_custom_label', '');
						update_post_meta($directory_id, '_wams_sortby_custom_type', '');
						update_post_meta($directory_id, '_wams_sortby_custom_order', '');
					}
				}
			}
		}
		...
		{
			$fields      = WAMS()->query()->get_attr('custom_fields', $form_id);
			$field_scope = WAMS()->builtin()->saved_fields;
		...
		{
			$fields      = WAMS()->query()->get_attr('custom_fields', $form_id);
			$field_scope = WAMS()->builtin()->predefined_fields;
		...
		{
			$fields     = WAMS()->query()->get_attr('custom_fields', $form_id);
			$all_fields = WAMS()->builtin()->saved_fields;
			...
			unset($all_fields[$new_metakey]['in_row'], $all_fields[$new_metakey]['in_sub_row'], $all_fields[$new_metakey]['in_colwamsn'], $all_fields[$new_metakey]['in_group'], $all_fields[$new_metakey]['position']);

			do_action('wams_add_new_field', $new_metakey, $duplicate);

			WAMS()->query()->update_attr('custom_fields', $form_id, $fields);
			update_option('wams_fields', $all_fields);
		}
		...
		public function aria_valid_attributes($is_error, $field_id, $context = 'error')
		{
			...
			if ($is_error) {
				if ('notice' === $context) {
					$errormessage_id = 'wams-notice-for-' . $field_id;
				} else {
					$errormessage_id = 'wams-error-for-' . $field_id;
				}
		...
		public function field_error($text, $input_id, $force_show = false)
		{
			...
			$error_id = 'wams-error-for-' . $input_id;

			if ($force_show) {
				$output = '<div class="wams-field-error" id="' . esc_attr($error_id) . '"><span class="wams-field-arrow"><i class="wams-faicon-caret-up"></i></span>' . wp_kses($text, WAMS()->get_allowed_html('templates')) . '</div>';
				return $output;
			}

			if (isset($this->set_id) && WAMS()->form()->processing === $this->set_id) {
				$output = '<div class="wams-field-error" id="' . esc_attr($error_id) . '"><span class="wams-field-arrow"><i class="wams-faicon-caret-up"></i></span>' . wp_kses($text, WAMS()->get_allowed_html('templates')) . '</div>';
			} else {
				$output = '';
			}

			if (!WAMS()->form()->processing) {
				$output = '<div class="wams-field-error" id="' . esc_attr($error_id) . '"><span class="wams-field-arrow"><i class="wams-faicon-caret-up"></i></span>' . wp_kses($text, WAMS()->get_allowed_html('templates')) . '</div>';
			}
		...
		public function field_notice($text, $input_id, $force_show = false)
		{
			...
			$notice_id = 'wams-notice-for-' . $input_id;

			if ($force_show) {
				$output = '<div class="wams-field-notice" id="' . esc_attr($notice_id) . '"><span class="wams-field-arrow"><i class="wams-faicon-caret-up"></i></span>' . wp_kses($text, WAMS()->get_allowed_html('templates')) . '</div>';
				return $output;
			}
		...
		{
			return WAMS()->form()->has_error($key);
		}
		...
		{
			return WAMS()->form()->has_notice($key);
		}
		...
		{
			$output  = null;
			$output .= '<div class="wams-field-label">';

			if (!empty($data['icon']) && isset($this->field_icons) && 'off' !== $this->field_icons && ('label' === $this->field_icons || true === $this->viewing)) {
				$output .= '<div class="wams-field-label-icon"><i class="' . esc_attr($data['icon']) . '" aria-label="' . esc_attr($label) . '"></i></div>';
			}
			...
			if (true === $this->viewing) {
				 ...
				 *     return $label;
				 * }
				 * add_filter( 'wams_view_label_first_name', 'my_change_first_name_label', 10, 2 );
				 */
				$label = apply_filters("wams_view_label_{$key}", $label, $data);
			} else {
				/**
				 ...
				 *     return $label;
				 * }
				 * add_filter( 'wams_edit_label_first_name', 'my_change_first_name_label', 10, 2 );
				 */
				$label = apply_filters("wams_edit_label_{$key}", $label, $data);
				/**
				 * Filters Ultimate Member field label on the Profile form: Edit mode.
				 *
				 * @since 2.0.0
				 *
				 * @hook wams_edit_label_all_fields
				 *
				 * @param {string} $label Field label.
				 * @param {string} $data  Field data.
				 *
				 ...
				 *     return $label;
				 * }
				 * add_filter( 'wams_edit_label_all_fields', 'my_change_first_name_label', 10, 2 );
				 */
				$label = apply_filters('wams_edit_label_all_fields', $label, $data);
			}

			$fields_without_metakey = WAMS()->builtin()->get_fields_without_metakey();
			$for_attr               = '';
			if (!in_array($data['type'], $fields_without_metakey, true)) {
				$for_attr = ' for="' . esc_attr($key . WAMS()->form()->form_suffix) . '"';
			}
			...
			if (!empty($data['help']) && false === $this->viewing && false === strpos($key, 'confirm_user_pass')) {
				if (!WAMS()->mobile()->isMobile()) {
					if (false === $this->disable_tooltips) {
						$output .= '<span class="wams-tip wams-tip-' . (is_rtl() ? 'e' : 'w') . '" title="' . esc_attr__($data['help'], 'ultimate-member') . '"><i class="wams-icon-help-circled"></i></span>';
					}
				}

				if (false !== $this->disable_tooltips || WAMS()->mobile()->isMobile()) {
					$output .= '<span class="wams-tip-text">' . __($data['help'], 'ultimate-member') . '</span>';
				}
			}

			$output .= '<div class="wams-clear"></div></div>';
		...
		public function get_class($key, $data, $add = null)
		{
			...
			if ($data['type'] == 'date') {
				$classes .= 'wams-datepicker ';
			}

			if ($data['type'] == 'time') {
				$classes .= 'wams-timepicker ';
			}
			 ...
			 *
			 * @type filter
			 * @title wams_extend_field_classes
			 ...
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_extend_field_classes', 'function_name', 10, 3 );
			 * @example
			 * <?php
			 * add_filter( 'wams_extend_field_classes', 'my_extend_field_classes', 10, 3 );
			 ...
			 * ?>
			 */
			$classes = apply_filters('wams_extend_field_classes', $classes, $key, $data);
		...
		{
			// preview in backend
			if (isset(WAMS()->user()->preview) && WAMS()->user()->preview) {
			...
			if (isset($_SESSION) && isset($_SESSION['wams_social_profile'][$key]) && isset($this->set_mode) && $this->set_mode == 'register') {
				return $_SESSION['wams_social_profile'][$key];
			}

			$type = (isset($data['type'])) ? $data['type'] : '';

			// normal state
			if (isset(WAMS()->form()->post_form[$key])) {
				...
				if ('profile' === $this->set_mode) {
					if (!isset(WAMS()->form()->post_form['profile_nonce']) || false === wp_verify_nonce(WAMS()->form()->post_form['profile_nonce'], 'wams-profile-nonce' . WAMS()->user()->target_id)) {
						return '';
					}
				}

				return stripslashes_deep(WAMS()->form()->post_form[$key]);
			} elseif (true === $this->editing && wams_user($key)) {
				 ...
				 *
				 * @type filter
				 * @title wams_edit_{$key}_field_value
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_edit_{$key}_field_value', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_filter( 'wams_edit_{$key}_field_value', 'my_edit_field_value', 10, 2 );
				 ...
				 * ?>
				 */
				$value = apply_filters("wams_edit_{$key}_field_value", $value, $key);
				/**
				 * WAMS hook
				 *
				 * @type filter
				 * @title wams_edit_{$type}_field_value
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_edit_{$type}_field_value', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_filter( 'wams_edit_{$type}_field_value', 'my_edit_field_value', 10, 2 );
				 * function my_edit_field_value( $value, $key ) {
				 *     // your code here
				 *     return $value;
				 * }
				 * ?>
				 */
				$value = apply_filters("wams_edit_{$type}_field_value", $value, $key);
			} elseif (true === $this->viewing && (wams_user($key) || isset($data['show_anyway']))) {

				return wams_filtered_value($key, $data);
			} elseif (isset(WAMS()->user()->profile[$key])) {

				$value = WAMS()->user()->profile[$key];
				/**
				 * WAMS hook
				 *
				 * @type filter
				 ...
				 *
				 * @type filter
				 * @title wams_field_default_value
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_field_default_value', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_filter( 'wams_field_default_value', 'my_field_default_value', 10, 2 );
				 ...
				 * ?>
				 */
				$default = apply_filters('wams_field_default_value', $default, $data, $type);
				/**
				 * WAMS hook
				 *
				 * @type filter
				 * @title wams_field_{$key}_default_value
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_field_{$key}_default_value', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_filter( 'wams_field_{$key}_default_value', 'my_field_default_value', 10, 2 );
				 ...
				 * ?>
				 */
				$default = apply_filters("wams_field_{$key}_default_value", $default, $data);
				/**
				 * WAMS hook
				 *
				 * @type filter
				 * @title wams_field_{$type}_default_value
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_field_{$type}_default_value', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_filter( 'wams_field_{$type}_default_value', 'my_field_default_value', 10, 2 );
				 * function my_field_default_value( $default, $data ) {
				 *     // your code here
				 *     return $default;
				 * }
				 * ?>
				 */
				$default = apply_filters("wams_field_{$type}_default_value", $default, $data);
			}
			...
			if (!isset($value) && ($this->set_mode == 'register' || true === $this->editing)) {
				 ...
				 * ?>
				 */
				$value = apply_filters("wams_edit_{$key}_field_value", $default, $key);
			 ...
			 * {"var":"$default","type":"string","desc":"Field Default Value"},
			 * {"var":"$data","type":"array","desc":"Field Data"}]
			 * @usage add_filter( 'wams_field_value', 'function_name', 10, 5 );
			 */
			return apply_filters('wams_field_value', $value, $default, $key, $type, $data);
		}
		...
		public function is_selected($key, $value, $data)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_is_selected_filter_key
			 * @description Change is selected filter key
			 * @input_vars
			 * [{"var":"$key","type":"string","desc":"Selected filter key"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_is_selected_filter_key', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_filter( 'wams_is_selected_filter_key', 'my_selected_filter_key', 10, 1 );
			 ...
			 * ?>
			 */
			$key = apply_filters('wams_is_selected_filter_key', $key);

			if (isset(WAMS()->form()->post_form[$key])) {

				if (is_array(WAMS()->form()->post_form[$key])) {

					if (in_array($value, WAMS()->form()->post_form[$key])) {
						return true;
					}

					$stripslashed = array_map('stripslashes', WAMS()->form()->post_form[$key]);
					if (in_array($value, $stripslashed)) {
						return true;
					}

					if (in_array(html_entity_decode($value), WAMS()->form()->post_form[$key])) {
						return true;
					}
				} else {

					if ($value == WAMS()->form()->post_form[$key]) {
						return true;
					}
				}
			} else {

				$field_value = wams_user($key);
				if (!$field_value) {
					$field_value = 0;
				}
				...
				if (strstr($key, 'role_') || 'role' === $key) {
					...
					if (!empty($role_keys)) {
						$field_value = WAMS()->roles()->get_editable_priority_user_role(wams_user('ID'));
						if (!empty($field_value)) {
							$field_value = strtolower($field_value);
							if (in_array($field_value, $role_keys, true)) {
								$field_value = 'wams_' . $field_value;
							}
						}
					}
				}
				 ...
				 *
				 * @type filter
				 * @title wams_is_selected_filter_value
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_is_selected_filter_value', 'function_name', 10, 2 );
				 * @example
				 * <?php
				 * add_filter( 'wams_is_selected_filter_value', 'my_selected_filter_value', 10, 2 );
				 ...
				 * ?>
				 */
				$field_value = apply_filters('wams_is_selected_filter_value', $field_value, $key, $value);

				/**
				 * WAMS hook
				 *
				 * @type filter
				 * @title wams_is_selected_filter_data
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_is_selected_filter_data', 'function_name', 10, 3 );
				 * @example
				 * <?php
				 * add_filter( 'wams_is_selected_filter_data', 'my_selected_filter_data', 10, 3 );
				 ...
				 * ?>
				 */
				$data = apply_filters('wams_is_selected_filter_data', $data, $key, $field_value);
				...
				if (false === $this->editing || 'custom' === $this->set_mode) {
					...
					$direct_db_value = $wpdb->get_var($wpdb->prepare("SELECT ISNULL( meta_value ) FROM {$wpdb->usermeta} WHERE user_id = %d AND meta_key = %s", wams_user('ID'), $key));
		...
		function is_radio_checked($key, $value, $data)
		{
			...
			if (isset(WAMS()->form()->post_form[$key])) {
				if (is_array(WAMS()->form()->post_form[$key]) && in_array($value, WAMS()->form()->post_form[$key])) {
					return true;
				} elseif ($value == WAMS()->form()->post_form[$key]) {
					return true;
				}
			} else {
				...
				if (true === $this->editing && 'custom' !== $this->set_mode) {
					if (wams_user($key)) {
						...
						$wams_user_value = wams_user($key);

						if (strstr($key, 'role_') || $key == 'role') {
							$wams_user_value = strtolower(WAMS()->roles()->get_editable_priority_user_role(wams_user('ID')));

							$role_keys = get_option('wams_roles', array());

							if (!empty($role_keys)) {
								if (in_array($wams_user_value, $role_keys)) {
									$wams_user_value = 'wams_' . $wams_user_value;
								}
							}
						}

						if ($wams_user_value == $value) {
							return true;
						}

						if (is_array($wams_user_value) && in_array($value, $wams_user_value)) {
							return true;
						}

						if (is_array($wams_user_value)) {
							foreach ($wams_user_value as $u) {
		...
		{
			$fields = WAMS()->builtin()->all_user_fields;
		...
		public function dropdown_options_source_blacklist()
		{
			...
			$blacklist = apply_filters('wams_dropdown_options_source_blacklist', $blacklist);
			return $blacklist;
		}
		...
		function get_option_value_from_callback($value, $data, $type)
		{
			...
			if (in_array($type, array('select', 'multiselect')) && !empty($data['custom_dropdown_options_source'])) {
				...
				$has_custom_source = apply_filters("wams_has_dropdown_options_source__{$data['metakey']}", false);

				if ($has_custom_source) {

					/** This filter is docwamsented in includes/core/class-fields.php */
					$opts        = apply_filters("wams_get_field__{$data['metakey']}", array());
					...
					if (isset($data['parent_dropdown_relationship'])) {
						...
						$_POST['parent_option'] = wams_user($data['parent_dropdown_relationship']);
		...
		function get_label($key)
		{
			 ...
			 *     return $label;
			 * }
			 * add_filter( 'wams_change_field_label', 'my_change_field_label', 10, 3 );
			 */
			$label = apply_filters('wams_change_field_label', $label, $key, $fields[$key]);
		...
		public function get_fields()
		{
			...
			if (empty($this->fields[$this->set_id])) {
				 ...
				 *     return $fields;
				 * }
				 * add_filter( 'wams_get_form_fields', 'my_form_fields', 10, 2 );
				 */
				$this->fields[$this->set_id] = apply_filters('wams_get_form_fields', array(), $this->set_id);
			}
		...
		public function get_field($key)
		{
			...
			if (isset($fields) && is_array($fields) && isset($fields[$key])) {
				...
				if (!isset(WAMS()->builtin()->predefined_fields[$key]) && !isset(WAMS()->builtin()->all_user_fields[$key])) {
					return '';
				}
				$array = (isset(WAMS()->builtin()->predefined_fields[$key])) ? WAMS()->builtin()->predefined_fields[$key] : WAMS()->builtin()->all_user_fields[$key];
			}
			...
			if (isset($array['conditions']) && is_array($array['conditions']) && false === $this->viewing) {
				...
				$array['classes'] .= ' wams-is-conditional';
			} else {
			...
			if (!in_array($array['type'], $fields_without_metakey, true)) {
				$array['classes'] .= ' wams-field-' . esc_attr($key);
			}
			$array['classes'] .= ' wams-field-' . esc_attr($array['type']);
			$array['classes'] .= ' wams-field-type_' . esc_attr($array['type']);
			 ...
			 *     return $field_data;
			 * }
			 * add_filter( 'wams_get_field_date', 'my_custom_get_field_date' );
			 */
			$array = apply_filters("wams_get_field_{$array['type']}", $array);
			/**
			 ...
			 *     return $field_data;
			 * }
			 * add_filter( 'wams_get_field__first_name', 'my_custom_disable_first_name' );
			 */
			return apply_filters("wams_get_field__{$key}", $array);
		}
		...
		function filter_field_non_utf8_value($option_value)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_field_non_utf8_value
			 * @description Change dropdown option text
			 * @input_vars
			 * [{"var":"$value","type":"string","desc":"Option Value"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_field_non_utf8_value', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_filter( 'wams_field_non_utf8_value', 'my_field_non_utf8_value', 10, 1 );
			 ...
			 * ?>
			 */
			return apply_filters('wams_field_non_utf8_value', $option_value);
		}
		 ...
		 *
		 * @param bool|int $_wams_profile_id
		 *
		 * @return array
		 */
		public function get_restricted_fields_for_edit($_wams_profile_id = false)
		{
			// fields that need to be disabled in edit mode (profile)
			$arr_restricted_fields = array('user_email', 'username', 'user_login', 'user_password', '_wams_last_login', 'user_registered');
			/**
			 * Filters the form fields that need to be disabled in edit mode (profile).
			 *
			 * @param {array} $fields         Form fields.
			 * @param {int}   $_wams_profile_id User Profile ID.
			 *
			 * @return {array} Form fields.
			 *
			 * @since 2.0
			 * @hook wams_user_profile_restricted_edit_fields
			 *
			 * @example <caption>Make user email field editable on the Profile Page.</caption>
			 * function my_make_email_editable( $fields, $_wams_profile_id ) {
			 *     $fields = array_flip( $fields );
			 *     unset( $fields['user_email'] );
			 *     $fields = array_keys( $fields );
			 *     return $fields;
			 * }
			 * add_filter( 'wams_user_profile_restricted_edit_fields', 'my_make_email_editable', 10, 2 );
			 */
			return apply_filters('wams_user_profile_restricted_edit_fields', $arr_restricted_fields, $_wams_profile_id);
		}
		...
		{
			global $_wams_profile_id;

			if (!empty($data['is_block'])) {
				$form_suffix = '';
			} else {
				$form_suffix = WAMS()->form()->form_suffix;
			}

			$output = '';

			if (empty($_wams_profile_id)) {
				$_wams_profile_id = wams_user('ID');
			}
			...
			if (isset($data['required_perm']) && !WAMS()->roles()->wams_user_can($data['required_perm'])) {
				return '';
			}

			// fields that need to be disabled in edit mode (profile) (email, username, etc.)
			$arr_restricted_fields = $this->get_restricted_fields_for_edit($_wams_profile_id);
			...
			if (!wams_can_view_field($data)) {
				return '';
			}

			wams_fetch_user($_wams_profile_id);
			...
			if (true === $this->editing && 'profile' === $this->set_mode) {
				if (!WAMS()->roles()->wams_user_can('can_edit_everyone')) {
			 ...
			 *     return $disabled;
			 * }
			 * add_filter( 'wams_is_field_disabled', 'my_is_field_disabled', 10, 2 );
			 */
			$disabled = apply_filters('wams_is_field_disabled', $disabled, $data);
			 ...
			 *
			 * @since 1.3.x
			 * @hook  wams_hook_for_field_{$type}
			 *
			 ...
			 *     return $type;
			 * }
			 * add_filter( 'wams_hook_for_field_{$type}', 'my_field_type', 10, 1 );
			 */
			$type = apply_filters("wams_hook_for_field_{$type}", $type);
			switch ($type) {
				case 'textarea':
				case 'multiselect':
					$field_id    = $key;
			 ...
			 *
			 * @since 2.0.13
			 * @hook  wams_completeness_field_id
			 *
			 * @param {string} $field_id Field id.
			 * @param {array}  $data     Field Data.
			 * @param {array}  $args     Optional field argwamsents.
			 *
			 ...
			 *     return $field_id;
			 * }
			 * add_filter( 'wams_completeness_field_id', 'function_name', 10, 3 );
			 */
			$field_id = apply_filters('wams_completeness_field_id', $field_id, $data, $args);
			...
			switch ($type) {
				...
				default:
					 ...
					 *
					 * @since 1.3.x
					 * @hook  wams_edit_field_{$mode}_{$type}
					 *
					 ...
					 *     return $output;
					 * }
					 * add_filter( 'wams_edit_field_{$mode}_{$type}', 'my_edit_field_html', 10, 2 );
					 */
					$output .= apply_filters("wams_edit_field_{$mode}_{$type}", $output, $data);
					break;
					/* Other fields */
				case 'googlemap':
				case 'youtube_video':
				...
				case 'soundcloud_track':
					...
					$output .= '<div class="wams-field-area">';

					if (!empty($data['icon']) && isset($this->field_icons) && 'field' === $this->field_icons) {
						$output .= '<div class="wams-field-icon"><i class="' . esc_attr($data['icon']) . '"></i></div>';
					}
				...
				case 'nwamsber':
					...
					$nwamsber_limit = '';
					if (isset($data['min'])) {
						$nwamsber_limit .= ' min="' . esc_attr($data['min']) . '" ';
					}
					if (isset($data['max'])) {
						$nwamsber_limit .= ' max="' . esc_attr($data['max']) . '" ';
					}

					$field_name  = $key . $form_suffix;
					$field_value = $this->field_value($key, $default, $data);

					$output .= '<input ' . $disabled . ' class="' . esc_attr($this->get_class($key, $data)) . '" type="nwamsber" name="' . esc_attr($field_name) . '" id="' . esc_attr($field_name) . '" value="' . esc_attr($field_value) . '" placeholder="' . esc_attr($placeholder) . '" data-validate="' . esc_attr($validate) . '" data-key="' . esc_attr($key) . '" ' . $nwamsber_limit . ' ' . $this->aria_valid_attributes($this->is_error($key), $field_name) . '/>
				...
				case 'password':
					...
					if ('single_user_password' === $key) {
						...
						if (WAMS()->options()->get('toggle_password')) {
							$output .= '<div class="wams-field-area-password">
									...
									<span class="wams-toggle-password"><i class="wams-icon-eye"></i></span>
								</div>
							</div>';
						} else {
						...
						if (('account' === $this->set_mode || wams_is_core_page('account')) && WAMS()->account()->current_password_is_required('password')) {
						...
						if (('account' === $this->set_mode && wams_is_core_page('account')) || ('password' === $this->set_mode && wams_is_core_page('password-reset'))) {
						...
						if ('password' === $this->set_mode && wams_is_core_page('password-reset')) {
							$name = $key;
						}
						...
						if ('login' !== $this->set_mode && !empty($data['force_confirm_pass'])) {
							...
							if (WAMS()->options()->get('toggle_password')) {
								$output .= '<div class="wams-field-area-password"><input class="' . esc_attr($this->get_class($key, $data)) . '" type="' . esc_attr($input) . '" name="' . esc_attr($name) . '" id="' . esc_attr($key . $form_suffix) . '" value="' . esc_attr($this->field_value($key, $default, $data)) . '" placeholder="' . esc_attr($placeholder) . '" data-validate="' . esc_attr($validate) . '" data-key="' . esc_attr($key) . '" ' . $this->aria_valid_attributes($this->is_error($key), $name) . '/><span class="wams-toggle-password"><i class="wams-icon-eye"></i></span></div>';
							} else {
				...
				case 'date':
					...
					if ($value) {
						// nwamseric (either unix or YYYYMMDD). ACF uses Ymd format of date inside the meta tables.
						if (is_nwamseric($value) && strlen($value) !== 8) {
							$unixtimestamp = $value;
						} else {
				...
				case 'textarea':
					...
					$bio_key = WAMS()->profile()->get_show_bio_key($this->global_args);
					...
					if (!empty($data['html']) && $bio_key !== $key) {
						$textarea_settings = array(
							...
							'tinymce'       => array(
								'toolbar1' => 'formatselect,bullist,nwamslist,bold,italic,underline,forecolor,blockquote,hr,removeformat,link,unlink,undo,redo',
								'toolbar2' => '',
							),
						);
						 ...
						 *
						 * @since 1.3.x
						 * @hook  wams_form_fields_textarea_settings
						 *
						 ...
						 * }
						 * add_filter( 'wams_form_fields_textarea_settings', 'function_name', 10, 2 );
						 */
						$textarea_settings = apply_filters('wams_form_fields_textarea_settings', $textarea_settings, $data);
				...
				case 'rating':
					...
					$output .= '<div class="wams-rating wams-raty" id="' . esc_attr($key) . '" data-key="' . esc_attr($key) . '" data-nwamsber="' . esc_attr($data['nwamsber']) . '" data-score="' . $this->field_value($key, $default, $data) . '" ' . $this->aria_valid_attributes($this->is_error($key), $key) . '></div>';
					$output .= '</div>';
				...
				case 'divider':
					...
					if (!empty($data['divider_text'])) {
						$output .= '<div class="wams-field-divider-text"><span>' . esc_html($data['divider_text']) . '</span></div>';
					}
					$output .= '</div>';
					break;
					/* Single Image Upload */
				case 'image':
					...
					$output     .= '<div class="wams-field-area" style="text-align: center;">';

					if (!empty($field_value) && 'empty_file' !== $field_value) {
						if (!in_array($key, array('profile_photo', 'cover_photo'), true)) {
							//							if ( isset( $this->set_mode ) && 'register' === $this->set_mode ) {
							//								$image_info = get_transient( "wams_{$field_value}" );
							//							} else {
							//								$image_info = wams_user( $data['metakey'] . '_metadata' );
							//							}

							if ((isset($this->set_mode) && 'register' === $this->set_mode) || file_exists(WAMS()->uploader()->get_core_temp_dir() . DIRECTORY_SEPARATOR . $field_value)) {
								$img_value = WAMS()->uploader()->get_core_temp_url() . '/' . $this->field_value($key, $default, $data);
							} else {
								$img_value = WAMS()->files()->get_download_link($this->set_id, $key, wams_user('ID'));
							}
							$img = '<img src="' . esc_attr($img_value) . '" alt="" />';
						} else {
							$img = '';
						}
						$output .= '<div class="wams-single-image-preview show ' . esc_attr($data['crop_class']) . '" data-crop="' . esc_attr($data['crop_data']) . '" data-key="' . esc_attr($key) . '">';
						if (empty($disabled)) {
							$output .= '<a href="javascript:void(0);" class="cancel"><i class="wams-icon-close"></i></a>';
						}
						$output .= $img;
						$output .= '</div>';
						if (empty($disabled)) {
							$output .= '<a href="javascript:void(0);" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="wams-button wams-btn-auto-width">' . esc_html__('Change photo', 'ultimate-member') . '</a>';
						}
					} else {
						$output .= '<div class="wams-single-image-preview ' . esc_attr($data['crop_class']) . '" data-crop="' . esc_attr($data['crop_data']) . '" data-key="' . esc_attr($key) . '">';
						if (empty($disabled)) {
							$output .= '<a href="javascript:void(0);" class="cancel"><i class="wams-icon-close"></i></a>';
						}
						$output .= '<img src="" alt="" /><div class="wams-clear"></div></div>';
						if (empty($disabled)) {
							$output .= '<a href="javascript:void(0);" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="wams-button wams-btn-auto-width">' . esc_html($data['button_text']) . '</a>';
						}
					}
					$output .= '</div>';
					/* modal hidden */
					if (empty($disabled)) {
						...
						$output .= '<div class="wams-modal-hidden-content">';
						$output .= '<div class="wams-modal-header"> ' . esc_html($modal_label) . '</div>';
						$output .= '<div class="wams-modal-body">';
						...
						$nonce   = wp_create_nonce('wams_upload_nonce-' . $this->timestamp);
						$output .= '<div class="wams-single-image-preview ' . esc_attr($data['crop_class']) . '"  data-crop="' . esc_attr($data['crop_data']) . '" data-ratio="' . esc_attr($data['ratio']) . '" data-min_width="' . esc_attr($data['min_width']) . '" data-min_height="' . esc_attr($data['min_height']) . '" data-coord=""><a href="javascript:void(0);" class="cancel"><i class="wams-icon-close"></i></a><img src="" alt="" /><div class="wams-clear"></div></div><div class="wams-clear"></div>';
						$output .= '<div class="wams-single-image-upload" data-user_id="' . esc_attr($_wams_profile_id) . '" data-nonce="' . esc_attr($nonce) . '" data-timestamp="' . esc_attr($this->timestamp) . '" ' . $data_icon . ' data-set_id="' . esc_attr($set_id) . '" data-set_mode="' . esc_attr($set_mode) . '" data-type="' . esc_attr($type) . '" data-key="' . esc_attr($key) . '" data-max_size="' . esc_attr($data['max_size']) . '" data-max_size_error="' . esc_attr($data['max_size_error']) . '" data-min_size_error="' . esc_attr($data['min_size_error']) . '" data-extension_error="' . esc_attr($data['extension_error']) . '"  data-allowed_types="' . esc_attr($allowed_types) . '" data-upload_text="' . esc_attr($data['upload_text']) . '" data-max_files_error="' . esc_attr($data['max_files_error']) . '" data-upload_help_text="' . esc_attr($data['upload_help_text']) . '">' . esc_html($data['button_text']) . '</div>';
						$output .= '<div class="wams-modal-footer">
									<div class="wams-modal-right">
										<a href="javascript:void(0);" class="wams-modal-btn wams-finish-upload image disabled" data-key="' . esc_attr($key) . '" data-change="' . esc_attr__('Change photo', 'ultimate-member') . '" data-processing="' . esc_attr__('Processing...', 'ultimate-member') . '">' . esc_html__('Apply', 'ultimate-member') . '</a>
										<a href="javascript:void(0);" class="wams-modal-btn alt" data-action="wams_remove_modal"> ' . esc_html__('Cancel', 'ultimate-member') . '</a>
									</div>
									<div class="wams-clear"></div>
								</div>';
						$output .= '</div>';
						$output .= '</div>';
					}
				...
				case 'file':
					...
					if (!empty($file_field_value) && 'empty_file' !== $file_field_value) {
						...
						if (wams_is_temp_file($file_field_value)) {
							$file_info = get_transient("wams_{$file_field_value}");
						} else {
							$file_info = wams_user($data['metakey'] . '_metadata');
						}
						...
						if ((isset($this->set_mode) && 'register' === $this->set_mode) || file_exists(WAMS()->uploader()->get_core_temp_dir() . DIRECTORY_SEPARATOR . $file_field_value)) {
							$file_url = WAMS()->uploader()->get_core_temp_url() . DIRECTORY_SEPARATOR . $file_field_value;
							$file_dir = WAMS()->uploader()->get_core_temp_dir() . DIRECTORY_SEPARATOR . $file_field_value;
						} else {
							$file_url = WAMS()->files()->get_download_link($this->set_id, $key, wams_user('ID'));
							$file_dir = WAMS()->uploader()->get_upload_base_dir() . wams_user('ID') . DIRECTORY_SEPARATOR . $file_field_value;
						}
						...
						if (file_exists($file_dir)) {
							$output .= '<div class="wams-single-file-preview show" data-key="' . esc_attr($key) . '">';
							if (empty($disabled)) {
								$output .= '<a href="#" class="cancel"><i class="wams-icon-close"></i></a>';
							}

							$fonticon_bg = WAMS()->files()->get_fonticon_bg_by_ext($file_type['ext']);
							$fonticon    = WAMS()->files()->get_fonticon_by_ext($file_type['ext']);

							$output .= '<div class="wams-single-fileinfo">';
							...
							$output .= '<div class="wams-single-file-preview show" data-key="' . esc_attr($key) . '">' . esc_html__('This file has been removed.', 'ultimate-member') . '</div>';
						}

						if (empty($disabled)) {
							$output .= '<a href="#" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="wams-button wams-btn-auto-width">' . esc_html__('Change file', 'ultimate-member') . '</a>';
						}
					} else {
						$output .= '<div class="wams-single-file-preview" data-key="' . esc_attr($key) . '"></div>';
						if (empty($disabled)) {
							$output .= '<a href="#" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="wams-button wams-btn-auto-width">' . esc_html($data['button_text']) . '</a>';
						}
					}
					$output .= '</div>';
					/* modal hidden */
					if (empty($disabled)) {
						...
						$output .= '<div class="wams-single-file-preview">
										<a href="javascript:void(0);" class="cancel"><i class="wams-icon-close"></i></a>
										<div class="wams-single-fileinfo">
						...
						$nonce   = wp_create_nonce('wams_upload_nonce-' . $this->timestamp);
						$output .= '<div class="wams-single-file-upload" data-user_id="' . esc_attr($_wams_profile_id) . '" data-timestamp="' . esc_attr($this->timestamp) . '" data-nonce="' . esc_attr($nonce) . '" ' . $data_icon . ' data-set_id="' . esc_attr($set_id) . '" data-set_mode="' . esc_attr($set_mode) . '" data-type="' . esc_attr($type) . '" data-key="' . esc_attr($key) . '" data-max_size="' . esc_attr($data['max_size']) . '" data-max_size_error="' . esc_attr($data['max_size_error']) . '" data-min_size_error="' . esc_attr($data['min_size_error']) . '" data-extension_error="' . esc_attr($data['extension_error']) . '"  data-allowed_types="' . esc_attr($allowed_types) . '" data-upload_text="' . esc_attr($data['upload_text']) . '" data-max_files_error="' . esc_attr($data['max_files_error']) . '" data-upload_help_text="' . esc_attr($data['upload_help_text']) . '">' . esc_html($data['button_text']) . '</div>';
						$output .= '<div class="wams-modal-footer">
									<div class="wams-modal-right">
										<a href="javascript:void(0);" class="wams-modal-btn wams-finish-upload file disabled" data-key="' . esc_attr($key) . '" data-change="' . esc_attr__('Change file', 'ultimate-member') . '" data-processing="' . esc_attr__('Processing...', 'ultimate-member') . '"> ' . esc_html__('Save', 'ultimate-member') . '</a>
										<a href="javascript:void(0);" class="wams-modal-btn alt" data-action="wams_remove_modal"> ' . esc_html__('Cancel', 'ultimate-member') . '</a>
									</div>
									<div class="wams-clear"></div>
								</div>';
				...
				case 'select':
					...
					$output .= '<div class="wams-field-area ' . ($has_icon ? 'wams-field-area-has-icon' : '') . ' ">';
					if ($has_icon) {
						$output .= '<div class="wams-field-icon"><i class="' . esc_attr($data['icon']) . '"></i></div>';
					}
					...
					if (!empty($data['parent_dropdown_relationship']) && !WAMS()->user()->preview) {
						 ...
						 *
						 * @since 1.3.x
						 * @hook  wams_custom_dropdown_options_parent__{$form_key}
						 *
						 ...
						 *     return $parent;
						 * }
						 * add_filter( 'wams_custom_dropdown_options_parent__{$form_key}', 'function_name', 10, 2 );
						 */
						$parent_dropdown_relationship = apply_filters("wams_custom_dropdown_options_parent__{$form_key}", $data['parent_dropdown_relationship'], $data);
						$atts_ajax                   .= ' data-wams-parent="' . esc_attr($parent_dropdown_relationship) . '" ';

						if (!empty($data['custom_dropdown_options_source']) && function_exists($data['custom_dropdown_options_source']) && wams_user($data['parent_dropdown_relationship'])) {
							if (!$this->is_source_blacklisted($data['custom_dropdown_options_source'])) {
								$options = call_user_func($data['custom_dropdown_options_source'], $data['parent_dropdown_relationship']);
							}

							$disabled_by_parent_option = '';
							if (wams_user($form_key)) {
								$select_original_option_value = ' data-wams-original-value="' . esc_attr(wams_user($form_key)) . '" ';
							}
						}
					}

					// Child dropdown option selected
					if (isset(WAMS()->form()->post_form[$form_key])) {
						$select_original_option_value = " data-wams-original-value='" . esc_attr(WAMS()->form()->post_form[$form_key]) . "' ";
					}

					// Child dropdown
					if ($has_parent_option) {
						if (!empty($data['custom_dropdown_options_source']) && function_exists($data['custom_dropdown_options_source']) && isset(WAMS()->form()->post_form[$form_key])) {
							if (!$this->is_source_blacklisted($data['custom_dropdown_options_source'])) {
								$options = call_user_func($data['custom_dropdown_options_source'], $data['parent_dropdown_relationship']);
							}
						}
					...
					if (!empty($data['custom_dropdown_options_source'])) {
						 ...
						 *
						 * @since 1.3.x
						 * @hook  wams_custom_dropdown_options_source__{$form_key}
						 *
						 ...
						 *     return $source;
						 * }
						 * add_filter( 'wams_custom_dropdown_options_source__{$form_key}', 'function_name', 10, 2 );
						 */
						$ajax_source = apply_filters("wams_custom_dropdown_options_source__{$form_key}", $data['custom_dropdown_options_source'], $data);
						$atts_ajax  .= ' data-wams-ajax-source="' . esc_attr($ajax_source) . '" ';
					}

					if (!$has_parent_option) {
						if (isset($options) && 'builtin' === $options) {
							$options = WAMS()->builtin()->get($data['filter']);
						}

						// 'country'
						if ('country' === $key && empty($options)) {
							$options = WAMS()->builtin()->get('countries');
						 ...
						 * function my_wams_selectbox_options( $options, $key ) {
						 *     // your code here
						 *     return $options;
						 * }
						 * add_filter( 'wams_selectbox_options', 'my_wams_selectbox_options', 10, 2 );
						 */
						$options = apply_filters('wams_selectbox_options', $options, $key);
						if (isset($options)) {
							/**
							 * Filters dropdown dynamic options.
							 *
							 * @since 1.3.x
							 * @hook  wams_select_dropdown_dynamic_options
							 *
							 ...
							 *     return $options;
							 * }
							 * add_filter( 'wams_select_dropdown_dynamic_options', 'my_select_dropdown_dynamic_options', 10, 2 );
							 */
							$options = apply_filters('wams_select_dropdown_dynamic_options', $options, $data);
							/**
							 * Filters dropdown dynamic options by field $key.
							 *
							 * @since 1.3.x
							 * @hook  wams_select_dropdown_dynamic_options_{$key}
							 *
							 * @param {array} $options Dynamic options.
							 *
							 * @return {array} Dynamic options.
							 ...
							 *     return $options;
							 * }
							 * add_filter( 'wams_select_dropdown_dynamic_options_{$key}', 'my_select_dropdown_dynamic_options', 10, 1 );
							 */
							$options = apply_filters("wams_select_dropdown_dynamic_options_{$key}", $options);
						}
					}
					 ...
					 *
					 * @since 1.3.x `wams_multiselect_option_value`
					 * @since 2.0 renamed to `wams_select_options_pair`
					 *
					 * @hook  wams_select_options_pair
					 *
					 ...
					 * function my_wams_select_options_pair( $options_pair, $data ) {
					 *     // your code here
					 *     return $options_pair;
					 * }
					 * add_filter( 'wams_select_options_pair', 'my_wams_select_options_pair', 10, 2 );
					 */
					$options_pair = apply_filters('wams_select_options_pair', null, $data);
					...
					if (!empty($options)) {
						foreach ($options as $k => $v) {
							...
							$option_value                 = $v;
							$wams_field_checkbox_item_title = $v;

							if ((!is_nwamseric($k) && 'role' === $form_key) || ('account' === $this->set_mode || wams_is_core_page('account'))) {
								$option_value = $k;
							}
							...
							$output .= '>' . esc_html__($wams_field_checkbox_item_title, 'ultimate-member') . '</option>';
						}
					}
				...
				case 'multiselect':
					...
					if (isset($options)) {
						 ...
						 *     return $options;
						 * }
						 * add_filter( 'wams_multiselect_options', 'my_multiselect_options', 10, 2 );
						 */
						$options = apply_filters('wams_multiselect_options', $options, $data);
						/**
						 * Filters multiselect options by field $key.
						 *
						 * @since 1.3.x
						 * @hook  wams_multiselect_options_{$key}
						 *
						 * @param {array}   $options  Multiselect Options.
						 *
						 ...
						 *     return $options;
						 * }
						 * add_filter( 'wams_multiselect_options_{$key}', 'my_multiselect_options', 10, 2 );
						 */
						$options = apply_filters("wams_multiselect_options_{$key}", $options);
						/**
						 * Filters multiselect options by field $type.
						 *
						 * @since 1.3.x
						 * @hook  wams_multiselect_options_{$type}
						 *
						 * @param {array} $options Multiselect Options.
						 * @param {array} $data    Field Data.
						 *
						 ...
						 *     return $options;
						 * }
						 * add_filter( 'wams_multiselect_options_{$type}', 'my_multiselect_options', 10, 2 );
						 */
						$options = apply_filters("wams_multiselect_options_{$type}", $options, $data);
					}

					/** This filter is docwamsented in includes/core/class-fields.php */
					$use_keyword = apply_filters('wams_select_options_pair', null, $data);
				...
				case 'radio':
					 ...
					 *     return $options;
					 * }
					 * add_filter( 'wams_radio_field_options', 'my_radio_field_options', 10, 2 );
					 */
					$options = apply_filters('wams_radio_field_options', $options, $data);
					/**
					 * Filters radio field options by field $key.
					 *
					 * @since 1.3.x
					 * @hook  wams_radio_field_options_{$key}
					 *
					 * @param {array} $options Radio Field Options.
					 *
					 * @return {array} Radio Field Options.
					 ...
					 *     return $options;
					 * }
					 * add_filter( 'wams_radio_field_options_{$key}', 'my_radio_field_options', 10, 1 );
					 */
					$options = apply_filters("wams_radio_field_options_{$key}", $options);
					$options = $this->get_available_roles($form_key, $options);
					 ...
					 *
					 * @since 2.0
					 * @hook  wams_radio_options_pair__{$key}
					 *
					 ...
					 *     return $options;
					 * }
					 * add_filter( 'wams_radio_options_pair__{$key}', 'my_radio_field_options', 10, 2 );
					 */
					$options_pair = apply_filters("wams_radio_options_pair__{$key}", false, $data);
					...
					if (!empty($options)) {
						foreach ($options as $k => $v) {
							...
							if ($this->is_radio_checked($key, $option_value, $data)) {
								$active = 'active';
								$class  = 'wams-icon-android-radio-button-on';
							} else {
								$active = '';
								$class  = 'wams-icon-android-radio-button-off';
							}

							// It's for a legacy case `array_key_exists( 'editable', $data )`.
							if (array_key_exists('editable', $data) && empty($data['editable'])) {
								$col_class .= ' wams-field-radio-state-disabled';
							}

							$output .= '<label class="wams-field-radio ' . esc_attr($active) . ' wams-field-half ' . esc_attr($col_class) . '">';
							...
							$output .= '<span class="wams-field-radio-state"><i class="' . esc_attr($class) . '"></i></span>';
							$output .= '<span class="wams-field-radio-option">' . esc_html__($wams_field_checkbox_item_title, 'ultimate-member') . '</span>';
							$output .= '</label>';

							if (0 === $i % 2) {
								$output .= '<div class="wams-clear"></div>';
							}
						}
					}
				...
				case 'checkbox':
					 ...
					 *
					 * @since 1.3.x
					 * @hook  wams_checkbox_field_options
					 *
					 ...
					 * function wams_checkbox_field_options( $options, $data ) {
					 *     // your code here
					 *     return $options;
					 * }
					 * add_filter( 'wams_checkbox_field_options', 'wams_checkbox_field_options', 10, 2 );
					 */
					$options = apply_filters('wams_checkbox_field_options', $options, $data);
					/**
					 * Filters checkbox options by field $key.
					 *
					 * @since 1.3.x
					 * @hook  wams_checkbox_field_options_{$key}
					 *
					 * @param {array} $options Checkbox Options.
					 *
					 * @return {array} Checkbox Options.
					 ...
					 *     return $options;
					 * }
					 * add_filter( 'wams_checkbox_field_options_{$key}', 'my_checkbox_options', 10, 1 );
					 */
					$options = apply_filters("wams_checkbox_field_options_{$key}", $options);
					...
					foreach ($options as $k => $v) {
						...
						if ($this->is_selected($key, $v, $data)) {
							$active = 'active';
							$class  = 'wams-icon-android-checkbox-outline';
						} else {
							$active = '';
							$class  = 'wams-icon-android-checkbox-outline-blank';
						}
						...
						$output .= '<label class="wams-field-checkbox ' . esc_attr($active) . ' wams-field-half ' . esc_attr($col_class) . '">';
						...
						$output .= '<span class="wams-field-checkbox-state"><i class="' . esc_attr($class) . '"></i></span>';

						/**
						 * Filters change Checkbox item title.
						 *
						 * @since 1.3.x
						 * @hook  wams_field_checkbox_item_title
						 *
						 * @param {string} $wams_field_checkbox_item_title Item Title.
						 * @param {string} $key                          Field Key.
						 ...
						 * function wams_checkbox_field_options( $wams_field_checkbox_item_title, $key, $v, $data ) {
						 *     // your code here
						 *     return $wams_field_checkbox_item_title;
						 * }
						 * add_filter( 'wams_field_checkbox_item_title', 'wams_checkbox_field_options', 10, 4 );
						 */
						$wams_field_checkbox_item_title = apply_filters('wams_field_checkbox_item_title', $wams_field_checkbox_item_title, $key, $v, $data);

						$output .= '<span class="wams-field-checkbox-option">' . esc_html__($wams_field_checkbox_item_title, 'ultimate-member') . '</span>';
						$output .= '</label>';
				...
				case 'shortcode':
					$content = array_key_exists('content', $data) ? $data['content'] : '';
					$content = str_replace('{profile_id}', wams_profile_id(), $content);
				...
				case 'group':
					...
					if (!empty($fields)) {
						...
						$output .= '<div class="wams-field-group" data-max_entries="' . esc_attr($data['max_entries']) . '">
								<div class="wams-field-group-head"><i class="wams-icon-plus"></i>' . esc_html__($data['label'], 'ultimate-member') . '</div>';
						$output .= '<div class="wams-field-group-body"><a href="javascript:void(0);" class="wams-field-group-cancel"><i class="wams-icon-close"></i></a>';
			...
			if (isset($this->set_mode)) {
				 ...
				 *
				 * @since 1.3.x
				 * @hook  wams_{$key}_form_edit_field
				 *
				 * @param {string} $output Field HTML.
				 ...
				 * function wams_checkbox_field_options( $output, $mode ) {
				 *     // your code here
				 *     return $output;
				 * }
				 * add_filter( 'wams_{$key}_form_edit_field', 'my_form_edit_field', 10, 2 );
				 */
				$output = apply_filters("wams_{$key}_form_edit_field", $output, $this->set_mode);
			}
		...
		function get_available_roles($form_key, $options = array())
		{
			...
			$roles         = WAMS()->roles()->get_roles(false, $exclude_roles);
		 ...
		 * @return array $arr
		 */
		function array_sort_by_colwamsn($arr, $col, $dir = SORT_ASC)
		{
			$sort_col = array();
		...
		/**
		 * Get fields in colwamsn
		 *
		 * @param  array   $fields
		 * @param  integer $col_nwamsber
		 *
		 * @return mixed
		 */
		function get_fields_in_colwamsn($fields, $col_nwamsber)
		{
			foreach ($fields as $key => $array) {
				if (isset($array['in_colwamsn']) && $array['in_colwamsn'] == $col_nwamsber) {
					$results[$key] = $array;
				}
			}
		...
		public function display($mode, $args)
		{
			...
			WAMS()->form()->form_suffix = '-' . $this->global_args['form_id'];

			$this->set_mode = $mode;

			if ('profile' === $mode) {
				WAMS()->form()->nonce = wp_create_nonce('wams-profile-nonce' . WAMS()->user()->target_id);
			}
					...
					if ($row_fields) {
						...
						for ($c = 0; $c < $sub_rows; $c++) {
							...
							if (is_array($subrow_fields)) {
								...
								$subrow_fields = $this->array_sort_by_colwamsn($subrow_fields, 'position');

								if ($cols_nwams == 1) {

									$output .= '<div class="wams-col-1">';
									$col1_fields = $this->get_fields_in_colwamsn($subrow_fields, 1);
									if ($col1_fields) {
									...
									$output .= '</div>';
								} else if ($cols_nwams == 2) {

									$output .= '<div class="wams-col-121">';
									$col1_fields = $this->get_fields_in_colwamsn($subrow_fields, 1);
									if ($col1_fields) {
										foreach ($col1_fields as $key => $data) {
											if (!empty($args['is_block'])) {
									...
									$output .= '<div class="wams-col-122">';
									$col2_fields = $this->get_fields_in_colwamsn($subrow_fields, 2);
									if ($col2_fields) {
									...
									$output .= '</div><div class="wams-clear"></div>';
								} else {

									$output .= '<div class="wams-col-131">';
									$col1_fields = $this->get_fields_in_colwamsn($subrow_fields, 1);
									if ($col1_fields) {
										foreach ($col1_fields as $key => $data) {
											$output .= $this->edit_field($key, $data);
									...
									$output .= '<div class="wams-col-133">';
									$col3_fields = $this->get_fields_in_colwamsn($subrow_fields, 3);
									if ($col3_fields) {
		...
		public function view_field($key, $data, $rule = false)
		{
			...
			if (!wams_field_conditions_are_met($data)) {
				return '';
			}
			...
			switch ($type) {
				...
				default:
					...
					if ((isset($_field_value) && '' !== $_field_value) || in_array($type, $fields_without_metakey, true)) {
						 ...
						 *     return $output;
						 * }
						 * add_filter( 'wams_view_field', 'my_view_field', 10, 3 );
						 */
						$res = apply_filters('wams_view_field', $res, $data, $type);
						/**
						 * Filters the inner field HTML on view mode by field type {$type}.
						 *
						 * @since 1.3.x
						 * @hook  wams_view_field_value_{$type}
						 *
						 * @param {string} $output Field inner HTML.
						 * @param {array}  $data   Field Data.
						 *
						 ...
						 *     return $output;
						 * }
						 * add_filter( 'wams_view_field_value_{$type}', 'my_view_field', 10, 2 );
						 */
						$res = apply_filters("wams_view_field_value_{$type}", $res, $data);

						$id_attr = '';
						if (!in_array($type, $fields_without_metakey, true)) {
							$id_attr = ' id="' . esc_attr($key . WAMS()->form()->form_suffix) . '"';
						}

						if (empty($res) && !('nwamsber' === $type && '' !== $res)) {
							$output = '';
						} else {
							$output .= '<div class="wams-field-area">';
							$output .= '<div class="wams-field-value"' . $id_attr . '>' . $res . '</div>';
							$output .= '</div>';
				...
				case 'oembed':
					...
					$output .= '<div class="wams-field-area">';
					$output .= '<div class="wams-field-value">' . $response . '</div>';
					$output .= '</div>';
					break;
					/* HTML */
				case 'block':
				...
				case 'rating':
					...
					$nwamsber = 5;
					if (array_key_exists('nwamsber', $data) && in_array(absint($data['nwamsber']), array(5, 10), true)) {
						$nwamsber = $data['nwamsber'];
					}
					ob_start();
				?>

					<div class="wams-field-area">
						<div class="wams-field-value">
							<div class="wams-rating-readonly wams-raty" id="<?php echo esc_attr($key); ?>" data-key="<?php echo esc_attr($key); ?>" data-nwamsber="<?php echo esc_attr($nwamsber); ?>" data-score="<?php echo esc_attr($this->field_value($key, $default, $data)); ?>"></div>
						</div>
					</div>
...
<?php
			...
			if (isset($this->set_mode)) {
				 ...
				 *
				 * @since 1.3.x
				 * @hook  wams_{$key}_form_show_field
				 *
				 ...
				 *     return $output;
				 * }
				 * add_filter( 'wams_{$key}_form_show_field', 'my_form_show_field', 10, 2 );
				 */
				$output = apply_filters("wams_{$key}_form_show_field", $output, $this->set_mode);
				/**
				 * Filters outer field HTML by field $type.
				 *
				 * @since 1.3.x
				 * @hook  wams_{$type}_form_show_field
				 *
				 * @param {string} $output Field outer HTML.
				 * @param {string} $mode   Field Mode.
				 *
				 ...
				 *     return $output;
				 * }
				 * add_filter( 'wams_{$type}_form_show_field', 'my_form_show_field', 10, 2 );
				 */
				$output = apply_filters("wams_{$type}_form_show_field", $output, $this->set_mode);
			}
		...
		function view_field_output($data)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_view_field_output_{$type}
			 * @description Change field data output by $type
			 * @input_vars
			 * [{"var":"$data","type":"array","desc":"Field Data"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_view_field_output_{$type}', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_filter( 'wams_view_field_output_{$type}', 'my_view_field_output', 10, 1 );
			 ...
			 * ?>
			 */
			return apply_filters("wams_view_field_output_" . $data['type'], $data);
		}
		...
		public function display_view($mode, $args)
		{
			...
			if (WAMS()->options()->get('profile_empty_text')) {

				$emo = WAMS()->options()->get('profile_empty_text_emo');
				if ($emo) {
					$emo = '<i class="wams-faicon-frown-o"></i>';
				} else {
					$emo = false;
				}
				...
				if (wams_is_myprofile()) {
					if (isset($_GET['profiletab']) && 'main' !== $_GET['profiletab']) {
						...
						$profile_url = wams_user_profile_url(wams_profile_id());
						$edit_url    = add_query_arg(array('profiletab' => $tab, 'wams_action' => $edit_action), $profile_url);
					} else {
						$edit_url    = wams_edit_profile_url();
					}
					// translators: %s: edit user link.
					$output .= '<p class="wams-profile-note">' . $emo . '<span>' . sprintf(__('Your profile is looking a little empty. Why not <a href="%s">add</a> some information!', 'ultimate-member'), esc_url($edit_url)) . '</span></p>';
				} else {
					$output .= '<p class="wams-profile-note">' . $emo . '<span>' . __('This user has not added any information to their profile yet.', 'ultimate-member') . '</span></p>';
				}
			}
		...
		public function new_row_output($row_id, $row_array)
		{
			...
			if ($heading) {
				...
				$output .= '<div class="wams-row-heading" style="' . esc_attr($css_heading_margin . $css_heading_padding . $css_heading_border . $css_heading_background_color . $css_heading_text_color) . '">';

				if (!empty($row_array['icon'])) {
					$css_icon_color = !empty($row_array['icon_color']) ? 'color: ' . esc_attr($row_array['icon_color']) . ';' : '';
					$output        .= '<span class="wams-row-heading-icon" style="' . esc_attr($css_icon_color) . '"><i class="' . esc_attr($row_array['icon']) . '"></i></span>';
				}
			...
			$output .= '<div class="wams-row ' . esc_attr($row_id . ' ' . $css_class) . '" style="' . esc_attr($css_padding . $css_background . $css_margin . $css_border . $css_borderstyle . $css_bordercolor . $css_borderradius . $css_text_color) . '">';
			return $output;
		}
		...
		function get_atts($key, $classes, $conditional, $data, $field_style = array())
		{
			...
			array_unshift($classes, 'wams-field-' . $data['type']);
			array_unshift($classes, 'wams-field');

			$field_atts = array(
				'id'        => array(
					"wams_field_{$this->set_id}_{$key}",
				),
				'class'     => $classes,
				'data-key'  => array(
					esc_attr($key)
			 ...
			 * @change_log
			 * ["Since: 2.0.57"]
			 * @usage add_filter( 'wams_field_extra_atts', 'function_name', 10, 3 );
			 * @example
			 * <?php
			 * add_filter( 'wams_field_extra_atts', 'function_name', 10, 3 );
			 ...
			 * ?>
			 */
			$field_atts = apply_filters('wams_field_extra_atts', $field_atts, $key, $data);
