...
function wams_upgrade_choice_callbacks240()
{
	...
	if (function_exists('wams_woo_directory_get_states')) {
		$functions[] = 'wams_woo_directory_get_states';
	}
	...
	$functions = implode("\r\n", $functions);
	WAMS()->options()->update('allowed_choice_callbacks', $functions);

	// delete temporarily option for fields upgrade
	update_option('wams_last_version_upgrade', '2.4.0');

	wp_send_json_success(array('message' => __('Custom callback functions whitelisted for 2.4.0 version.', 'wams')));
}
