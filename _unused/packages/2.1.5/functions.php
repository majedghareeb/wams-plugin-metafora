...
function wams_upgrade_balance_field215()
{
	...
	if (function_exists('mycred')) {
		WAMS()->options()->update('mycred_point_types', array($point_type));
	}
	...
	$wpdb->query(
		...
		WHERE meta_key = '_wams_sortby'"
	);

	// Update role_select and role_radio filters to role
	$postmeta = $wpdb->get_results("SELECT * FROM {$wpdb->postmeta} WHERE meta_key='_wams_sorting_fields'", ARRAY_A);
	if (!empty($postmeta)) {
		foreach ($postmeta as $row) {
			...
			if (is_array($meta_value)) {
				...
				if ($update) {
					update_post_meta($row['post_id'], '_wams_sorting_fields', $meta_value);
				}
			}
		}
	}
	...
	update_option('wams_last_version_upgrade', '2.1.5');
