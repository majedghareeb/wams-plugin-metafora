...
//WAMS Roles to WP Roles
//all WAMS Roles from post type
$role_keys = array();

register_post_type('wams_role', array(
	'labels' => array(
		'name'                  => __('User Roles'),
...
if (!empty($wams_roles)) {
	foreach ($wams_roles as $wams_role) {
		//old role key which inserted for each user to usermeta "role"
		$key_in_meta = $wams_role->post_name;

		if (preg_match("/[a-z0-9]+$/i", $wams_role->post_title)) {
			$role_key = sanitize_title($wams_role->post_title);
		} else {
			$auto_increment = WAMS()->options()->get('custom_roles_increment');
			$auto_increment = !empty($auto_increment) ? $auto_increment : 1;
			$role_key = 'custom_role_' . $auto_increment;

			$auto_increment++;
			WAMS()->options()->update('custom_roles_increment', $auto_increment);
		}
		...
		if (!empty($all_role_metadata)) {
			foreach ($all_role_metadata as $metadata) {
				...
				if (
					'_wams_can_edit_roles' == $metadata['meta_key'] || '_wams_can_delete_roles' == $metadata['meta_key']
					|| '_wams_can_view_roles' == $metadata['meta_key'] || '_wams_can_follow_roles' == $metadata['meta_key']
					|| '_wams_can_friend_roles' == $metadata['meta_key'] || '_wams_can_review_roles' == $metadata['meta_key']
				) {
		...
		if (!in_array($role_key, $all_wp_roles)) {
			$role_meta = array_merge($role_metadata, array(
				'name'              => $wams_role->post_title,
				'wp_capabilities'   => array('read' => true),
				'_wams_is_custom'     => true,
			));
		} else {
			$role_meta = $role_metadata;
		}

		//$old_key = ! empty( $role_meta['_wams_core'] ) ? $role_meta['_wams_core'] : $role_key;
		if (!in_array($role_key, $all_wp_roles)) {
			$roles_associations[$key_in_meta] = 'wams_' . $role_key;
		} else {
		...
		if (!empty($role_meta['_wams_core']))
			unset($role_meta['_wams_core']);

		update_option("wams_role_{$role_key}_meta", $role_meta);
	}
	...
	foreach ($wams_roles as $wams_role) {
		...
		if (!empty($role_meta)) {
			foreach ($role_meta as $metakey => $metadata) {
				...
				if (
					'_wams_can_edit_roles' == $metakey || '_wams_can_delete_roles' == $metakey
					|| '_wams_can_view_roles' == $metakey || '_wams_can_follow_roles' == $metakey
					|| '_wams_can_friend_roles' == $metakey || '_wams_can_review_roles' == $metakey
				) {
				...
				} elseif ('_wams_profilec_upgrade_role' == $metakey) {
...
update_option('wams_roles', $role_keys);
...
//temporary option
update_option('wams_roles_associations', $roles_associations);
