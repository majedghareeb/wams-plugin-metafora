...
function wams_upgrade_usermeta_count280()
{
	...
	$count = $wpdb->get_var(
		...
		WHERE meta_key = 'use_gdpr_agreement' OR
		      meta_key = 'wams_user_blocked__timestamp' OR
		      meta_key = '_wams_last_login' OR
		      meta_key = 'submitted'"
	);
...
function wams_upgrade_metadata_per_user280()
{
	WAMS()->admin()->check_ajax_nonce();
	...
	foreach ($usermeta as $usermeta_row) {
		...
		if ('use_gdpr_agreement' === $usermeta_row['meta_key']) {
			if (is_numeric($usermeta_row['meta_value'])) {
				update_user_meta($user_id, 'wams_use_gdpr_agreement_backup', $usermeta_row['meta_value']);
				$updated_meta = gmdate('Y-m-d H:i:s', $usermeta_row['meta_value']);
			}
		} elseif ('_wams_last_login' === $usermeta_row['meta_key']) {
			if (is_numeric($usermeta_row['meta_value'])) {
				update_user_meta($user_id, '_wams_last_login_backup', $usermeta_row['meta_value']);
				if (!empty($offset)) {
		...
		} elseif ('wams_user_blocked__timestamp' === $usermeta_row['meta_key']) {
			if (!empty($offset)) {
				$last_scanned_time = $usermeta_row['meta_value'];
				if (!empty($last_scanned_time)) {
					update_user_meta($user_id, 'wams_user_blocked__timestamp_backup', $last_scanned_time);
	...
	wp_send_json_success(array('message' => sprintf(__('Metadata from %1$s to %2$s were upgraded successfully...', 'wams'), $from, $to)));
}

function wams_upgrade_update_options280()
{
	WAMS()->admin()->check_ajax_nonce();
	...
	if (!empty($offset)) {
		...
		if (!empty($last_scanned_time)) {
			update_option('wams_secure_last_time_scanned_backup', $last_scanned_time);
			$last_scanned_time = strtotime($last_scanned_time) - $offset;
			$last_scanned_time = gmdate('Y-m-d H:i:s', $last_scanned_time);

			update_option('wams_secure_last_time_scanned', $last_scanned_time);
		}
	}

	// delete temporarily option for fields upgrade
	update_option('wams_last_version_upgrade', '2.8.0');
