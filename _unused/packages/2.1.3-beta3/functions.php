...
function wams_upgrade_users_count213beta3()
{
...
function wams_upgrade_metadata_per_user213beta3()
{
	WAMS()->admin()->check_ajax_nonce();
	...
	$metadata = $wpdb->get_results($wpdb->prepare(
		...
		FROM {$wpdb->users} u
		LEFT JOIN {$wpdb->usermeta} um ON ( um.user_id = u.ID AND um.meta_key IN( 'account_status','hide_in_members','synced_gravatar_hashed_id','synced_profile_photo','profile_photo','cover_photo','_wams_verified' ) )
		WHERE u.ID >= %d AND
		      u.ID <= %d",
		$min_max['MinID'],
		$min_max['MaxID']
		...
		switch ($metadatarow['meta_key']) {
			...
			case 'hide_in_members':
				...
				if (!empty($metadatarow['meta_value'])) {
					if (
						$metadatarow['meta_value'] == 'Yes' || $metadatarow['meta_value'] == __('Yes', 'wams') ||
						(is_array($metadatarow['meta_value']) && array_intersect(array('Yes', __('Yes', 'wams')), $metadatarow['meta_value']))
					) {
	...
	if (!empty($users_map)) {
		foreach ($users_map as $user_id => $metavalue) {
			update_user_meta($user_id, 'wams_member_directory_data', $metavalue);
		}
	}
	...
	wp_send_json_success(array('message' => sprintf(__('Metadata from %1$s to %2$s users were upgraded successfully...', 'wams'), $from, $to)));
}


function wams_upgrade_metatable213beta3()
{
	WAMS()->admin()->check_ajax_nonce();
	...
	$sql = "CREATE TABLE {$wpdb->prefix}wams_metadata (
umeta_id bigint(20) unsigned NOT NULL auto_increment,
user_id bigint(20) unsigned NOT NULL default '0',
wams_key varchar(255) default NULL,
wams_value longtext default NULL,
PRIMARY KEY  (umeta_id),
KEY user_id_indx (user_id),
KEY meta_key_indx (wams_key),
KEY meta_value_indx (wams_value(191))
) $charset_collate;";

	require_once(ABSPATH . 'wp-admin/includes/upgrade.php');
	dbDelta($sql);

	update_option('wams_last_version_upgrade', '2.1.3-beta3');
	wp_send_json_success(array('message' => __('Usermeta table was upgraded successfully', 'wams')));
}
