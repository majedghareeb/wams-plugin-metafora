...
if (!class_exists('wams\core\Password')) {
	...
	class Password
	{
		...
		{
			add_shortcode('wams_password', array(&$this, 'wams_password'));

			add_action('template_redirect', array(&$this, 'form_init'), 10001);

			add_action('wams_reset_password_errors_hook', array(&$this, 'wams_reset_password_errors_hook'));
			add_action('wams_reset_password_process_hook', array(&$this, 'wams_reset_password_process_hook'));

			add_action('wams_change_password_errors_hook', array(&$this, 'wams_change_password_errors_hook'));
			add_action('wams_change_password_process_hook', array(&$this, 'wams_change_password_process_hook'));
		}
		...
		function reset_url()
		{
			...
			$key       = WAMS()->user()->maybe_generate_password_reset_key($user_data);
		...
		function get_class($mode)
		{
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_form_official_classes__hook', 'function_name', 10, 1 ); ?>
			 * @example
			 * <?php
		 ...
		 * @return string
		 */
		public function wams_password($args = array())
		{
			/** There is possible to use 'shortcode_atts_wams_password' filter for getting customized $atts. This filter is documented in wp-includes/shortcodes.php "shortcode_atts_{$shortcode}" */
			$args = shortcode_atts(
				array(
					'template'  => 'password-reset',
					'mode'      => 'password',
					'form_id'   => 'wams_password_id',
					'max_width' => '450px',
					'align'     => 'center',
				),
				$args,
			...
			if (empty($args['use_custom_settings'])) {
				$args = array_merge($args, WAMS()->shortcodes()->get_css_args($args));
			} else {
				$args = array_merge(WAMS()->shortcodes()->get_css_args($args), $args);
			}
			/**
			 * Filters extend Reset Password Arguments
			 *
			 * @since 1.3.x
			 * @hook  wams_reset_password_shortcode_args_filter
			 *
			 ...
			 *     return $args;
			 * }
			 * add_filter( 'wams_reset_password_shortcode_args_filter', 'my_reset_password_shortcode_args', 10, 1 );
			 */
			$args = apply_filters('wams_reset_password_shortcode_args_filter', $args);
			...
			WAMS()->fields()->set_id = absint($args['form_id']);
		...
		{
			if (wams_is_core_page('password-reset') && isset($_POST['_wams_password_reset'])) {
				return true;
			}
		...
		{
			if (isset($_POST['_wams_account']) == 1 && isset($_POST['_wams_account_tab']) && sanitize_key($_POST['_wams_account_tab']) === 'password') {
				return true;
			} elseif (isset($_POST['_wams_password_change']) && $_POST['_wams_password_change'] == 1) {
				return true;
			}
		...
		{
			if (wams_is_core_page('password-reset')) {
				WAMS()->fields()->set_mode = 'password';
			}

			// validate $rp_cookie and hash via check_password_reset_key
			if (wams_is_core_page('password-reset') && isset($_REQUEST['act']) && 'reset_password' === sanitize_key($_REQUEST['act'])) {
				wp_fix_server_vars();
				...
				if (isset($_GET['hash']) && isset($_GET['login'])) {
					...
					// Not `wams_safe_redirect()` because password-reset page is predefined page and is situated on the same host.
				...
				if (!$user || is_wp_error($user)) {
					...
					if ($user && 'expired_key' === $user->get_error_code()) {
						wp_redirect(add_query_arg(array('updated' => 'expiredkey'), wams_get_core_page('password-reset')));
					} else {
						wp_redirect(add_query_arg(array('updated' => 'invalidkey'), wams_get_core_page('password-reset')));
					}
					exit;
				}
			...
			if ($this->is_reset_request()) {
				...
				if (empty(WAMS()->form()->post_form['mode'])) {
					WAMS()->form()->post_form['mode'] = 'password';
				}
				 ...
				 *
				 * @hook wams_reset_password_errors_hook
				 *
				 ...
				 *     // your code here
				 * }
				 * add_action( 'wams_reset_password_errors_hook', 'my_reset_password_errors', 10, 2 );
				 */
				do_action('wams_reset_password_errors_hook', WAMS()->form()->post_form, $form_data);
				...
				if (!isset(WAMS()->form()->errors)) {
					 ...
					 * @since 2.6.8 Added $form_data attribute.
					 *
					 * @hook wams_reset_password_process_hook
					 *
					 * @param {array} $submission_data Form submitted data.
					 * @param {array} $form_data       Form data. Since 2.6.8
					 *
					 ...
					 *     // your code here
					 * }
					 * add_action( 'wams_reset_password_process_hook', 'my_reset_password_process', 10, 2 );
					 */
					do_action('wams_reset_password_process_hook', WAMS()->form()->post_form, $form_data);
				}
			}
			...
			if ($this->is_change_request()) {
				 ...
				 *
				 * @type action
				 * @title wams_change_password_errors_hook
				 * @description Action on change password submit form
				 * @input_vars
				 * [{"var":"$post","type":"array","desc":"Form submitted"}]
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_action( 'wams_change_password_errors_hook', 'function_name', 10, 1 );
				 * @example
				 * <?php
				 * add_action( 'wams_change_password_errors_hook', 'my_change_password_errors', 10, 1 );
				 * function my_change_password_errors( $post ) {
				 *     // your code here
				 * }
				 * ?>
				 */
				do_action('wams_change_password_errors_hook', WAMS()->form()->post_form);
				...
				if (!isset(WAMS()->form()->errors)) {
					 ...
					 *
					 * @type action
					 * @title wams_change_password_process_hook
					 * @description Action on change password success submit form
					 * @input_vars
					 * [{"var":"$post","type":"array","desc":"Form submitted"}]
					 * @change_log
					 * ["Since: 2.0"]
					 * @usage add_action( 'wams_change_password_process_hook', 'function_name', 10, 1 );
					 * @example
					 * <?php
					 * add_action( 'wams_change_password_process_hook', 'my_change_password_process', 10, 1 );
					 * function my_change_password_process( $post ) {
					 *     // your code here
					 * }
					 * ?>
					 */
					do_action('wams_change_password_process_hook', WAMS()->form()->post_form);
				}
			}
		}
		 ...
		 * @param $args
		 */
		public function wams_reset_password_errors_hook($args)
		{
			if (isset($args[WAMS()->honeypot]) && '' !== $args[WAMS()->honeypot]) {
			...
			if (empty($user)) {
				WAMS()->form()->add_error('username_b', __('Please provide your username or email', 'wams'));
			}
			...
			if ((!is_email($user) && username_exists($user)) || (is_email($user) && email_exists($user))) {
				...
				if (WAMS()->options()->get('enable_reset_password_limit')) { // if reset password limit is set
					if (!(WAMS()->options()->get('disable_admin_reset_password_limit') && $is_admin)) {
						// Doesn't trigger this when a user has admin capabilities and when reset password limit is disabled for admins
						$limit = WAMS()->options()->get('reset_password_limit_number');
						if ($attempts >= $limit) {
							WAMS()->form()->add_error('username_b', __('You have reached the limit for requesting password change for this user already. Contact support if you cannot open the email', 'wams'));
						} else {
		 ...
		 * @param $args
		 */
		public function wams_reset_password_process_hook($args)
		{
			$user = null;
			...
			if (isset($data) && is_a($data, '\WP_User')) {
				wams_fetch_user($data->ID);
				WAMS()->user()->password_reset();
			}

			wp_redirect(wams_get_core_page('password-reset', 'checkemail'));
			exit;
		}
		 ...
		 * @param $args
		 */
		public function wams_change_password_errors_hook($args)
		{
			if (isset($args[WAMS()->honeypot]) && '' !== $args[WAMS()->honeypot]) {
				wp_die(esc_html__('Hello, spam bot!', 'wams'));
			}

			if (isset($args['_wams_account']) == 1 && isset($args['_wams_account_tab']) && 'password' === sanitize_key($args['_wams_account_tab'])) {
			...
			if (!empty($args['user_password']) && WAMS()->options()->get('change_password_request_limit') && is_user_logged_in()) {
				$transient_id       = '_wams_change_password_rate_limit__' . wams_user('ID');
				$last_request       = get_transient($transient_id);
				$request_limit_time = apply_filters('wams_change_password_attempt_limit_interval', 30 * MINUTE_IN_SECONDS);
				if (!$last_request) {
					set_transient($transient_id, time(), $request_limit_time);
				} else {
					WAMS()->form()->add_error('user_password', __('Unable to change password because of password change limit. Please try again later.', 'wams'));
					return;
				}
			}

			if (isset($args['user_password']) && empty($args['user_password'])) {
				WAMS()->form()->add_error('user_password', __('You must enter a new password', 'wams'));
			}
			...
			if (isset($args['confirm_user_password']) && empty($args['confirm_user_password'])) {
				WAMS()->form()->add_error('confirm_user_password', __('You must confirm your new password', 'wams'));
			}

			if (isset($args['user_password']) && isset($args['confirm_user_password']) && $args['user_password'] !== $args['confirm_user_password']) {
				WAMS()->form()->add_error('confirm_user_password', __('Your passwords do not match', 'wams'));
			}
		}
		 ...
		 * @param $args
		 */
		public function wams_change_password_process_hook($args)
		{
			if (isset($args['_wams_password_change']) && $args['_wams_password_change'] == 1) {
				...
				if ((!$errors->get_error_code())) {
					...
					WAMS()->user()->password_changed($user->ID);
					 ...
					 *
					 * @type action
					 * @title wams_after_changing_user_password
					 * @description Hook that runs after user change their password
					 * @input_vars
					 * [{"var":"$user_id","type":"int","desc":"User ID"}]
					 * @change_log
					 * ["Since: 2.0"]
					 * @usage add_action( 'wams_after_changing_user_password', 'function_name', 10, 1 );
					 * @example
					 * <?php
					 * add_action( 'wams_after_changing_user_password', 'my_after_changing_user_password', 10, 1 );
					 * function my_user_login_extra( $user_id ) {
					 *     // your code here
					 * }
					 * ?>
					 */
					do_action('wams_after_changing_user_password', $user->ID);

					if (!is_user_logged_in()) {
						$url = wams_get_core_page('login', 'password_changed');
					} else {
						$url = wams_get_core_page('password-reset', 'password_changed');
					}
					wp_redirect($url);
					exit;
				}
		...
		/**
		 * WAMS Placeholders for reset password
		 *
		 * @param $placeholders
		 *
		 * @return array
		...
		/**
		 * WAMS Replace Placeholders for reset password
		 *
		...
		{
			$replace_placeholders[] = wams_user('password_reset_link');
