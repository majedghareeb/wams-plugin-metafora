 ...
 *
 * This template can be overridden by copying it to yourtheme/wams/templates/members.php
 *
 * Page: "Members"
 *
 * @version 2.6.8
...
$def_args = array();
foreach (WAMS()->config()->core_directory_meta['members'] as $k => $v) {
	$key = str_replace('_wams_', '', $k);
	$def_args[$key] = $v;
}
...
if (is_user_logged_in()) {
	$priority_user_role = WAMS()->roles()->get_priority_user_role(wams_user('ID'));
}

$args = apply_filters('wams_member_directory_agruments_on_load', $args);
...
if (!empty($args['view_types']) && is_array($args['view_types'])) {
	$args['view_types'] = array_filter($args['view_types'], function ($item) {
		return in_array($item, array_keys(WAMS()->member_directory()->view_types));
	});
}
...
if (!empty($args['enable_sorting'])) {
	...
	$all_sorting_options = WAMS()->member_directory()->sort_fields;
	...
	if (!in_array($default_sorting, $sorting_options_prepared, true)) {
		...
		$label = ('random' === $label) ? __('Random', 'wams') : $label;
	...
	$sorting_options = apply_filters('wams_member_directory_pre_display_sorting', $sorting_options, $args);
...
if (isset($args['search_fields'])) {
	$search_filters = apply_filters('wams_frontend_member_search_filters', array_unique(array_filter($args['search_fields'])));
}
...
if (!empty($search_filters)) {
	$search_filters = array_filter(
		$search_filters,
		function ($item) {
			return array_key_exists($item, WAMS()->member_directory()->filter_fields);
		}
	);
...
foreach ($search_filters as $key => $filter) {
	$filter_data = WAMS()->fields()->get_field($filter);
	if (!wams_can_view_field($filter_data)) {
...
foreach ($args['view_types'] as $type) {
	$basename = WAMS()->member_directory()->get_type_basename($type);
	WAMS()->get_template('members-' . $type . '.php', $basename, $args, true);
}
WAMS()->get_template('members-header.php', '', $args, true);
WAMS()->get_template('members-pagination.php', '', $args, true);
...
if ((($search && $show_search) || ($filters && $show_filters && count($search_filters))) && isset($args['must_search']) && $args['must_search'] == 1) {
	...
	if ($search && $show_search && !empty($search_from_url)) {
		...
		foreach ($search_filters as $filter) {
			...
			switch (WAMS()->member_directory()->filter_types[$filter]) {
				default: {

						$not_searched = apply_filters('wams_member_directory_filter_value_from_url', $not_searched, $filter);
...
<div class="um <?php echo esc_attr($this->get_class($mode)); ?> um-<?php echo esc_attr(substr(md5($form_id), 10, 5)); ?>" data-hash="<?php echo esc_attr(substr(md5($form_id), 10, 5)) ?>" data-base-post="<?php echo esc_attr($postid) ?>" data-must-search="<?php echo esc_attr($must_search); ?>" data-searched="<?php echo $not_searched ? '0' : '1'; ?>" data-view_type="<?php echo esc_attr($current_view) ?>" data-page="<?php echo esc_attr($current_page) ?>" data-sorting="<?php echo esc_attr($sort_from_url) ?>">
	...
	<div class="um-member-directory-header um-form">
		...
		<?php do_action('wams_members_directory_before_head', $args, $form_id, $not_searched); ?>
		...
		<?php if ($search && $show_search) { ?>
			<div class="um-member-directory-header-row um-member-directory-search-row">
				<div class="um-member-directory-search-line">
					<label>
						<span><?php _e('Search:', 'wams'); ?></span>
						<input type="search" class="um-search-line" placeholder="<?php esc_attr_e('Search', 'wams') ?>" value="<?php echo esc_attr($search_from_url) ?>" aria-label="<?php esc_attr_e('Search', 'wams') ?>" speech />
					</label>
					<input type="button" class="um-do-search um-button" value="<?php esc_attr_e('Search', 'wams'); ?>" />
				</div>
			</div>
		<?php
		}
			...
			<div class="um-member-directory-header-row">
				<div class="um-member-directory-nav-line">
					...
					if (!$single_view) {
						...
						foreach (WAMS()->member_directory()->view_types as $key => $value) {
					...
					?>
								...
								// translators: %s: title.
								$data_title = sprintf(__('Change to %s', 'wams'), $value['title']);
								$view_types++;
									?>

									<a href="javascript:void(0)" class="um-member-directory-view-type-a<?php if (!$not_searched) { ?> um-tip-n<?php } ?>" data-type="<?php echo esc_attr($key); ?>" data-default="<?php echo ($default_view === $key) ? 1 : 0; ?>" title="<?php echo esc_attr($data_title); ?>" default-title="<?php echo esc_attr($value['title']); ?>" next-item=""><i class="<?php echo esc_attr($value['icon']); ?>"></i></a>
								<?php
							}
						}
						...
						if (!empty($args['enable_sorting']) && !empty($sorting_options) && count($sorting_options) > 1) { ?>
								...
								WAMS()->member_directory()->dropdown_menu('.um-member-directory-sorting-a', 'click', $items); ?>
			...
			if (is_array($search_filters)) { ?>
						...
						<# _.each( data.filters, function( filter, key, list ) { #>
							<div class="um-members-filter-tag">
								...
								<div class="um-members-filter-remove um-tip-n" data-name="{{{filter.name}}}"
									 ...
									 data-type="{{{filter.type}}}" title="<?php esc_attr_e('Remove filter', 'wams') ?>">&times;</div>
							</div>
						<# }); #>
					<# } #>
				</script>
				...
				<div class="um-member-directory-header-row um-member-directory-filters-bar<?php if (!$filters_expanded) { ?> um-header-row-invisible<?php } ?>">
					<div class="um-search um-search-<?php echo count($search_filters) ?><?php if (!$filters_expanded) { ?> um-search-invisible<?php } ?>">
						<?php $i = 0;
						foreach ($search_filters as $filter) {
							$filter_content = WAMS()->member_directory()->show_filter($filter, $args);
							if (empty($filter_content)) {
								continue;
							}

							$type = WAMS()->member_directory()->filter_types[$filter]; ?>
				...
				<div class="um-member-directory-header-row">
					<div class="um-filtered-line">
						<div class="um-clear-filters"><a href="javascript:void(0);" class="um-clear-filters-a" title="<?php esc_attr_e('Remove all filters', 'wams') ?>"><?php _e('Clear all', 'wams'); ?></a></div>
					</div>
				</div>
		<?php
			}
		}
		do_action('wams_members_directory_head', $args, $form_id, $not_searched); ?>
	</div>
	 ...
	 *
	 * @type action
	 * @title wams_members_directory_footer
	 * @description Member directory display footer
	 * @input_vars
	 * [{"var":"$args","type":"array","desc":"Member directory shortcode arguments"}]
	 * @change_log
	 * ["Since: 2.0"]
	 * @usage add_action( 'wams_members_directory_footer', 'function_name', 10, 1 );
	 * @example
	 * <?php
	 * add_action( 'wams_members_directory_footer', 'my_members_directory_footer', 10, 1 );
	 * function my_members_directory_footer( $args ) {
	 *     // your code here
	 * }
	 * ?>
	 */
	do_action('wams_members_directory_footer', $args, $form_id, $not_searched); ?>
