...
if (!class_exists('wams\core\Member_Directory_Meta')) {
	...
	class Member_Directory_Meta extends Member_Directory
	{
		...
		public function __construct()
		{
			...
			add_action('wams_add_new_field', array(&$this, 'on_new_field_added'), 10, 2);
			add_action('wams_delete_custom_field', array(&$this, 'on_delete_custom_field'), 10, 2);
		}


		/**
		 * Delete custom field and metakey from WAMS usermeta table
		 *
		 * @param $metakey
		 * @param $args
		 */
		...
		{
			$metakeys = get_option('wams_usermeta_fields', array());
			...
			if (in_array($metakey, $metakeys)) {
				...
				update_option('wams_usermeta_fields', array_values($metakeys));
			}

			do_action('wams_metadata_on_delete_custom_field', $metakeys, $metakey, $args);
		}
		...
		function on_new_field_added($metakey, $args)
		{
			...
			do_action('wams_metadata_on_new_field_added', $metakeys, $metakey, $args);
		}


		/**
		 * When you delete usermeta - remove row from wams_metadata
		 *
		...
		/**
		 * When you add/update usermeta - add/update row from wams_metadata
		 *
		 * @param int $meta_id
		 * @param int $object_id
		 * @param string $meta_key
		...
		function on_update_usermeta($meta_id, $object_id, $meta_key, $_meta_value)
		{
			...
			$result = $wpdb->get_var($wpdb->prepare(
				"SELECT umeta_id
				FROM {$wpdb->prefix}wams_metadata
				WHERE user_id = %d AND
				      wams_key = %s
				LIMIT 1",
				$object_id,
			...
			if (empty($result)) {
				$wpdb->insert(
					...
					array(
						'user_id'   => $object_id,
						'wams_key'    => $meta_key,
						'wams_value'  => maybe_serialize($_meta_value),
					),
					array(
						'%d',
						'%s',
		...
		function handle_filter_query($directory_data, $field, $value, $i, $is_default = false)
		{
			...
			switch ($field) {
				default:
					 ...
					 * ["Since: 2.0"]
					 * @usage
					 * <?php add_filter( 'wams_query_args_{$field}__filter_meta', 'function_name', 10, 4 ); ?>
					 * @example
					 * <?php
					 * add_filter( 'wams_query_args_{$field}__filter_meta', 'my_query_args_filter', 10, 4 );
					 ...
					 * ?>
					 */
					$skip_default = apply_filters("wams_query_args_{$field}__filter_meta", false, $this, $field, $value, $filter_type, $is_default);

					$skip_default = apply_filters('wams_query_args_filter_global_meta', $skip_default, $this, $field, $value, $filter_type, $is_default);
					...
					if (!$skip_default) {
						...
						switch ($filter_type) {
							default:
								...
								do_action("wams_query_args_{$field}_{$filter_type}__filter_meta", $field, $value, $filter_type, $i, $is_default);
								break;

							case 'text':

								$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata {$join_slug}{$i} ON {$join_slug}{$i}.user_id = u.ID";

								$value = trim(stripslashes($value));

								$compare = apply_filters('wams_members_directory_filter_text', '=', $field);
								$value = apply_filters('wams_members_directory_filter_text_meta_value', $value, $field);

								$this->where_clauses[] = $wpdb->prepare("{$join_slug}{$i}.wams_key = %s AND {$join_slug}{$i}.wams_value {$compare} %s", $field, $value);
							...
							case 'select':
								...
								foreach ($value as $single_val) {
									...
									$values_array[] = $wpdb->prepare("{$join_slug}{$i}.wams_value LIKE %s", '%"' . $single_val . '"%');
									$values_array[] = $wpdb->prepare("{$join_slug}{$i}.wams_value LIKE %s", '%' . serialize((string) $single_val) . '%');
									$values_array[] = $wpdb->prepare("{$join_slug}{$i}.wams_value = %s", $single_val);

									if (is_numeric($single_val)) {
										$values_array[] = $wpdb->prepare("{$join_slug}{$i}.wams_value LIKE %s", '%' . serialize((int) $single_val) . '%');
									}
								}

								$values = implode(' OR ', $values_array);

								$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = %s AND ( {$values} ) )", $field);
							...
							case 'slider':
								...
								$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = %s AND {$join_slug}{$i}.wams_value BETWEEN %d AND %d )", $field, $min, $max);
							...
							case 'datepicker':
								...
								$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = %s AND {$join_slug}{$i}.wams_value BETWEEN %s AND %s )", $field, $from_date, $to_date);
							...
							case 'timepicker':
								...
								$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata {$join_slug}{$i} ON {$join_slug}{$i}.user_id = u.ID";
								if ($value[0] == $value[1]) {
									$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = %s AND {$join_slug}{$i}.wams_value = %s )", $field, $value[0]);
								} else {
									$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = %s AND CAST( {$join_slug}{$i}.wams_value AS TIME ) BETWEEN %s AND %s )", $field, $value[0], $value[1]);
								}
				...
				case 'role':
					...
					if (empty($this->roles) && !is_multisite()) {
						$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_roles ON ( umm_roles.user_id = u.ID AND umm_roles.wams_key = '" . $wpdb->get_blog_prefix($blog_id) . "capabilities' )";
						$this->roles = $value;
					...
					foreach ($value as $role) {
						$roles_clauses[] = $wpdb->prepare("umm_roles.wams_value LIKE %s", '%"' . $role . '"%');
					}
				...
				case 'birth_date':
					...
					$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = 'birth_date' AND {$join_slug}{$i}.wams_value BETWEEN %s AND %s )", $to_date, $from_date);
				...
				case 'last_login':
					...
					$this->where_clauses[] = $wpdb->prepare("( {$join_slug}{$i}.wams_key = '_wams_last_login' AND {$join_slug}{$i}.wams_value BETWEEN %s AND %s )", $from_date, $to_date);
		...
		function ajax_get_members()
		{
			...
			if ($directory_data['has_profile_photo'] == 1) {
				$profile_photo_where = " AND umm_general.wams_value LIKE '%s:13:\"profile_photo\";b:1;%'";
			}

			$cover_photo_where = '';
			if ($directory_data['has_cover_photo'] == 1) {
				$cover_photo_where = " AND umm_general.wams_value LIKE '%s:11:\"cover_photo\";b:1;%'";
			}

			if (!WAMS()->roles()->wams_user_can('can_edit_everyone')) {
				if (!$this->general_meta_joined) {
					$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_general ON umm_general.user_id = u.ID";
					$this->general_meta_joined = true;
				}
				$this->where_clauses[] = "( umm_general.wams_key = 'wams_member_directory_data' AND
				umm_general.wams_value LIKE '%s:14:\"account_status\";s:8:\"approved\";%' AND umm_general.wams_value LIKE '%s:15:\"hide_in_members\";b:0;%'{$profile_photo_where}{$cover_photo_where} )";
			} else {
				if (!empty($cover_photo_where) || !empty($profile_photo_where)) {
					...
					$this->where_clauses[] = "( umm_general.wams_key = 'wams_member_directory_data'{$profile_photo_where}{$cover_photo_where} )";
				}
			}

			//$this->roles = array();
			if (WAMS()->roles()->wams_user_can('can_view_all')) {
				$view_roles = wams_user('can_view_roles');
			...
			if (!empty($this->roles)) {
				...
				if (!$this->roles_in_query && is_multisite()) {
					...
					$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_roles ON ( umm_roles.user_id = u.ID AND umm_roles.wams_key = '" . $wpdb->get_blog_prefix($blog_id) . "capabilities' )";
					$this->where_clauses[] = "umm_roles.wams_value IS NOT NULL";
			...
			if (!empty($_POST['search'])) {
				...
				if (!empty($search_line)) {
					...
					$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_search ON umm_search.user_id = u.ID";

					$additional_search = apply_filters('wams_member_directory_meta_general_search_meta_query', '', $search_line);

					$search_like_string = apply_filters('wams_member_directory_meta_search_like_type', '%' . $search_line . '%', $search_line);

					$this->where_clauses[] = $wpdb->prepare("( umm_search.wams_value = %s OR umm_search.wams_value LIKE %s OR umm_search.wams_value LIKE %s OR {$core_search}{$additional_search})", $search_line, $search_like_string, '%' . serialize((string) $search_line) . '%');
			...
			if ($sortby == $directory_data['sortby_custom'] || in_array($sortby, $custom_sort)) {
				...
				$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort ON ( umm_sort.user_id = u.ID AND umm_sort.wams_key = '{$sortby}' )";
				...
				$this->sql_order = " ORDER BY CAST( umm_sort.wams_value AS {$custom_sort_type} ) {$custom_sort_order} ";
				...
				$this->joins[]   = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort ON ( umm_sort.user_id = u.ID AND umm_sort.wams_key = '{$sortby}' )";
				$this->sql_order = " ORDER BY CAST( umm_sort.wams_value AS SIGNED ) {$order}, u.user_registered DESC ";
				...
				if ($display_name == 'username') {
					...
					$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort ON ( umm_sort.user_id = u.ID AND umm_sort.wams_key = 'full_name' )";

					$this->sql_order = " ORDER BY CAST( umm_sort.wams_value AS CHAR ) {$order}, u.display_name {$order} ";
				}
			} elseif (in_array($sortby, array('last_name', 'first_name', 'nickname'))) {

				$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort ON ( umm_sort.user_id = u.ID AND umm_sort.wams_key = '{$sortby}' )";

				$this->sql_order = " ORDER BY CAST( umm_sort.wams_value AS CHAR ) {$order} ";
			} elseif ('last_login' === $sortby) {

				$this->joins[]   = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort ON ( umm_sort.user_id = u.ID AND umm_sort.wams_key = '_wams_last_login' )";
				$this->sql_order = ' ORDER BY CAST( umm_sort.wams_value AS DATETIME ) DESC ';
			} elseif ($sortby == 'last_first_name') {

				$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort ON ( umm_sort.user_id = u.ID AND umm_sort.wams_key = 'last_name' )";
				$this->joins[] = "LEFT JOIN {$wpdb->prefix}wams_metadata umm_sort2 ON ( umm_sort2.user_id = u.ID AND umm_sort2.wams_key = 'first_name' )";

				$this->sql_order = " ORDER BY CAST( umm_sort.wams_value AS CHAR ) ASC, CAST( umm_sort2.wams_value AS CHAR ) ASC ";
			...
			$this->sql_order = apply_filters('wams_modify_sortby_parameter_meta', $this->sql_order, $sortby);
			...
			do_action('wams_pre_users_query', $this, $directory_data, $sortby);
			...
			/**
			 * Filters the member directory query result when wams_usermeta table is used.
			 *
			 * @since 2.1.3
			 * @hook wams_prepare_user_results_array_meta
			 *
			 ...
			 * function my_custom_wams_prepare_user_results_array_meta( $user_ids, $query_args ) {
			 *     $user_ids = array_diff( $user_ids, array( 10, 12 ) );
			 *     return $user_ids;
			 * }
			 * add_filter( 'wams_prepare_user_results_array_meta', 'my_custom_wams_prepare_user_results_array', 10, 2 );
			 */
			$user_ids = apply_filters('wams_prepare_user_results_array_meta', $user_ids, $query);
