...
if (!class_exists('wams\core\User')) {
	...
	class User
	{
		...
		public function __construct()
		{
			...
			add_action('wams_delete_user', array(&$this, 'remove_cache'), 10, 1);

			// When user cache should be cleared
			add_action('wams_after_user_updated', array(&$this, 'remove_cache'));
			add_action('wams_after_user_account_updated', array(&$this, 'remove_cache'));
			add_action('personal_options_update', array(&$this, 'remove_cache'));
			//add_action('edit_user_profile_update', array(&$this, 'remove_cache') );
			add_action('wams_when_role_is_set', array(&$this, 'remove_cache'));
			add_action('wams_when_status_is_set', array(&$this, 'remove_cache'));
			...
			add_filter('wams_user_profile_additional_fields', array($this, 'secondary_role_field'), 1, 2);
			...
			if (is_multisite()) {
				add_action('added_existing_user', array(&$this, 'add_wams_role_existing_user'), 10, 2);
				add_action('wpmu_activate_user', array(&$this, 'add_wams_role_wpmu_new_user'), 10, 1);
			}
			...
			add_action('update_user_meta', array(&$this, 'flush_wams_count_users_transient_update'), 10, 4);
			add_action('added_user_meta', array(&$this, 'flush_wams_count_users_transient_add'), 10, 4);
			add_action('delete_user_meta', array(&$this, 'flush_wams_count_users_transient_delete'), 10, 4);
		 ...
		 * Avoid to handle `meta_key` by the WAMS Forms if it contains $this->banned_keys inside.
		 *
		 * @since 2.6.5
		 *
		...
		public function is_metakey_banned($meta_key, $context = '')
		{
			...
			if (!$is_banned && 'submission' === $context && !in_array($meta_key, WAMS()->form()->usermeta_whitelist, true)) {
				$is_banned = true;
			}
		...
		/**
		 * Low-level checking to avoid updating banned user metakeys while WAMS Forms submission.
		 *
		 * @since 2.6.4
		 *
		 ...
		 * @param $_meta_value
		 */
		public function flush_wams_count_users_transient_update($meta_ids, $object_id, $meta_key, $_meta_value)
		{
			...
			if (!in_array($old, array('checkmail', 'pending'), true)) {
				...
				$count = get_transient("wams_count_users_{$old}");
				if (false !== $count) {
					if (!is_numeric($count)) {
						delete_transient("wams_count_users_{$old}");
					} else {
						if (0 < $count) {
							$count--;
						} else {
						...
						set_transient("wams_count_users_{$old}", $count);
					}
				}

				if (in_array($old, $pending_statuses, true) && !in_array($_meta_value, $pending_statuses, true)) {
					// deduct old transient count
					$count = get_transient('wams_count_users_pending_dot');
					if (false !== $count) {
						if (!is_numeric($count)) {
							...
							set_transient('wams_count_users_pending_dot', $count);
						}
					}
				}
			}
			...
			$count = get_transient("wams_count_users_{$_meta_value}");
			if (false !== $count) {
				if (is_numeric($count)) {
					$count++;
				} else {
			...
			set_transient("wams_count_users_{$_meta_value}", $count);
		 ...
		 * @param $_meta_value
		 */
		public function flush_wams_count_users_transient_add($meta_ids, $object_id, $meta_key, $_meta_value)
		{
			if ('account_status' !== $meta_key) {
				return;
			}
			...
			if (in_array($_meta_value, $pending_statuses, true)) {
				// add new transient count
				$pending_count = get_transient('wams_count_users_pending_dot');
				...
				set_transient('wams_count_users_pending_dot', $pending_count);
			}
		}
		 ...
		 * @param $_meta_value
		 */
		public function flush_wams_count_users_transient_delete($meta_ids, $object_id, $meta_key, $_meta_value)
		{
			if ('account_status' !== $meta_key) {
				return;
			}
			...
			// deduct old transient count
			$count = get_transient("wams_count_users_{$value}");
			if (false !== $count) {
				if (!is_numeric($count)) {
					delete_transient("wams_count_users_{$value}");
				} else {
					if (0 < $count) {
						$count--;
					} else {
					...
					set_transient("wams_count_users_{$value}", $count);
				}
			}
		...
		function on_delete_usermeta($meta_ids, $object_id, $meta_key, $_meta_value)
		{
			...
			$metakeys = array('account_status', 'hide_in_members', 'synced_gravatar_hashed_id', 'synced_profile_photo', 'profile_photo', 'cover_photo', '_wams_verified');
			if (!in_array($meta_key, $metakeys)) {
				return;
			}

			$md_data = get_user_meta($object_id, 'wams_member_directory_data', true);
			if (empty($md_data)) {
				$md_data = array(
					'account_status'  => 'approved',
					'hide_in_members' => WAMS()->member_directory()->get_hide_in_members_default(),
					'profile_photo'   => false,
					'cover_photo'     => false,
					'verified'        => false,
				);
			...
			switch ($meta_key) {
				...
				case 'hide_in_members':
					$md_data['hide_in_members'] = WAMS()->member_directory()->get_hide_in_members_default();
					break;
			...
			update_user_meta($object_id, 'wams_member_directory_data', $md_data);
		}
			...
			switch ($meta_key) {
				...
				case 'hide_in_members':
					...
					if (!empty($_meta_value)) {
						if (
							$_meta_value == 'Yes' || $_meta_value == __('Yes', 'wams') ||
							array_intersect(array('Yes', __('Yes', 'wams')), $_meta_value)
						) {
		...
		public function delete_user_handler($user_id)
		{
			 ...
			 *
			 * @type action
			 * @title wams_delete_user_hook
			 * @description On delete user
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_delete_user_hook', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_delete_user_hook', 'my_delete_user', 10 );
			 * function my_delete_user() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_delete_user_hook');

			/**
			 * WAMS hook
			 *
			 * @type action
			 * @title wams_delete_user
			 * @description On delete user
			 * @input_vars
			 * [{"var":"$user_id","type":"int","desc":"User ID"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_delete_user', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_action( 'wams_delete_user', 'my_delete_user', 10, 1 );
			 * function my_delete_user( $user_id ) {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_delete_user', wams_user('ID'));

			// remove user's comments
			if (WAMS()->options()->get('delete_comments')) {
				$user = get_user_by('id', wams_user('ID'));

				$comments = array_merge(get_comments('author_email=' . $user->user_email), get_comments('user_id=' . wams_user('ID')));
			...
			if ($this->send_mail_on_delete) {
				WAMS()->mail()->send(wams_user('user_email'), 'deletion_email');

				$emails = wams_multi_admin_email();
				if (!empty($emails)) {
					foreach ($emails as $email) {
						WAMS()->mail()->send($email, 'notification_deletion', array('admin' => true));
					}
				}
			}
			...
			WAMS()->files()->remove_dir(WAMS()->uploader()->get_upload_base_dir() . wams_user('ID') . DIRECTORY_SEPARATOR);
		...
		function check_membership()
		{
			...
			if ('rejected' == $status) {
				wp_logout();
				session_unset();
				exit(wp_redirect(wams_get_core_page('login')));
			}
		 ...
		 * @param $result
		 */
		function add_wams_role_existing_user($user_id, $result)
		{
			...
			if (!empty($_POST['um-role']) && current_user_can('promote_users')) {
				if (!user_can($user_id, sanitize_key($_POST['um-role']))) {
					WAMS()->roles()->set_role($user_id, sanitize_key($_POST['um-role']));
				}
			}
		 ...
		 * @param $user_id
		 */
		function add_wams_role_wpmu_new_user($user_id)
		{
			// Bail if no user ID was passed
			if (empty($user_id)) {
				return;
		...
		public function get_profile_slug($user_id, $raw = false)
		{
			...
			if (empty($profile_slug)) {
				if ('user_login' !== $permalink_base) {
					$profile_slug = get_user_meta($user_id, 'wams_user_profile_url_slug_user_login', true);
				}
		...
		function get_profile_link($user_id)
		{
			...
			return WAMS()->permalinks()->profile_permalink($profile_slug);
		}
		...
		public function generate_user_hash($user_id)
		{
			...
			while (1) {
				$user_in_url         = '~' . substr(strrev(md5(uniqid('wams_user_hash' . $append, true) . $user_id)), 0, 18);
				$slug_exists_user_id = WAMS()->permalinks()->slug_exists_user_id($user_in_url);
		...
		public function generate_profile_slug($user_id, $force = false)
		{
			...
			if ('custom_meta' === $permalink_base) {
				...
				if (empty($custom_meta)) {
					 ...
					 *
					 * @since 2.7.0
					 * @hook wams_custom_meta_permalink_base_generate_user_slug
					 *
					 ...
					 *     return $slug;
					 * }
					 * add_filter( 'wams_custom_meta_permalink_base_generate_user_slug', 'my_custom_meta_permalink_base_generate_user_slug', 10, 3 );
					 */
					$user_in_url = apply_filters('wams_custom_meta_permalink_base_generate_user_slug', $user_in_url, $user_id, $custom_meta);
					...
					if (empty($user_in_url)) {
						...
						if (is_email($user_in_url)) {
							...
							update_user_meta($user_id, "wams_email_as_username_{$user_in_url}", $user_email);
						} else {
							$user_in_url = urlencode($user_in_url);
						}
						update_user_meta($user_id, "wams_user_profile_url_slug_user_login", $user_in_url);
					}
				}
			}
			...
			if (in_array($permalink_base, $full_name_permalinks, true)) {
				...
				$profile_slug = WAMS()->permalinks()->profile_slug($full_name, $first_name, $last_name);
				...
				while (1) {
					...
					$slug_exists_user_id = WAMS()->permalinks()->slug_exists_user_id($profile_slug . (empty($append) ? '' : "{$separate}{$append}"));
					if (empty($slug_exists_user_id) || $user_id == $slug_exists_user_id) {
						break;
					}
					$append++;
				...
				$user_in_url = WAMS()->permalinks()->profile_slug($username, $first_name, $last_name);
				if (empty($user_in_url)) {
					$user_in_url = $userdata->user_login;
			...
			$user_in_url = apply_filters('wams_change_user_profile_slug', $user_in_url, $user_id);

			if ($force || empty($current_profile_slug) || $current_profile_slug !== $user_in_url) {
				update_user_meta($user_id, "wams_user_profile_url_slug_{$permalink_base}", $user_in_url);
			}
		}
		...
		function user_register_via_admin($user_id)
		{
			...
			if (is_admin()) {
				...
				if (!empty($_POST['um-role']) && current_user_can('promote_users')) {
					...
					WAMS()->user()->profile['role'] = sanitize_key($_POST['um-role']);
					WAMS()->user()->update_usermeta_info('role');
				}
				/** This action is documented in wams/includes/common/um-actions-register.php */
				do_action('wams_user_register', $user_id, $_POST, null);
			}
		...
		function profile_update($user_id, $old_data)
		{
			 ...
			 *
			 * @type action
			 * @title wams_after_member_role_upgrade
			 ...
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_after_member_role_upgrade', 'function_name', 10, 2 );
			 * @example
			 * <?php
			 * add_action( 'wams_after_member_role_upgrade', 'my_after_member_role_upgrade', 10, 2 );
			 * function my_after_member_role_upgrade( $new_roles, $old_roles ) {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_after_member_role_upgrade', $new_roles, $old_roles, $user_id);
		...
		/**
		 * Additional section for WP Profile page with WAMS data fields
		 *
		...
		function profile_form_additional_section($userdata)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_user_profile_additional_fields
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_user_profile_additional_fields', 'function_name', 10, 2 ); ?>
			 * @example
			 * <?php
			 * add_filter( 'wams_user_profile_additional_fields', 'my_admin_pending_queue', 10, 2 );
			 ...
			 * ?>
			 */
			$section_content = apply_filters('wams_user_profile_additional_fields', '', $userdata);

			if (!empty($section_content) && !(is_multisite() && is_network_admin())) {

				if ($userdata !== 'add-new-user' && $userdata !== 'add-existing-user') { ?>
					<h3 id="wams_user_screen_block"><?php esc_html_e('WAMS', 'wams'); ?></h3>
			<?php
				}
		...
		/**
		 * Default interface for setting a wams role
		 *
		...
		public function secondary_role_field($content, $userdata)
		{
			...
			if ($userdata !== 'add-new-user' && $userdata !== 'add-existing-user') {
				...
				$user_role = WAMS()->roles()->get_wams_user_role($userdata->ID);
				if ($user_role && !empty($userdata->roles) && count($userdata->roles) == 1) {
					$style = 'style="display:none;"';
				}
			}

			$class = ($userdata == 'add-existing-user') ? 'wams_role_existing_selector_wrapper' : 'wams_role_selector_wrapper';
			...
			<div id="<?php echo esc_attr($class); ?>" <?php echo $style; ?>>
				<table class="form-table">
					<tbody>
						<tr>
							<th><label for="um-role"><?php esc_html_e('WAMS Role', 'wams'); ?></label></th>
							<td>
								<select name="um-role" id="um-role">
									<option value="" <?php selected(empty($user_role)); ?>><?php esc_html_e('&mdash; No role for WAMS &mdash;', 'wams'); ?></option>
...
<?php
		...
		{
			$disallow_cache = WAMS()->options()->get('wams_profile_object_cache_stop');
			if ($disallow_cache) {
				return '';
			}

			if (is_numeric($user_id) && $user_id > 0) {
				$find_user = get_option("wams_cache_userdata_{$user_id}");
				if ($find_user) {
					/** This filter is documented in includes/core/class-roles-capabilities.php */
					return apply_filters('wams_user_permissions_filter', $find_user, $user_id);
				}
			}
			return '';
		}
		...
		function setup_cache($user_id, $profile)
		{
			...
			update_option("wams_cache_userdata_{$user_id}", $profile, false);
		}
		...
		{
			global $wpdb;
			$wpdb->query("DELETE FROM {$wpdb->options} WHERE option_name LIKE 'wams_cache_userdata_%'");
		}
		 ...
		 *
		 * @usage <?php WAMS()->user()->set( $user_id, $clean = false ); ?>
		 *
		...
		WAMS()->user()->set( 12 );
		$display_name = WAMS()->user()->profile['display_name']; // Should print user display name
			...
			if ($this->get_cached_data($this->id)) {
				...
				if ($this->id && $this->toArray($this->data)) {
					...
					if ($this->usermeta['account_status'][0] == 'approved') {
						$this->usermeta['account_status_name'][0] = __('Approved', 'wams');
					}

					if ($this->usermeta['account_status'][0] == 'awaiting_email_confirmation') {
						$this->usermeta['account_status_name'][0] = __('Awaiting E-mail Confirmation', 'wams');
					}

					if ($this->usermeta['account_status'][0] == 'awaiting_admin_review') {
						$this->usermeta['account_status_name'][0] = __('Pending Review', 'wams');
					}

					if ($this->usermeta['account_status'][0] == 'rejected') {
						$this->usermeta['account_status_name'][0] = __('Membership Rejected', 'wams');
					}

					if ($this->usermeta['account_status'][0] == 'inactive') {
						$this->usermeta['account_status_name'][0] = __('Membership Inactive', 'wams');
					}
					...
					// add permissions
					$user_role              = WAMS()->roles()->get_priority_user_role($this->id);
					$this->profile['role']  = $user_role;
					$this->profile['roles'] = WAMS()->roles()->get_all_user_roles($this->id);

					$role_meta = WAMS()->roles()->role_data($user_role);
					/** This filter is documented in includes/core/class-roles-capabilities.php */
					$role_meta = apply_filters('wams_user_permissions_filter', $role_meta, $this->id);
		 ...
		 *
		 * @usage <?php WAMS()->user()->auto_login( $user_id, $rememberme = false ); ?>
		 *
		...
		<?php WAMS()->user()->auto_login( 10, true ); ?>
		 *
		 */
		...
		public function set_registration_details($submitted, $args, $form_data)
		{
			...
			foreach ($submitted as $k => $v) {
				if ('password' === WAMS()->fields()->get_field_type($k)) {
					$password_fields[] = $k;
			 ...
			 *
			 * @since 2.0
			 * @hook wams_before_save_filter_submitted
			 *
			 ...
			 *     return $submitted;
			 * }
			 * add_filter( 'wams_before_save_filter_submitted', 'my_before_save_filter_submitted', 10, 3 );
			 */
			$submitted = apply_filters('wams_before_save_filter_submitted', $submitted, $args, $form_data);

			/**
			 * Fires before save registration details to the user.
			 *
			 * @since 1.3.x
			 * @hook wams_before_save_registration_details
			 *
			 * @param {int}   $user_id        User ID.
			 ...
			 *     // your code here
			 * }
			 * add_action( 'wams_before_save_registration_details', 'my_before_save_registration_details', 10, 3 );
			 */
			do_action('wams_before_save_registration_details', $this->id, $submitted, $form_data);
			 ...
			 *
			 * @since 1.3.x
			 * @hook wams_after_save_registration_details
			 *
			 * @param {int}   $user_id        User ID.
			 * @param {array} $submitted_data $_POST Submission array.
			 ...
			 *     // your code here
			 * }
			 * add_action( 'wams_after_save_registration_details', 'my_after_save_registration_details', 10, 3 );
			 */
			do_action('wams_after_save_registration_details', $this->id, $submitted, $form_data);
		}
		...
		{
			update_user_meta($this->id, '_wams_last_login', current_time('mysql', true));
		}
		...
		function set_status($status)
		{
			 ...
			 *
			 * @type action
			 * @title wams_when_status_is_set
			 * @description Action on user status changed
			 * @input_vars
			 * [{"var":"$user_id","type":"int","desc":"User ID"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_when_status_is_set', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_action( 'wams_when_status_is_set', 'my_when_status_is_set', 10, 1 );
			 * function my_when_status_is_set( $user_id ) {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_when_status_is_set', wams_user('ID'));
			 ...
			 *
			 * @type action
			 * @title wams_after_user_status_is_changed_hook
			 * @description Action after user status changed
			 * @input_vars
			 * [{"var":"$user_id","type":"int","desc":"User ID"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_after_user_status_is_changed_hook', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_after_user_status_is_changed_hook', 'my_after_user_status_is_changed', 10 );
			 * function my_after_user_status_is_changed() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_after_user_status_is_changed_hook', wams_user('ID'));

			/**
			 * WAMS hook
			 *
			 * @type action
			 * @title wams_after_user_status_is_changed
			 * @description Action after user status changed
			 * @input_vars
			 ...
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_after_user_status_is_changed', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_action( 'wams_after_user_status_is_changed', 'my_after_user_status_is_changed', 10, 1 );
			 * function my_after_user_status_is_changed( $status ) {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_after_user_status_is_changed', $status, wams_user('ID'));
		}
		...
		function assign_secretkey()
		{
			 ...
			 *
			 * @type action
			 * @title wams_before_user_hash_is_changed
			 * @description Action before user hash is changed
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_before_user_hash_is_changed', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_before_user_hash_is_changed', 'my_before_user_hash_is_changed', 10 );
			 * function my_before_user_hash_is_changed() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_before_user_hash_is_changed');

			$this->profile['account_secret_hash'] = WAMS()->validation()->generate();
			$this->update_usermeta_info('account_secret_hash');

			$expiry_time = WAMS()->options()->get('activation_link_expiry_time');
			 ...
			 *
			 * @type action
			 * @title wams_after_user_hash_is_changed
			 * @description Action after user hash is changed
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_after_user_hash_is_changed', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_after_user_hash_is_changed', 'my_after_user_hash_is_changed', 10 );
			 * function my_after_user_hash_is_changed() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_after_user_hash_is_changed');
		}
		...
		{
			$userdata = get_userdata(wams_user('ID'));
			...
			WAMS()->mail()->send($userdata->user_email, 'resetpw_email');
		}
		...
		function password_changed($user_id = null)
		{
			...
			WAMS()->mail()->send(wams_user('user_email'), 'changedpw_email');
		...
		function approve($repeat = true)
		{
			...
			if (wams_user('account_status') == 'awaiting_admin_review') {
				$userdata = get_userdata($user_id);

				$this->maybe_generate_password_reset_key($userdata);

				WAMS()->mail()->send(wams_user('user_email'), 'approved_email');
			} else {
				//$userdata = get_userdata( $user_id );
				//get_password_reset_key( $userdata );
				WAMS()->mail()->send(wams_user('user_email'), 'welcome_email');
			}
			 ...
			 *
			 * @type action
			 * @title wams_after_user_is_approved
			 * @description Action after user was approved
			 * @input_vars
			 * [{"var":"$user_id","type":"int","desc":"User ID"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_after_user_is_approved', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_action( 'wams_after_user_is_approved', 'my_after_user_is_approved', 10, 1 );
			 * function my_after_user_hash_is_changed( $user_id ) {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_after_user_is_approved', wams_user('ID'));
		}
		...
		function email_pending()
		{
			...
			$user = \WP_Session_Tokens::get_instance(wams_user('ID'));
			$user->destroy_all();

			WAMS()->mail()->send(wams_user('user_email'), 'checkmail_email');
		}
		...
		function pending()
		{
			...
			WAMS()->mail()->send(wams_user('user_email'), 'pending_email');
		}
		...
		function reject()
		{
			...
			WAMS()->mail()->send(wams_user('user_email'), 'rejected_email');
		}
		 ...
		 *
		 * @usage <?php WAMS()->user()->deactivate(); ?>
		 *
		...
		function deactivate()
		{
			 ...
			 *
			 * @type action
			 * @title wams_after_user_is_inactive
			 * @description Action after user was inactive
			 * @input_vars
			 * [{"var":"$user_id","type":"int","desc":"User ID"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_after_user_is_inactive', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_action( 'wams_after_user_is_inactive', 'my_after_user_is_inactive', 10, 1 );
			 * function my_after_user_is_inactive( $user_id ) {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_after_user_is_inactive', wams_user('ID'));

			WAMS()->mail()->send(wams_user('user_email'), 'inactive_email');
		}
		...
		function delete($send_mail = true)
		{
			...
			if ('approved' != wams_user('account_status')) {
		...
		if ( WAMS()->user()->get_role() == 'paid-member' ) {
		 ...
		 *
		 * @usage <?php WAMS()->user()->delete_meta( $key ); ?>
		 *
		...
		function get_admin_actions()
		{
			 ...
			 * ?>
			 */
			$actions = apply_filters('wams_admin_user_actions_hook', array(), wams_profile_id());
			if (empty($actions)) {
				return $items;
			}
			...
			foreach ($actions as $id => $arr) {
				...
				/*$url = add_query_arg( 'wams_action', $id );
				$url = add_query_arg( 'uid', wams_profile_id(), $url );*/
		 ...
		 *
		 * @since 2.1.16
		 * @usage <?php WAMS()->user()->is_profile_noindex( $user_id ); ?>
		 *
		...
		function is_profile_noindex($user_id)
		{
			...
			if (!$profile_noindex) {
				...
				$permissions = WAMS()->roles()->role_data($role);

				if (isset($permissions['profile_noindex']) && '' !== $permissions['profile_noindex']) {
					// Setting "Avoid indexing profile by search engines" in [wp-admin > WAMS > User Roles > Edit Role]
					$profile_noindex = (bool) $permissions['profile_noindex'];
				} else {
					// Setting "Avoid indexing profile by search engines" in [wp-admin > WAMS > Settings > General > Users]
					$profile_noindex = (bool) WAMS()->options()->get('profile_noindex');
				}
			}

			return apply_filters('wams_user_is_profile_noindex', $profile_noindex, $user_id, $this);
		}
		 ...
		 *
		 * @usage <?php WAMS()->user()->is_private_profile( $user_id ); ?>
		 *
		...
		wams_fetch_user( 60 );
		$is_private = WAMS()->user()->is_private_profile( 60 );
		if ( ! $is_private ) {
		echo 'User is public and his name is ' . wams_user('display_name');
		}
		...
		function is_private_profile($user_id)
		{
			...
			if ($privacy == __('Only me', 'wams') || $privacy == 'Only me') {
				return true;
			}
		 ...
		 *
		 * @usage <?php WAMS()->user()->is_approved( $user_id ); ?>
		 *
		...
		function is_private_case($user_id, $case)
		{
			...
			if ($privacy == $case) {
				 ...
				 *
				 * @type filter
				 * @title wams_is_private_filter_hook
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_is_private_filter_hook', 'function_name', 10, 3 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_is_private_filter_hook', 'my_is_private_filter', 10, 3 );
				 ...
				 * ?>
				 */
				$bool = apply_filters('wams_is_private_filter_hook', false, $privacy, $user_id);
				return $bool;
			}
		...
		{
			wams_deprecated_function('update_files', '2.1.0', '');
		}
		...
		public function update_profile($changes, $context = '')
		{
			 ...
			 *
			 * @since 1.3.x
			 * @hook wams_before_update_profile
			 *
			 ...
			 *     return $changes;
			 * }
			 * add_filter( 'wams_before_update_profile', 'my_custom_before_update_profile', 10, 2 );
			 */
			$changes = apply_filters('wams_before_update_profile', $changes, $args['ID']);
		...
		function user_has_metadata($key, $value)
		{
			...
			$value = WAMS()->validation()->safe_name_in_url($value);
		...
		public function user_exists_by_name($value)
		{
			...
			$raw_value = $value;
			$value     = WAMS()->validation()->safe_name_in_url($value);
			$value     = wams_clean_user_basename($value);
			...
			if ($ids->total_users > 0) {
				$wams_user_query = current($ids->get_results());
				return $wams_user_query->ID;
			}
		 ...
		 *
		 * @usage <?php WAMS()->user()->user_exists_by_id( $user_id ); ?>
		 *
		...
		$boolean = WAMS()->user()->user_exists_by_id( 15 );
		if ( $boolean ) {
		// That user exists
		}
		 ...
		 *
		 * @usage <?php WAMS()->user()->user_exists_by_email_as_username( $slug ); ?>
		 *
		 * @return bool
		 *
		 * @example Basic Usage
		...
		$boolean = WAMS()->user()->user_exists_by_email_as_username( 'calumgmail-com' );
		if ( $boolean ) {
		// That user exists
		}
		...
		function user_exists_by_email_as_username($slug)
		{
			...
			$ids = get_users(
				array(
					'fields'   => 'ID',
					'meta_key' => 'wams_email_as_username_' . $slug,
				)
			);
			if (!empty($ids[0])) {
				$user_id = $ids[0];
		...
		function set_gravatar($user_id)
		{
			...
			wams_fetch_user($user_id);
			$email_address      = wams_user('user_email');
			$hash_email_address = '';
		...
		/**
		 * WAMS Placeholders for activation link in email
		 *
		 * @param $placeholders
		 *
		 * @return array
		...
		/**
		 * WAMS Replace Placeholders for activation link in email
		 *
		...
		{
			$replace_placeholders[] = wams_user('account_activation_link');
		...
		{
			_deprecated_function(__METHOD__, '2.4.2', 'WAMS()->query()->get_pending_users_count()');
			return WAMS()->query()->get_pending_users_count();
		}
