...
if (!class_exists('wams\admin\core\Admin_DragDrop')) {
	...
	class Admin_DragDrop
	{
		...
		public function update_order()
		{
			...
			if (empty($_POST['form_id'])) {
				wp_send_json_error(__('Invalid form ID.', 'wams'));
			}
			...
			$this->row_data   = get_option('wams_form_rowdata_' . $form_id, array());
			...
			foreach ($_POST as $key => $value) {
				...
				// adding rows
				if (0 === strpos($key, '_wams_row_')) {
					$update_args = null;

					$row_id = str_replace('_wams_row_', '', $key);

					if (false !== strpos($_POST['_wams_rowcols_' . $row_id . '_cols'], ':')) {
						$cols = sanitize_text_field($_POST['_wams_rowcols_' . $row_id . '_cols']);
					} else {
						$cols = absint($_POST['_wams_rowcols_' . $row_id . '_cols']);
					}

					$row_array = array(
						'type'     => 'row',
						'id'       => sanitize_key($value),
						'sub_rows' => absint($_POST['_wams_rowsub_' . $row_id . '_rows']),
						'cols'     => $cols,
						'origin'   => sanitize_key($_POST['_wams_roworigin_' . $row_id . '_val']),
					);
				...
				if (0 === strpos($key, 'wams_position_')) {
					$field_key = str_replace('wams_position_', '', $key);
				...
				if (0 === strpos($key, 'wams_row_')) {
					$field_key = str_replace('wams_row_', '', $key);
					if (isset($fields[$field_key])) {
				...
				if (0 === strpos($key, 'wams_subrow_')) {
					$field_key = str_replace('wams_subrow_', '', $key);
					if (isset($fields[$field_key])) {
				...
				// change field column
				if (0 === strpos($key, 'wams_col_')) {
					$field_key = str_replace('wams_col_', '', $key);
					if (isset($fields[$field_key])) {
				...
				// add field to group
				if (0 === strpos($key, 'wams_group_')) {
					$field_key = str_replace('wams_group_', '', $key);
					if (isset($fields[$field_key])) {
			...
			update_option('wams_existing_rows_' . $form_id, $this->exist_rows);

			update_option('wams_form_rowdata_' . $form_id, $this->row_data);
		...
		public function load_field_order()
		{
			...
			if (!isset($screen, $screen->id) || 'wams_form' !== $screen->id) {
				return;
			} ?>
			...
			<div class="um-col-demon-row" style="display:none;">
				...
				<div class="um-admin-drag-row-icons">
					...
					<a href="javascript:void(0);" class="um-admin-drag-row-edit um-tip-n" title="<?php esc_attr_e('Edit Row', 'wams'); ?>" data-modal="WAMS_edit_row" data-modal-size="normal" data-dynamic-content="wams_admin_edit_field_popup" data-arg1="row" data-arg2="<?php echo esc_attr(get_the_ID()); ?>"><i class="um-faicon-pencil"></i></a>
