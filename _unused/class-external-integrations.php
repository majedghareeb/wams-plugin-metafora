...
if (!class_exists('wams\core\External_Integrations')) {
	...
	class External_Integrations
	{
		...
		{
			//WPML translations
			add_filter('wams_get_core_page_filter', array(&$this, 'get_core_page_url'), 10, 3);
			add_filter('wams_admin_settings_email_section_fields', array(&$this, 'wams_admin_settings_email_section_fields'), 10, 2);
			add_filter('wams_email_send_subject', array(&$this, 'wams_email_send_subject'), 10, 2);
			add_filter('wams_locate_email_template', array(&$this, 'locate_email_template'), 10, 2);
			add_filter('wams_change_email_template_file', array(&$this, 'change_email_template_file'), 10, 1);
			add_filter('wams_email_templates_columns', array(&$this, 'add_email_templates_wpml_column'), 10, 1);


			add_action('wams_access_fix_external_post_content', array(&$this, 'bbpress_no_access_message_fix'), 10);
			add_action('wams_access_fix_external_post_content', array(&$this, 'forumwp_fix'), 11);
			add_action('wams_access_fix_external_post_content', array(&$this, 'woocommerce_fix'), 12);

			add_filter('wams_external_profile_url', array(&$this, 'wams_localize_profile_url'), 10, 2);
			add_filter('wams_get_current_page_url', array(&$this, 'wams_localize_profile_nav_url'), 10, 2);
			add_filter('icl_ls_languages', array(&$this, 'wams_core_page_wpml_permalink'), 10, 1);
			 ...
			 */
			//add_filter( 'wams_pre_args_setup',  array( &$this, 'shortcode_pre_args_setup' ), 20, 1 );
		...
		/**
		 * WAMS filter - Restore original arguments on translated page
		 *
		...
		function shortcode_pre_args_setup($args)
		{
			if (WAMS()->external_integrations()->is_wpml_active()) {
				...
				if ($original_form_id != $args['form_id']) {
					$original_post_data = WAMS()->query()->post_data($original_form_id);
		...
		public function transposh_user_profile()
		{
			...
			$profile_id = WAMS()->options()->get('core_user');
			...
			if (!empty($_SERVER['REQUEST_URI']) && stripos($_SERVER['REQUEST_URI'], "$my_transposh_plugin->target_language/$post->post_name") !== false) {
				...
				if (isset($matches[1])) {
					...
					set_query_var('wams_user', $matches[1]);
					wp_reset_postdata();
				}
			}
		}
		...
		{
			if (WAMS()->dependencies()->woocommerce_active_check()) {
		 ...
		 *
		 * @see  \wams\core\Permalinks::profile_permalink()
		 *
		 ...
		 * @return string
		 */
		public function wams_localize_profile_url($profile_url, $slug)
		{
			...
			if ($lang_post_id && $lang_post_id !== $page_id) {
				...
				if (WAMS()->is_permalinks) {
					...
					$profile_url = add_query_arg('wams_user', strtolower($slug), $user_page_url);
				}
			}
		 ...
		 *
		 * @see  \wams\core\Permalinks::get_current_url()
		 *
		 ...
		 * @return string
		 */
		public function wams_localize_profile_nav_url($page_url, $no_query_params)
		{
			if (!$this->is_wpml_active()) {
				return $page_url;
			}
		 ...
		 * @return array
		 */
		public function wams_core_page_wpml_permalink($array)
		{
			if ($this->is_wpml_active() && wams_is_core_page('user') && is_array($array)) {
				global $sitepress;

				$current_language = wpml_get_current_language();
				$slug             = strtolower(WAMS()->user()->get_profile_slug(wams_profile_id()));

				foreach ($array as $lang_code => $arr) {
					$sitepress->switch_lang($lang_code);

					$lang_post_id  = apply_filters('wpml_object_id', WAMS()->config()->permalinks['user'], 'page', true);
					$user_page_url = get_permalink($lang_post_id);
		...
		function get_core_page_url($url, $slug, $updated)
		{
			...
			if (function_exists('icl_get_current_language') && icl_get_current_language() != icl_get_default_language()) {
				$url = $this->get_url_for_language(WAMS()->config()->permalinks[$slug], icl_get_current_language());
		 ...
		 * @return array
		 */
		function wams_admin_settings_email_section_fields($section_fields, $email_key)
		{
			if (!$this->is_wpml_active()) {
			...
			$value_default = WAMS()->options()->get($email_key . '_sub');
			$value = WAMS()->options()->get($email_key . '_sub' . $lang);
		 ...
		 * @return string
		 */
		function wams_email_send_subject($subject, $template)
		{
			if (!$this->is_wpml_active()) {
				return $subject;
			}
			...
			$value_default = WAMS()->options()->get($template . '_sub');
			$value = WAMS()->options()->get($template . '_sub' . $lang);
		...
		function locate_email_template($template, $template_name)
		{
			...
			$template = locate_template(array(
				trailingslashit('wams/email') . $lang . $template_name . '.php',
				trailingslashit('wams/email') . $template_name . '.php'
			));

			//if there isn't template at theme folder get template file from plugin dir
			if (!$template) {
				$path = !empty(WAMS()->mail()->path_by_slug[$template_name]) ? WAMS()->mail()->path_by_slug[$template_name] : WAMS_PATH . 'templates/email';
		...
		function get_status_html($template, $code)
		{
			...
			//theme location
			$template_path = trailingslashit(get_stylesheet_directory() . '/wams/email') . $lang . $template . '.php';

			//plugin location for default language
			if (empty($lang) && !file_exists($template_path)) {
				$template_path = WAMS()->mail()->get_template_file('plugin', $template);
			}
