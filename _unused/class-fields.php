...
?>
		...
		public function show_social_urls()
		{
			...
			?>
					...
					<a href="<?php echo esc_url(wams_filtered_social_link($k, $match)); ?>" style="background: <?php echo esc_attr($arr['color']); ?>;" target="<?php echo esc_attr($arr['url_target']); ?>" class="um-tip-n" title="<?php echo esc_attr($arr['title']); ?>"><i class="<?php echo esc_attr($arr['icon']); ?>"></i></a>
		...
		function update_field($id, $args, $form_id)
		{
			...
			if (isset(WAMS()->builtin()->predefined_fields[$id]) && isset(WAMS()->builtin()->predefined_fields[$id]['custom'])) {
				$args = array_merge(WAMS()->builtin()->predefined_fields[$id], $args);
			}
		...
		public function field_error($text, $input_id, $force_show = false)
		{
			...
			if ($force_show) {
				$output = '<div class="um-field-error" id="' . esc_attr($error_id) . '"><span class="um-field-arrow"><i class="um-faicon-caret-up"></i></span>' . wp_kses($text, WAMS()->get_allowed_html('templates')) . '</div>';
				return $output;
			}
		...
		public function field_notice($text, $input_id, $force_show = false)
		{
			...
			if ($force_show) {
				$output = '<div class="um-field-notice" id="' . esc_attr($notice_id) . '"><span class="um-field-arrow"><i class="um-faicon-caret-up"></i></span>' . wp_kses($text, WAMS()->get_allowed_html('templates')) . '</div>';
				return $output;
			}
		...
		public function field_label($label, $key, $data)
		{
			...
			if (true === $this->viewing) {
				/**
				 * Filters WAMS field label on the Profile form: View mode.
				...
				/**
				 * Filters WAMS field label on the Profile form: Edit mode.
				 * Note: $key it's field metakey.
				 *
				 * @since 2.0.0
				 * @since 2.8.0 Added $data attribute.
			...
			$output .= '<label' . $for_attr . '>' . __($label, 'wams') . '</label>';
			...
			if (!empty($data['help']) && false === $this->viewing && false === strpos($key, 'confirm_user_pass')) {
				if (!WAMS()->mobile()->isMobile()) {
					if (false === $this->disable_tooltips) {
						$output .= '<span class="um-tip um-tip-' . (is_rtl() ? 'e' : 'w') . '" title="' . esc_attr__($data['help'], 'wams') . '"><i class="um-icon-help-circled"></i></span>';
					}
				}

				if (false !== $this->disable_tooltips || WAMS()->mobile()->isMobile()) {
					$output .= '<span class="um-tip-text">' . __($data['help'], 'wams') . '</span>';
				}
			}
		...
		function field_value($key, $default = false, $data = null)
		{
			...
			if (isset(WAMS()->form()->post_form[$key])) {
				...
				if ('profile' === $this->set_mode) {
					if (!isset(WAMS()->form()->post_form['profile_nonce']) || false === wp_verify_nonce(WAMS()->form()->post_form['profile_nonce'], 'um-profile-nonce' . WAMS()->user()->target_id)) {
						return '';
					}
				}
			...
			switch ($array['type']) {
				...
				case 'image':
					...
					if (!isset($array['invalid_image'])) {
						$array['invalid_image'] = __('Please upload a valid image!', 'wams');
					}
					...
					if (!isset($array['button_text'])) {
						$array['button_text'] = __('Upload', 'wams');
					}
					if (!isset($array['extension_error'])) {
						$array['extension_error'] = __('Sorry this is not a valid image.', 'wams');
					}
					if (!isset($array['max_size_error'])) {
						$array['max_size_error'] = __('This image is too large!', 'wams');
					}
					if (!isset($array['min_size_error'])) {
						$array['min_size_error'] = __('This image is too small!', 'wams');
					}
					if (!isset($array['max_files_error'])) {
						$array['max_files_error'] = __('You can only upload one image', 'wams');
					}
				...
				case 'file':
					...
					if (!isset($array['extension_error'])) {
						$array['extension_error'] = __('Sorry this is not a valid file.', 'wams');
					}
					if (!isset($array['max_size_error'])) {
						$array['max_size_error'] = __('This file is too large!', 'wams');
					}
					if (!isset($array['min_size_error'])) {
						$array['min_size_error'] = __('This file is too small!', 'wams');
					}
					if (!isset($array['max_files_error'])) {
						$array['max_files_error'] = __('You can only upload one file', 'wams');
					}
					if (empty($array['max_size'])) {
						$array['max_size'] = 999999999;
					}
			...
			switch ($type) {
				...
				case 'password':
					...
					if ('single_user_password' === $key) {
						...
						if (('account' === $this->set_mode || wams_is_core_page('account')) && WAMS()->account()->current_password_is_required('password')) {
							...
							if (isset($data['label'])) {
								$output .= $this->field_label(__('Current Password', 'wams'), $key, $data);
							}
						...
						if (('account' === $this->set_mode && wams_is_core_page('account')) || ('password' === $this->set_mode && wams_is_core_page('password-reset'))) {
							...
							$output .= $this->field_label(__('New Password', 'wams'), $key, $data);
						...
						if ('login' !== $this->set_mode && !empty($data['force_confirm_pass'])) {
							...
							if (!empty($data['label_confirm_pass'])) {
								$label_confirm_pass = __($data['label_confirm_pass'], 'wams');
								$output            .= $this->field_label($label_confirm_pass, $key, $data);
							} elseif (isset($data['label'])) {
								$data['label'] = __($data['label'], 'wams');
								// translators: %s: label.
								$output .= $this->field_label(sprintf(__('Confirm %s', 'wams'), $data['label']), $key, $data);
							}
							...
							if (!empty($data['label_confirm_pass'])) {
								$placeholder = __($data['label_confirm_pass'], 'wams');
							} elseif (!empty($placeholder) && !isset($data['label'])) {
								// translators: %s: placeholder.
								$placeholder = sprintf(__('Confirm %s', 'wams'), $placeholder);
							} elseif (isset($data['label'])) {
								// translators: %s: label.
								$placeholder = sprintf(__('Confirm %s', 'wams'), $data['label']);
							}
				...
				case 'date':
					...
					if ($value) {
						...
						// WAMS date field stores the date in metatable in the format Y/m/d. Convert to it before echo.
				...
				case 'divider':
					...
					/* Single Image Upload */
				case 'image':
					$output .= '<div ' . $this->get_atts($key, $classes, $conditional, $data) . ' data-mode="' . esc_attr($this->set_mode) . '" data-upload-label="' . (!empty($data['button_text']) ? esc_attr($data['button_text']) : esc_attr__('Upload', 'wams')) . '">';
					...
					$modal_label = array_key_exists('label', $data) ? $data['label'] : __('Upload Photo', 'wams');
					...
					if (!empty($field_value) && 'empty_file' !== $field_value) {
						...
						$output .= '</div>';
						if (empty($disabled)) {
							$output .= '<a href="javascript:void(0);" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="um-button um-btn-auto-width">' . esc_html__('Change photo', 'wams') . '</a>';
						}
					} else {
						...
						if (empty($disabled)) {
							$output .= '<a href="javascript:void(0);" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="um-button um-btn-auto-width">' . esc_html($data['button_text']) . '</a>';
						}
					}
					$output .= '</div>';
					/* modal hidden */
				...
				case 'file':
					...
					$modal_label = array_key_exists('label', $data) ? $data['label'] : __('Upload File', 'wams');
					$output     .= '<div class="um-field-area" style="text-align: center;">';
					...
					if (!empty($file_field_value) && 'empty_file' !== $file_field_value) {
						...
						if (file_exists($file_dir)) {
							...
							$output .= '<div class="um-single-file-preview show" data-key="' . esc_attr($key) . '">' . esc_html__('This file has been removed.', 'wams') . '</div>';
						}

						if (empty($disabled)) {
							$output .= '<a href="#" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="um-button um-btn-auto-width">' . esc_html__('Change file', 'wams') . '</a>';
						}
					} else {
						$output .= '<div class="um-single-file-preview" data-key="' . esc_attr($key) . '"></div>';
						if (empty($disabled)) {
							$output .= '<a href="#" data-modal="wams_upload_single" data-modal-size="' . esc_attr($data['modal_size']) . '" data-modal-copy="1" class="um-button um-btn-auto-width">' . esc_html($data['button_text']) . '</a>';
						}
					}
					$output .= '</div>';
					/* modal hidden */
					if (empty($disabled)) {
						...
						$nonce   = wp_create_nonce('wams_upload_nonce-' . $this->timestamp);
						$output .= '<div class="um-single-file-upload" data-user_id="' . esc_attr($_wams_profile_id) . '" data-timestamp="' . esc_attr($this->timestamp) . '" data-nonce="' . esc_attr($nonce) . '" ' . $data_icon . ' data-set_id="' . esc_attr($set_id) . '" data-set_mode="' . esc_attr($set_mode) . '" data-type="' . esc_attr($type) . '" data-key="' . esc_attr($key) . '" data-max_size="' . esc_attr($data['max_size']) . '" data-max_size_error="' . esc_attr($data['max_size_error']) . '" data-min_size_error="' . esc_attr($data['min_size_error']) . '" data-extension_error="' . esc_attr($data['extension_error']) . '"  data-allowed_types="' . esc_attr($allowed_types) . '" data-upload_text="' . esc_attr($data['upload_text']) . '" data-max_files_error="' . esc_attr($data['max_files_error']) . '" data-upload_help_text="' . esc_attr($data['upload_help_text']) . '">' . esc_html($data['button_text']) . '</div>';
						$output .= '<div class="um-modal-footer">
									<div class="um-modal-right">
										<a href="javascript:void(0);" class="um-modal-btn um-finish-upload file disabled" data-key="' . esc_attr($key) . '" data-change="' . esc_attr__('Change file', 'wams') . '" data-processing="' . esc_attr__('Processing...', 'wams') . '"> ' . esc_html__('Save', 'wams') . '</a>
										<a href="javascript:void(0);" class="um-modal-btn alt" data-action="wams_remove_modal"> ' . esc_html__('Cancel', 'wams') . '</a>
									</div>
									<div class="um-clear"></div>
								</div>';
				...
				case 'select':
					...
					if (!empty($data['parent_dropdown_relationship']) && !WAMS()->user()->preview) {
						...
						if (!empty($data['custom_dropdown_options_source']) && function_exists($data['custom_dropdown_options_source']) && wams_user($data['parent_dropdown_relationship'])) {
							...
							if (wams_user($form_key)) {
								$select_original_option_value = ' data-um-original-value="' . esc_attr(wams_user($form_key)) . '" ';
							}
						}
					}
					...
					if (isset(WAMS()->form()->post_form[$form_key])) {
						$select_original_option_value = " data-um-original-value='" . esc_attr(WAMS()->form()->post_form[$form_key]) . "' ";
					}
					...
					if (!empty($options)) {
						foreach ($options as $k => $v) {
							...
							if ((!is_numeric($k) && 'role' === $form_key) || ('account' === $this->set_mode || wams_is_core_page('account'))) {
								$option_value = $k;
							}
							...
							$output .= '>' . esc_html__($wams_field_checkbox_item_title, 'wams') . '</option>';
						}
					}
				...
				case 'radio':
					...
					if (!empty($options)) {
						foreach ($options as $k => $v) {
							...
							$output .= '<span class="um-field-radio-option">' . esc_html__($wams_field_checkbox_item_title, 'wams') . '</span>';
							$output .= '</label>';
				...
				case 'checkbox':
					...
					foreach ($options as $k => $v) {
						...
						$output .= '<span class="um-field-checkbox-option">' . esc_html__($wams_field_checkbox_item_title, 'wams') . '</span>';
						$output .= '</label>';
				...
				case 'group':
					...
					if (!empty($fields)) {
						...
						$output .= '<div class="um-field-group" data-max_entries="' . esc_attr($data['max_entries']) . '">
								<div class="um-field-group-head"><i class="um-icon-plus"></i>' . esc_html__($data['label'], 'wams') . '</div>';
		...
		public function display($mode, $args)
		{
			...
			if ('profile' === $mode) {
				WAMS()->form()->nonce = wp_create_nonce('um-profile-nonce' . WAMS()->user()->target_id);
			}
				...
				?>
					...
					<div class="um-field-area">
						<div class="um-field-value">
							<div class="um-rating-readonly um-raty" id="<?php echo esc_attr($key); ?>" data-key="<?php echo esc_attr($key); ?>" data-number="<?php echo esc_attr($number); ?>" data-score="<?php echo esc_attr($this->field_value($key, $default, $data)); ?>"></div>
						</div>
					</div>
		...
		public function display_view($mode, $args)
		{
			...
			if (WAMS()->options()->get('profile_empty_text')) {
				...
				if (wams_is_myprofile()) {
					...
					$output .= '<p class="um-profile-note">' . $emo . '<span>' . sprintf(__('Your profile is looking a little empty. Why not <a href="%s">add</a> some information!', 'wams'), esc_url($edit_url)) . '</span></p>';
				} else {
					$output .= '<p class="um-profile-note">' . $emo . '<span>' . __('This user has not added any information to their profile yet.', 'wams') . '</span></p>';
				}
			}
