 ...
 *
 * This template can be overridden by copying it to yourtheme/wams/members-list.php
 *
 * Page: "Members"
 *
 * @version 2.6.1
				...
				<div id="um-member-{{{user.card_anchor}}}-<?php echo esc_attr($unique_hash) ?>" class="um-member um-role-{{{user.role}}} {{{user.account_status}}} <?php if ($cover_photos) {
						...
						<?php if ($profile_photo) { ?>
							<div class="um-member-photo radius-<?php echo esc_attr(WAMS()->options()->get('profile_photocorner')); ?>">
								<a href="{{{user.profile_url}}}" title="<# if ( user.display_name ) { #>{{{user.display_name}}}<# } #>">
									...
									<?php do_action('wams_members_list_in_profile_photo_tmpl', $args); ?>
								</a>
							</div>
						<?php } ?>
						...
						<div class="um-member-card <?php echo !$profile_photo ? 'no-photo' : '' ?>">
							<div class="um-member-card-content">
								<div class="um-member-card-header">
									...
									<?php do_action('wams_members_list_after_user_name_tmpl', $args); ?>
								...
								if ($show_userinfo) { ?>
									...
									<# if ( $show_block ) { #>
										<div class="um-member-meta-main<?php if (!$userinfo_animate) {
																			echo ' no-animate';
																		} ?>">
							...
							<div class="um-member-card-actions">
								...
								<# if ( Object.keys( user.dropdown_actions ).length > 0 ) { #>
									<div class="um-member-cog">
										...
										<?php WAMS()->member_directory()->dropdown_menu_js('.um-member-cog', 'click', 'user'); ?>
									</div>
								<# } #>
					...
					<div class="um-member-card-footer <?php echo !$profile_photo ? 'no-photo' : '' ?> <?php if ($show_userinfo && $userinfo_animate) { ?><# if ( ! $show_block ) { #>no-reveal<# } #><?php } ?>">
						...
						<div class="um-member-card-footer-buttons">
							<?php do_action('wams_members_list_just_after_actions_tmpl', $args); ?>
						</div>
