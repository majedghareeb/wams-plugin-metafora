...
if (!class_exists('wams\admin\core\Admin_Forms_Settings')) {
	...
	class Admin_Forms_Settings extends Admin_Forms
	{
		...
		{
			$default = isset($field_data['default' . $i]) ? $field_data['default' . $i] : WAMS()->options()->get_default($field_data['id' . $i]);

			if ($field_data['type'] == 'checkbox' || $field_data['type'] == 'multi_checkbox') {
				if (isset($field_data['value' . $i])) {
					return $field_data['value' . $i];
				} else {
					$value = WAMS()->options()->get($field_data['id' . $i]);
