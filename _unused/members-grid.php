 ...
 *
 * This template can be overridden by copying it to yourtheme/wams/members-grid.php
 *
 * Page: "Members"
 *
 * @version 2.6.1
	...
	<div class="um-members um-members-grid">
		...
		<# if ( data.length > 0 ) { #>
			<# _.each( data, function( user, key, list ) { #>
				...
				<div id="um-member-{{{user.card_anchor}}}-<?php echo esc_attr($unique_hash) ?>" class="um-member um-role-{{{user.role}}} {{{user.account_status}}} <?php if ($cover_photos) {
																																										echo 'with-cover';
																																									} ?>">
					...
					<?php if ($cover_photos) { ?>
						<div class="um-member-cover" data-ratio="<?php echo esc_attr(WAMS()->options()->get('profile_cover_ratio')); ?>">
					...
					if ($profile_photo) { ?>
						<div class="um-member-photo radius-<?php echo esc_attr(WAMS()->options()->get('profile_photocorner')); ?>">
							<a href="{{{user.profile_url}}}" title="<# if ( user.display_name ) { #>{{{user.display_name}}}<# } #>">
								{{{user.avatar}}}
								<?php do_action('wams_members_in_profile_photo_tmpl', $args); ?>
							</a>
						</div>
					<?php } ?>


					<div class="um-member-card <?php if (!$profile_photo) {
													echo 'no-photo';
												} ?>">
						<?php if ($show_name) { ?>
						...
						do_action('wams_members_just_after_name_tmpl', $args); ?>
						...
						<# if ( user.can_edit ) { #>
							<div class="um-members-edit-btn">
								<a href="{{{user.edit_profile_url}}}" class="um-edit-profile-btn um-button um-alt">
									<?php _e('Edit profile', 'wams') ?>
								</a>
							</div>
						<# } #>


						<?php do_action('wams_members_after_user_name_tmpl', $args); ?>
