...
if (!class_exists('wams\core\Member_Directory')) {
	...
	class Member_Directory
	{
		...
		{
			/**
			 * Filters the WordPress core searching fields in wp_users query for WAMS Member directory query.
			 *
			 ...
			 *
			 * @since 2.6.10
			 * @hook wams_member_directory_core_search_fields
			 *
			 * @example <caption>Extends or remove wp_users core search fields.</caption>
			 * function my_wams_member_directory_core_search_fields( $core_search_fields ) {
			 *     $core_search_fields = array_flip( $core_search_fields );
			 *     unset( $core_search_fields['user_email'] );
			 *     $core_search_fields = array_flip( $core_search_fields );
			 *     return $core_search_fields;
			 * }
			 * add_filter( 'wams_member_directory_core_search_fields', 'my_wams_member_directory_core_search_fields' );
			 */
			return apply_filters('wams_member_directory_core_search_fields', $this->core_search_fields);
		}
		...
		{
			$default = false;
			$option = WAMS()->options()->get('account_hide_in_directory_default');
			if ($option == 'Yes') {
				$default = true;
			}

			$default = apply_filters('wams_member_directory_hide_in_members_default', $default);
			return $default;
		}
		...
		{
			return apply_filters("wams_member_directory_{$type}_type_template_basename", '');
		}
		...
		{
			if (WAMS()->options()->get('members_page') == 0 && wams_is_core_page('members')) {
				wams_redirect_home();
			}
		}
		...
		public function before_save_data($value, $key, $post_id)
		{
			...
			if ('wams_directory' === $post->post_type) {
				...
				if (!empty($value) && in_array($key, array('_wams_view_types', '_wams_roles', '_wams_roles_can_search', '_wams_roles_can_filter'), true)) {
					$value = array_keys($value);
				} elseif ('_wams_search_filters' === $key) {
					$temp_value = array();
					...
					$value = $temp_value;
				} elseif ('_wams_sorting_fields' === $key) {
				...
				} elseif ('_wams_sortby_custom' === $key) {
					$value = sanitize_text_field($value);
				} elseif ('_wams_sortby_custom_label' === $key) {
					$value = wp_strip_all_tags($value);
				} elseif ('_wams_sortby_custom_type' === $key) {
					$value = sanitize_text_field($value);
				} elseif ('_wams_sortby_custom_order' === $key) {
					$value = sanitize_text_field($value);
				}
			}
		...
		function init_variables()
		{
			...
			// Types
			$this->view_types = apply_filters('wams_member_directory_views', array(
				'grid'  => array('title' => __('Grid', 'wams'), 'icon' => 'um-faicon-th'),
				'list'  => array('title' => __('List', 'wams'), 'icon' => 'um-faicon-list'),
			));

			// Sort
			$this->sort_fields = apply_filters('wams_members_directory_sort_fields', array(
			...
			$this->sorting_supported_fields = apply_filters('wams_members_directory_custom_field_types_supported_sorting', array('number'));

			$this->sort_data_types = array(
				'CHAR'     => __('CHAR', 'wams'),
				'NUMERIC'  => __('NUMERIC', 'wams'),
				'BINARY'   => __('BINARY', 'wams'),
				'DATE'     => __('DATE', 'wams'),
				'DATETIME' => __('DATETIME', 'wams'),
				'DECIMAL'  => __('DECIMAL', 'wams'),
				'SIGNED'   => __('SIGNED', 'wams'),
				'TIME'     => __('TIME', 'wams'),
				'UNSIGNED' => __('UNSIGNED', 'wams'),
			);

			$this->sort_data_types = apply_filters('wams_members_directory_sort_data_types', $this->sort_data_types);

			if (!empty(WAMS()->builtin()->saved_fields)) {
				foreach (WAMS()->builtin()->saved_fields as $key => $data) {
					if ('_wams_last_login' === $key) {
						continue;
					}

					if (isset($data['type']) && in_array($data['type'], $this->sorting_supported_fields)) {
						// translators: %s: title.
						if (isset($data['title']) && array_search(sprintf(__('%s DESC', 'wams'), $data['title']), $this->sort_fields) !== false) {
						...
						// translators: %s: title.
						$this->sort_fields[$key . '_desc'] = sprintf(__('%s DESC', 'wams'), $title);
						// translators: %s: title.
						$this->sort_fields[$key . '_asc'] = sprintf(__('%s ASC', 'wams'), $title);
					}
				}
			}
			...
			$this->default_sorting = apply_filters('wams_members_directory_default_sort', array_merge($this->sort_fields, array(
			...
			$this->filter_supported_fields = apply_filters('wams_members_directory_custom_field_types_supported_filter', array('date', 'time', 'select', 'multiselect', 'radio', 'checkbox', 'rating', 'text', 'textarea', 'number'));
			...
			$this->filter_fields = apply_filters('wams_members_directory_filter_fields', $this->filter_fields);

			ksort($this->filter_fields);

			$this->filter_types = apply_filters('wams_members_directory_filter_types', array(
				'country'               => 'select',
				'gender'                => 'select',
				'languages'             => 'select',
				'role'                  => 'select',
			...
			foreach ($custom_fields_types as $key => &$value) {
				if (!isset($fields[$key])) {
					...
					switch ($fields[$key]['type']) {
						default:
							$value = apply_filters('wams_custom_field_filter_type', 'select', $fields[$key]);
							break;
						case 'text':
						case 'textarea':
							$value = 'text';
		...
		function show_filter($filter, $directory_data, $default_value = false, $admin = false)
		{
			...
			if (isset($fields[$field_key])) {
				 ...
				 *
				 * @type filter
				 * @title wams_custom_search_field_{$filter}
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_custom_search_field_{$filter}', 'function_name', 10, 1 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_custom_search_field_{$filter}', 'my_custom_search_field', 10, 1 );
				 ...
				 * ?>
				 */
				$attrs = apply_filters("wams_custom_search_field_{$filter}", array(), $field_key);
			}

			// skip private invisible fields
			if (!wams_can_view_field($attrs)) {
				return '';
			}
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_search_fields', 'function_name', 10, 1 ); ?>
			 * @example
			 * <?php
			 * add_filter( 'wams_search_fields', 'my_search_fields', 10, 1 );
			 ...
			 * ?>
			 */
			$attrs = apply_filters('wams_search_fields', $attrs, $field_key, $directory_data['form_id']);
			...
			switch ($this->filter_types[$filter]) {
				default: {
						...
						do_action("wams_member_directory_filter_type_{$this->filter_types[$filter]}", $filter, $directory_data, $unique_hash, $attrs, $default_value);
				...
				case 'text': {
						...
						<input type="text" autocomplete="off" id="<?php echo $filter; ?>" name="<?php echo $filter; ?>" placeholder="<?php esc_attr_e(stripslashes($label), 'wams'); ?>" value="<?php echo esc_attr($filter_from_url) ?>" class="um-form-field" aria-label="<?php esc_attr_e(stripslashes($label), 'wams'); ?>" />
					<?php
						break;
					}
				case 'select': {
						...
						if (isset($attrs['metakey']) && strstr($attrs['metakey'], 'role_')) {
							$shortcode_roles = get_post_meta($directory_data['form_id'], '_wams_roles', true);
							$wams_roles = WAMS()->roles()->get_roles(false);

							if (!empty($shortcode_roles) && is_array($shortcode_roles)) {
								$attrs['options'] = array();

								foreach ($wams_roles as $key => $value) {
						...
						if (!empty($attrs['custom_dropdown_options_source'])) {
							...
							$ajax_source = apply_filters("wams_custom_dropdown_options_source__{$filter}", $attrs['custom_dropdown_options_source'], $attrs);

							$custom_dropdown .= ' data-um-ajax-source="' . esc_attr($ajax_source) . '" ';

							$attrs['options'] = WAMS()->fields()->get_options_from_callback($attrs, $attrs['type']);
						} else {
							/** This filter is documented in includes/core/class-fields.php */
							$option_pairs = apply_filters('wams_select_options_pair', null, $attrs);
						}
						...
						$attrs['options'] = apply_filters('wams_member_directory_filter_select_options', $attrs['options'], $values_array, $attrs);
						...
						$attrs['options'] = apply_filters('wams_member_directory_filter_select_options_sorted', $attrs['options'], $attrs);
					...
					?>
						...
						<select class="um-s1" id="<?php echo esc_attr($filter); ?>" name="<?php echo esc_attr($filter); ?><?php if ($admin && count($attrs['options']) > 1) { ?>[]<?php } ?>" data-placeholder="<?php esc_attr_e(stripslashes($label), 'wams'); ?>" aria-label="<?php esc_attr_e(stripslashes($label), 'wams'); ?>" <?php if ($admin && count($attrs['options']) > 1) { ?>multiple<?php } ?> <?php echo $custom_dropdown; ?>>
							...
							<?php if (!empty($attrs['options'])) {
								foreach ($attrs['options'] as $k => $v) {
									...
									<option value="<?php echo esc_attr($opt); ?>" data-value_label="<?php esc_attr_e($v, 'wams'); ?>" <?php disabled(!empty($filter_from_url) && in_array($opt, $filter_from_url));
				...
				case 'slider': {
						...
						if ($range) {
							...
							<input type="hidden" id="<?php echo $filter; ?>_min" name="<?php echo $filter; ?>[]" class="wams_range_min" value="<?php echo !empty($default_value) ? esc_attr(min($default_value)) : '' ?>" />
							<input type="hidden" id="<?php echo $filter; ?>_max" name="<?php echo $filter; ?>[]" class="wams_range_max" value="<?php echo !empty($default_value) ? esc_attr(max($default_value)) : '' ?>" />
							...
							<div class="um-slider-range" data-placeholder-s="<?php echo esc_attr($single_placeholder); ?>" data-placeholder-p="<?php echo esc_attr($plural_placeholder); ?>" data-label="<?php echo (!empty($attrs['label'])) ? esc_attr__(stripslashes($attrs['label']), 'wams') : ''; ?>"></div>
						<?php }
				...
				case 'datepicker':
					...
					if ($range) {
						...
						?>
						<input type="text" id="<?php echo $filter; ?>_from" name="<?php echo $filter; ?>_from" class="um-datepicker-filter" placeholder="<?php esc_attr_e(sprintf('%s From', stripslashes($label)), 'wams'); ?>" data-filter-label="<?php echo esc_attr(stripslashes($label)); ?>" data-date_min="<?php echo esc_attr($min); ?>" data-date_max="<?php echo esc_attr($max); ?>" data-filter_name="<?php echo $filter; ?>" data-range="from" data-value="<?php echo !empty($default_value) ? esc_attr(strtotime(min($default_value))) : '' ?>" />
						<input type="text" id="<?php echo $filter; ?>_to" name="<?php echo $filter; ?>_to" class="um-datepicker-filter" placeholder="<?php esc_attr_e(sprintf('%s To', stripslashes($label)), 'wams'); ?>" data-filter-label="<?php echo esc_attr(stripslashes($label)); ?>" data-date_min="<?php echo esc_attr($min); ?>" data-date_max="<?php echo esc_attr($max); ?>" data-filter_name="<?php echo $filter; ?>" data-range="to" data-value="<?php echo !empty($default_value) ? esc_attr(strtotime(max($default_value))) : '' ?>" />
						<?php
					}
					break;
				case 'timepicker': {
						...
						if ($range) { ?>
							...
							<input type="text" id="<?php echo $filter; ?>_from" name="<?php echo $filter; ?>_from" class="um-timepicker-filter" placeholder="<?php esc_attr_e(sprintf('%s From', stripslashes($label)), 'wams'); ?>" data-filter-label="<?php echo esc_attr(stripslashes($label)); ?>" data-min="<?php echo $range[0] ?>" data-max="<?php echo $range[1] ?>" data-format="<?php echo esc_attr($js_format) ?>" data-intervals="<?php echo esc_attr($attrs['intervals']) ?>" data-filter_name="<?php echo $filter; ?>" data-range="from" />
							<input type="text" id="<?php echo $filter; ?>_to" name="<?php echo $filter; ?>_to" class="um-timepicker-filter" placeholder="<?php esc_attr_e(sprintf('%s To', stripslashes($label)), 'wams'); ?>" data-filter-label="<?php echo esc_attr(stripslashes($label)); ?>" data-min="<?php echo $range[0] ?>" data-max="<?php echo $range[1] ?>" data-format="<?php echo esc_attr($js_format) ?>" data-intervals="<?php echo esc_attr($attrs['intervals']) ?>" data-filter_name="<?php echo $filter; ?>" data-range="to" />
		...
		function slider_filters_range($filter, $directory_data)
		{
			...
			switch ($filter) {
				...
				default: {
						...
						$range = apply_filters('wams_member_directory_filter_slider_common', $range, $directory_data, $filter);
						$range = apply_filters("wams_member_directory_filter_{$filter}_slider", $range, $directory_data);
		...
		function slider_range_placeholder($filter, $attrs)
		{
			switch ($filter) {
				default: {
						...
						$label = ucwords(str_replace(array('wams_', '_'), array('', ' '), $label));
						$placeholders = apply_filters('wams_member_directory_filter_slider_range_placeholder', false, $filter);
						...
						if (!$placeholders) {
							switch ($attrs['type']) {
								...
								case 'rating':
									$placeholders = array(
										"<strong>$label:</strong>&nbsp;{value}" . __(' stars', 'wams'),
										"<strong>$label:</strong>&nbsp;{min_range} - {max_range}" . __(' stars', 'wams')
									);
									break;
							}
						}
				...
				case 'birth_date': {
						$placeholders = array(
							__('<strong>Age:</strong>&nbsp;{value} years old', 'wams'),
							__('<strong>Age:</strong>&nbsp;{min_range} - {max_range} years old', 'wams')
						);
						break;
					}
			}
		...
		public function datepicker_filters_range($filter)
		{
			...
			switch ($filter) {
				default:
					...
					$range = apply_filters("wams_member_directory_filter_{$filter}_datepicker", $range);
				...
				case 'last_login':
					$meta = $wpdb->get_row(
						...
						FROM {$wpdb->usermeta}
						WHERE meta_key = '_wams_last_login'",
						ARRAY_A
					);
		...
		function timepicker_filters_range($filter)
		{
			...
			switch ($filter) {
				...
				default: {
						...
						$range = apply_filters("wams_member_directory_filter_{$filter}_timepicker", $range);
		...
		function restriction_options()
		{
			...
			do_action('wams_member_directory_restrictions_handle_extend');
		}
		...
		function hide_not_approved()
		{
			...
			$this->query_args['meta_query'] = array_merge($this->query_args['meta_query'], array(array(
				'key'       => 'wams_member_directory_data',
		...
		function hide_by_role()
		{
			...
			$roles = wams_user('can_view_roles');
			$roles = maybe_unserialize($roles);

			if (WAMS()->roles()->wams_user_can('can_view_all') && empty($roles)) {
				return;
			}
		...
		function general_options($directory_data)
		{
			...
			do_action('wams_member_directory_general_options_handle_extend', $directory_data);
		}
		...
		function pagination_options($directory_data)
		{
			...
			if (WAMS()->mobile()->isMobile() && isset($directory_data['profiles_per_page_mobile'])) {
		...
		function sorting_query($directory_data)
		{
			...
			if ('username' == $sortby) {
				...
				$display_name = WAMS()->options()->get('display_name');
				...
				$this->query_args['orderby']      = array('wams_last_login' => 'DESC');
				$this->query_args['meta_query'][] = array(
					'relation'      => 'OR',
					array(
						'key'     => '_wams_last_login',
						'compare' => 'EXISTS',
						'type'    => 'DATETIME',
					),
					'wams_last_login' => array(
						'key'     => '_wams_last_login',
						'compare' => 'NOT EXISTS',
						'type'    => 'DATETIME',
					),
				 ...
				 *
				 * @since 2.1.3
				 * @hook wams_member_directory_custom_sorting_type
				 *
				 ...
				 * function my_wams_member_directory_custom_sorting_type( $custom_sort_type, $sortby, $directory_data ) {
				 ...
				 * }
				 * add_filter( 'wams_member_directory_custom_sorting_type', 'my_wams_member_directory_custom_sorting_type', 10, 3 );
				 */
				$custom_sort_type = apply_filters('wams_member_directory_custom_sorting_type', $custom_sort_type, $sortby, $directory_data);
			 ...
			 *
			 * @type filter
			 * @title wams_modify_sortby_parameter
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_modify_sortby_parameter', 'function_name', 10, 2 ); ?>
			 * @example
			 * <?php
			 * add_filter( 'wams_modify_sortby_parameter', 'my_modify_sortby_parameter', 10, 2 );
			 ...
			 * ?>
			 */
			$this->query_args = apply_filters('wams_modify_sortby_parameter', $this->query_args, $sortby);
		}
		...
		function sortby_randomly($query)
		{
			if ('random' == $query->query_vars['orderby']) {
				...
				if (wams_is_session_started() === false) {
					@session_start();
				}

				// Reset seed on load of initial
				if (empty($_REQUEST['directory_id']) && isset($_SESSION['wams_member_directory_seed'])) {
					unset($_SESSION['wams_member_directory_seed']);
				}

				// Get seed from session variable if it exists
				$seed = false;
				if (isset($_SESSION['wams_member_directory_seed'])) {
					$seed = $_SESSION['wams_member_directory_seed'];
				}

				// Set new seed if none exists
				if (!$seed) {
					$seed = rand();
					$_SESSION['wams_member_directory_seed'] = $seed;
				}
		...
		public function general_search()
		{
			...
			if (!empty($_POST['search'])) {
				...
				if (!empty($search)) {
					...
					$meta_query = apply_filters('wams_member_directory_general_search_meta_query', $meta_query, $search);
		 ...
		 * for search only by WAMS user meta fields and WP core fields in WP Users table
		 *
			...
			if (!empty($_POST['search'])) {
				...
				if (!empty($search)) {
					...
					if (isset($join_matches[1])) {
						...
						// skip private invisible fields
						$custom_fields = array();
						foreach (array_keys(WAMS()->builtin()->all_user_fields) as $field_key) {
							if (empty($field_key)) {
								continue;
							}

							$data = WAMS()->fields()->get_field($field_key);
						...
						$custom_fields = apply_filters('wams_general_search_custom_fields', $custom_fields);
		...
		function filters($directory_data)
		{
			...
			$ignore_empty_filters = apply_filters('wams_member_directory_ignore_empty_filters', false);
			...
			foreach ($filter_query as $field => $value) {
				...
				$attrs = WAMS()->fields()->get_field($field);
				// skip private invisible fields
				if (!wams_can_view_field($attrs)) {
					continue;
				}
				...
				switch ($field) {
					default:
						 ...
						 *
						 * @type filter
						 * @title wams_query_args_{$field}__filter
						 ...
						 * ["Since: 2.0"]
						 * @usage
						 * <?php add_filter( 'wams_query_args_{$field}__filter', 'function_name', 10, 1 ); ?>
						 * @example
						 * <?php
						 * add_filter( 'wams_query_args_{$field}__filter', 'my_query_args_filter', 10, 1 );
						 ...
						 * ?>
						 */
						$field_query = apply_filters("wams_query_args_{$field}__filter", false, $field, $value, $filter_type);

						$field_query = apply_filters('wams_query_args_filter_global', $field_query, $field, $value, $filter_type);
						...
						if (!$field_query) {
							...
							switch ($filter_type) {
								default:
									...
									$field_query = apply_filters("wams_query_args_{$field}_{$filter_type}__filter", false, $field, $value, $filter_type);
								...
								case 'text':
									...
									$field_query = array(
										...
										array(
											'key'       => $field,
											'value'     => trim($value),
											'compare'   => apply_filters('wams_members_directory_filter_text', 'LIKE', $field)
										),
									);
					...
					case 'role':
						...
						if (!empty($this->query_args['role__in'])) {
							...
							$wams_role = array_diff($value, $default_role);

							foreach ($wams_role as $key => &$val) {
								$val = 'wams_' . str_replace(' ', '-', $val);
							}
							$this->query_args['role__in'] = array_merge($default_role, $wams_role);
						} else {
		...
		function default_filters($directory_data)
		{
			...
			$gmt_offset = get_post_meta($directory_data['form_id'], '_wams_search_filters_gmt', true);
							...
							switch ($filter_type) {
								...
								case 'text':
									...
									$field_query = array(
										'key'       => $field,
										'value'     => $value,
										'compare'   => apply_filters('wams_members_directory_filter_text', '=', $field),
									);
			...
			if (get_current_user_id() != $user_id) {
				...
				if (WAMS()->roles()->wams_current_user_can('edit', $user_id)) {
					$actions['um-editprofile'] = array(
						'title' => __('Edit Profile', 'wams'),
						'url' => wams_edit_profile_url(),
					);
				}
				 ...
				 *
				 * @type filter
				 * @title wams_admin_user_actions_hook
				 ...
				 * ["Since: 2.0"]
				 * @usage
				 * <?php add_filter( 'wams_admin_user_actions_hook', 'function_name', 10, 1 ); ?>
				 * @example
				 * <?php
				 * add_filter( 'wams_admin_user_actions_hook', 'my_admin_user_actions', 10, 1 );
				 ...
				 * ?>
				 */
				$admin_actions = apply_filters('wams_admin_user_actions_hook', array(), $user_id);
				if (!empty($admin_actions)) {
					foreach ($admin_actions as $id => $arr) {
						$url = add_query_arg(array('wams_action' => $id, 'uid' => $user_id), wams_get_core_page('user'));
				...
				$actions = apply_filters('wams_member_directory_users_card_actions', $actions, $user_id);
			} else {
				...
				$actions['um-myaccount'] = array(
					'title' => __('My Account', 'wams'),
					'url'   => wams_get_core_page('account'),
				);

				$actions['um-logout'] = array(
					'title' => __('Logout', 'wams'),
					'url'   => wams_get_core_page('logout'),
				);

				$actions = apply_filters('wams_member_directory_my_user_card_actions', $actions, $user_id);
			}
		...
		function build_user_card_data($user_id, $directory_data)
		{
			...
			$actions = array();
			$can_edit = WAMS()->roles()->wams_current_user_can('edit', $user_id);

			// Replace hook 'wams_members_just_after_name'
			ob_start();
			do_action('wams_members_just_after_name', $user_id, $directory_data);
			$hook_just_after_name = ob_get_clean();

			// Replace hook 'wams_members_after_user_name'
			ob_start();
			do_action('wams_members_after_user_name', $user_id, $directory_data);
			...
			$data_array = array(
				...
				'account_status_name'   => wams_user('account_status_name'),
				'cover_photo'           => wams_user('cover_photo', $this->cover_size),
				'display_name'          => wams_user('display_name'),
				'profile_url'           => wams_user_profile_url(),
				'can_edit'              => $can_edit,
				'edit_profile_url'      => wams_edit_profile_url(),
				'avatar'                => get_avatar($user_id, $this->avatar_size),
				'display_name_html'     => wams_user('display_name', 'html'),
				...
				if (!empty($directory_data['reveal_fields'])) {
					...
					if (is_array($directory_data['reveal_fields'])) {
						foreach ($directory_data['reveal_fields'] as $key) {
							...
							$label = WAMS()->fields()->get_label($key);
							...
							$data_array["label_{$key}"] = __($label, 'wams');
							$data_array[$key] = $value;
						}
					}
				}
			...
			$data_array = apply_filters('wams_ajax_get_members_data', $data_array, $user_id, $directory_data);

			wams_reset_user_clean();
		...
		function pagination_changes($user_query)
		{
			...
			$directory_data = WAMS()->query()->post_data($directory_id);
		...
		function predefined_no_caps($directory_data)
		{
			...
			if (is_user_logged_in() && !WAMS()->roles()->wams_user_can('can_view_all')) {
				$pagination_data = array(
					'pages_to_show' => array(),
					'current_page'  => 1,
					'total_pages'   => 0,
		...
		function ajax_get_members()
		{
			...
			if (empty($_POST['directory_id'])) {
				wp_send_json_error(__('Wrong member directory data', 'wams'));
			}
			...
			do_action('wams_member_directory_before_query');
			 ...
			 *
			 * @type filter
			 * @title wams_prepare_user_query_args
			 ...
			 * ["Since: 2.0"]
			 * @usage
			 * <?php add_filter( 'wams_prepare_user_query_args', 'function_name', 10, 2 ); ?>
			 * @example
			 * <?php
			 * add_filter( 'wams_prepare_user_query_args', 'my_prepare_user_query_args', 10, 2 );
			 ...
			 * ?>
			 */
			$this->query_args = apply_filters('wams_prepare_user_query_args', $this->query_args, $directory_data);
			...
			if (isset($this->query_args['role__in']) && empty($this->query_args['role__in'])) {
				$member_directory_response = apply_filters('wams_ajax_get_members_response', array(
			 ...
			 * }
			 * add_action( 'wams_user_before_query', 'my_user_before_query', 10, 2 );
			 */
			do_action('wams_user_before_query', $this->query_args, $this);
			 ...
			 *     // your code here
			 * }
			 * add_action( 'wams_user_after_query', 'my_user_after_query', 10, 2 );
			 */
			do_action('wams_user_after_query', $this->query_args, $user_query);
			 ...
			 *
			 * @since 2.0
			 * @hook wams_prepare_user_results_array
			 *
			 ...
			 * function my_custom_wams_prepare_user_results_array( $user_ids, $query_args ) {
			 *     $user_ids = array_diff( $user_ids, array( 10, 12 ) );
			 *     return $user_ids;
			 * }
			 * add_filter( 'wams_prepare_user_results_array', 'my_custom_wams_prepare_user_results_array', 10, 2 );
			 */
			$user_ids = apply_filters('wams_prepare_user_results_array', $user_ids, $this->query_args);

			$sizes = WAMS()->options()->get('cover_thumb_sizes');

			$this->cover_size = WAMS()->mobile()->isTablet() ? $sizes[1] : end($sizes);

			$this->cover_size = apply_filters('wams_member_directory_cover_image_size', $this->cover_size, $directory_data);

			$avatar_size = WAMS()->options()->get('profile_photosize');
			$this->avatar_size = str_replace('px', '', $avatar_size);
			$this->avatar_size = apply_filters('wams_member_directory_avatar_image_size', $this->avatar_size, $directory_data);
		...
		public function get_member_directory_id($page_id)
		{
			...
			if (!empty($members_page) && !is_wp_error($members_page)) {
				if (!empty($members_page->post_content)) {
					preg_match_all('/\[wams[^\]]*?form_id\=[\'"]*?(\d+)[\'"]*?/i', $members_page->post_content, $matches);
