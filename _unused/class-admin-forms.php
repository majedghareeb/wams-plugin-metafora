...
if (!class_exists('wams\admin\core\Admin_Forms')) {
	...
	class Admin_Forms
	{
		...
		function render_form($echo = true)
		{
			...
			function render_field_by_hook($data)
			{
				 ...
				 *
				 * @type filter
				 * @title wams_render_field_type_{$type}
				 ...
				 * @change_log
				 * ["Since: 2.0"]
				 * @usage add_filter( 'wams_render_field_type_{$type}', 'function_name', 10, 4 );
				 * @example
				 * <?php
				 * add_filter( 'wams_render_field_type_{$type}', 'my_render_field_type', 10, 4 );
				 ...
				 * ?>
				 */
				return apply_filters('wams_render_field_type_' . $data['type'], '', $data, $this->form_data, $this);
			}
			...
			function render_field_label($data)
			{
				...
				if (isset($data['required']) && $data['required']) {
					$label = $label . '<span class="um-req" title="' . esc_attr__('Required', 'wams') . '">*</span>';
				}

				$tooltip = !empty($data['tooltip']) ? WAMS()->tooltip($data['tooltip'], false, false) : '';
			...
			public function render_icon($field_data)
			{
				...
				$html = '<span class="wams_admin_fonticon_wrapper"><a href="javascript:void(0);" class="button" data-modal="WAMS_fonticons" data-modal-size="normal" data-dynamic-content="wams_admin_fonticon_selector" data-arg1="" data-arg2="" data-back="" data-icon_field="' . esc_attr($id) . '">' . esc_html__('Choose Icon', 'wams') . '</a>
				<span class="um-admin-icon-value">';

				if (!empty($value)) {
					$html .= '<i class="' . esc_attr($value) . '"></i>';
				} else {
					$html .= esc_html__('No Icon', 'wams');
				}
			...
			public function render_users_dropdown($field_data)
			{
				...
				$placeholder = !empty($field_data['placeholder']) ? $field_data['placeholder'] : __('Select Users', 'wams');
			...
			function render_sortable_items($field_data)
			{
				...
				foreach ($field_data['items'] as $tab_id => $tab_name) {
					$content = apply_filters('wams_render_sortable_items_item_html', $tab_name, $tab_id, $field_data);
			...
			function render_multi_selects($field_data)
			{
				...
				if (!empty($values) && is_array($values)) {
					foreach ($values as $k => $value) {
						...
						<span class=\"um-field-control\"><a href=\"javascript:void(0);\" class=\"um-select-delete\">" . __('Remove', 'wams') . "</a></span></li>";
					}
			...
			function render_multi_text($field_data)
			{
				...
				if (!empty($values)) {
					foreach ($values as $k => $value) {
						...
						<span class=\"um-field-control\"><a href=\"javascript:void(0);\" class=\"um-text-delete\">" . __('Remove', 'wams') . "</a></span></li>";
					}
				} elseif (!empty($field_data['show_default_number']) && is_numeric($field_data['show_default_number']) && $field_data['show_default_number'] > 0) {
					$i = 0;
					while ($i < $field_data['show_default_number']) {
			...
			function render_media($field_data)
			{
				...
				$upload_frame_title = !empty($field_data['upload_frame_title']) ? $field_data['upload_frame_title'] : __('Select media', 'wams');
				...
				$html .= '<input type="button" class="um-set-image button button-primary" value="' . esc_attr__('Select', 'wams') . '" data-upload_frame="' . $upload_frame_title . '" />
					<input type="button" class="um-clear-image button" value="' . esc_attr__('Clear', 'wams') . '" /></div>';
			...
			<div class="email_template_wrapper <?php echo $field_data['in_theme'] ? 'in_theme' : '' ?>" data-key="<?php echo $field_data['id'] ?>" style="position: relative;">
				...
				<span class="description">For default text for plain-text emails please see this <a href="https://docs.wams.com/article/1342-plain-text-email-default-templates#<?php echo $field_data['id'] ?>" target="_blank">doc</a></span>
			</div>
...
<?php $html = ob_get_clean();
			...
			function render_ajax_button($field_data)
			{
				...
				$html = "<input type=\"button\" $id_attr $class_attr $name_attr $data_attr $value_attr /><div class='clear'></div><div class='wams_setting_ajax_button_response'></div>";
			...
			function render_md_default_filters($field_data)
			{
				...
				if (!empty($filters) && is_array($filters)) {
					foreach ($filters as $k => $value) {
						...
						<select $id_attr $name_attr $class_attr $data_attr>$options</select></span>
						<span class=\"um-field-control\"><a href=\"javascript:void(0);\" class=\"um-select-delete\">" . __('Remove', 'wams') . "</a></span><span class=\"um-field-wrapper2 um\">" . WAMS()->member_directory()->show_filter($value, array('form_id' => $post->ID), $values[$value], true) . "</span></li>";
					}
				} elseif (!empty($field_data['show_default_number']) && is_numeric($field_data['show_default_number']) && $field_data['show_default_number'] > 0) {
					$i = 0;
					while ($i < $field_data['show_default_number']) {
			...
			function render_md_sorting_fields($field_data)
			{
				...
				if (!empty($values) && is_array($values)) {
					foreach ($values as $k => $value) {
						...
						$data_types_html = '';
						foreach (WAMS()->member_directory()->sort_data_types as $type_key => $type_label) {
							$data_types_html .= '<option value="' . esc_attr($type_key) . '" ' . selected($other_type, $type_key, false) . '>' . esc_html($type_label) . '</option>';
						}

						$html .= '<span class="um-field-wrapper">
						<select ' . $id_attr . ' ' . $name_attr . ' ' . $class_attr . ' ' . $data_attr . '>' . $options . '</select></span>
						<span class="um-field-control"><a href="javascript:void(0);" class="um-select-delete">' . __('Remove', 'wams') . '</a></span>
						<span class="um-field-wrapper um-custom-order-fields"><label>' . __('Meta key', 'wams') . ':&nbsp;<input type="text" name="wams_metadata[_wams_sorting_fields][other_data][' . $k . '][meta_key]" value="' . esc_attr($other_key) . '" /></label></span>
						<span class="um-field-wrapper um-custom-order-fields"><label>' . __('Data type', 'wams') . ':&nbsp;<select name="wams_metadata[_wams_sorting_fields][other_data][' . $k . '][data_type]" />' .
							$data_types_html .
							'</select></label></span>
						<span class="um-field-wrapper um-custom-order-fields"><label>' . __('Order', 'wams') . ':&nbsp;<select name="wams_metadata[_wams_sorting_fields][other_data][' . $k . '][order]" />
						<option value="ASC" ' . selected($other_order, 'ASC', false) . '>' . __('ASC', 'wams') . '</option>
						<option value="DESC" ' . selected($other_order, 'DESC', false) . '>' . __('DESC', 'wams') . '</option>
						</select></label></span>
						<span class="um-field-wrapper um-custom-order-fields"><label>' . __('Label', 'wams') . ':&nbsp;<input type="text" name="wams_metadata[_wams_sorting_fields][other_data][' . $k . '][label]" value="' . esc_attr($other_label) . '" /></label></span>
						</li>';
					}
				} elseif (!empty($field_data['show_default_number']) && is_numeric($field_data['show_default_number']) && $field_data['show_default_number'] > 0) {
					$i = 0;
					while ($i < $field_data['show_default_number']) {
						...
						<select $id_attr $name_attr $class_attr $data_attr>$options</select></span>
						<span class=\"um-field-control\"><a href=\"javascript:void(0);\" class=\"um-select-delete\">" . __('Remove', 'wams') . "</a></span>
						<span class=\"um-field-wrapper um-custom-order-fields\"><label>" . __('Meta key', 'wams') . ":&nbsp;<input type=\"text\" name=\"wams_metadata[_wams_sorting_fields][other_data][" . $i . "][meta_key]\" value=\"\" /></label></span>
						<span class=\"um-field-wrapper um-custom-order-fields\"><label>" . __('Label', 'wams') . ":&nbsp;<input type=\"text\" name=\"wams_metadata[_wams_sorting_fields][other_data][" . $i . "][label]\" value=\"\" /></label></span>
						</li>";
