...
if (!class_exists('wams\core\Access')) {
	...
	class Access
	{
		...
		function __construct()
		{
			...
			add_action('wams_access_check_blog_page_settings', array(&$this, 'wams_access_check_blog_page_settings'));
			add_action('wams_access_check_individual_term_settings', array(&$this, 'wams_access_check_individual_term_settings'));
			add_action('wams_access_check_global_settings', array(&$this, 'wams_access_check_global_settings'));
		...
		public function initialize_hooks()
		{
			...
			if (!WAMS()->options()->get('disable_restriction_pre_queries')) {
				return;
			}
		...
		function exclude_posts_array($force = false, $post_types = false)
		{
			...
			if (!empty($force_cache_key) && array_key_exists($force_cache_key, $cache)) {
				...
				if (!empty($post_ids)) {
					foreach ($post_ids as $post_id) {
						...
						if (!empty($content_restriction['_wams_access_hide_from_queries'])) {
				...
				$restricted_taxonomies = WAMS()->options()->get('restricted_access_taxonomy_metabox');
				...
				if (!empty($restricted_taxonomies)) {
					...
					$terms = $wpdb->get_results(
						...
						WHERE tm.meta_key = 'wams_content_restriction' AND
								...
								array(
									...
									'meta_query'  => array(
										...
										array(
											...
											array(
												'key'     => 'wams_content_restriction',
												'value'   => 's:26:"_wams_custom_access_settings";s:1:"1"',
												'compare' => 'NOT LIKE',
											),
											array(
												'key'     => 'wams_content_restriction',
												'value'   => 's:26:"_wams_custom_access_settings";b:1',
												'compare' => 'NOT LIKE',
											),
										),
										array(
			...
			$exclude_posts = apply_filters('wams_exclude_restricted_posts_ids', $exclude_posts, $force);
		...
		function exclude_terms_array($query)
		{
			...
			$exclude = apply_filters('wams_exclude_restricted_terms_ids', $exclude);
			$cache[$cache_key] = $exclude;
			return $exclude;
		}
		...
		{
			if (current_user_can('administrator') || !empty($query->query_vars['wams_ignore_exclude'])) {
				return;
			}
		...
		function exclude_posts($query)
		{
			...
			if ($query->is_main_query() || !empty($query->query_vars['wams_main_query'])) {
		...
		function exclude_posts_where($where, $query)
		{
			...
			if (!empty($query->query_vars['p']) && $this->is_restricted($query->query_vars['p'])) {
				...
				if (!empty($restriction_settings['_wams_access_hide_from_queries']) && !empty($query->query_vars['post__not_in'])) {
					global $wpdb;
		...
		function custom_count_posts_handler($counts, $type = 'post', $perm = '')
		{
			...
			$query = "SELECT post_status, COUNT( * ) AS nwams_posts FROM {$wpdb->posts} WHERE post_type = %s";
			...
			foreach ($results as $row) {
				$counts[$row['post_status']] = $row['nwams_posts'];
			}
		...
		function filter_restricted_post_title($title, $id = null)
		{
			...
			$ignore = apply_filters('wams_ignore_restricted_title', false, $id);
			if ($ignore) {
				return $title;
			}

			if ($this->is_restricted($id)) {
				$restricted_global_title = WAMS()->options()->get('restricted_access_post_title');
		...
		function filter_restricted_post_content($content)
		{
			...
			$ignore = apply_filters('wams_ignore_restricted_content', false, $id);
			if ($ignore) {
				return $content;
			}
			...
			if ($this->is_restricted($id)) {
				...
				if (!isset($restriction['_wams_restrict_by_custom_message']) || '0' == $restriction['_wams_restrict_by_custom_message']) {
					$content = stripslashes(WAMS()->options()->get('restricted_access_message'));
				} elseif ('1' == $restriction['_wams_restrict_by_custom_message']) {
					$content = !empty($restriction['_wams_restrict_custom_message']) ? stripslashes($restriction['_wams_restrict_custom_message']) : '';
				}

				// translators: %s: Restricted post message.
				$content = sprintf(__('%s', 'wams'), $content);
			}
		...
		function filter_restricted_post_excerpt($post_excerpt = '', $post = null)
		{
			...
			$ignore = apply_filters('wams_ignore_restricted_excerpt', false, $post->ID);
			if ($ignore) {
				return $post_excerpt;
			}
		...
		function filter_post_thumbnail($has_thumbnail, $post = null, $thumbnail_id = false)
		{
			...
			$has_thumbnail = apply_filters('wams_restrict_post_thumbnail', $has_thumbnail, $post, $thumbnail_id);
	...
	",
				...
				if (!empty($menu_item->object_id) && !empty($menu_item->object)) {
					if (isset($menu_item->type) && 'taxonomy' === $menu_item->type) {
						...
						if (!$this->is_restricted($menu_item->object_id)) {
							...
							if (empty($restriction_settings['_wams_access_hide_from_queries']) || WAMS()->options()->get('disable_restriction_pre_queries')) {
		...
		function restrict_blocks($block_content, $block)
		{
			...
			if (!isset($block['attrs']['wams_is_restrict']) || $block['attrs']['wams_is_restrict'] !== true) {
				return $block_content;
			}

			if (empty($block['attrs']['wams_who_access'])) {
				return $block_content;
			}

			$default_message = WAMS()->options()->get('restricted_block_message');
			switch ($block['attrs']['wams_who_access']) {
				case '1': {
						if (!is_user_logged_in()) {
							$block_content = '';
							if (isset($block['attrs']['wams_message_type'])) {
								if ($block['attrs']['wams_message_type'] == '1') {
									$block_content = $default_message;
								} elseif ($block['attrs']['wams_message_type'] == '2') {
									$block_content = isset($block['attrs']['wams_message_content']) ? esc_textarea($block['attrs']['wams_message_content']) : '';
								}
							}
						} else {
							$display = true;
							...
							if (!empty($block['attrs']['wams_roles_access'])) {
								$display = false;
								foreach ($block['attrs']['wams_roles_access'] as $role) {
							...
							$display = apply_filters('wams_loggedin_block_restriction', $display, $block);
		...
		function maybe_replace_title($post)
		{
			...
			$ignore = apply_filters('wams_ignore_restricted_title', false, $post->ID);
			if ($ignore) {
				return $post;
			}
		...
		function maybe_replace_nav_menu_title($nav_item)
		{
			...
			$ignore = apply_filters('wams_ignore_restricted_title', false, $nav_item->ID);
			if ($ignore) {
				return $nav_item;
			}
		...
		function filter_protected_posts($posts, $query)
		{
			...
			if (
				is_object($query) && is_a($query, '\WP_Query') &&
				($query->is_main_query() || !empty($query->query_vars['wams_main_query']))
			) {
				if ($is_singular) {
					if (!WAMS()->options()->get('disable_restriction_pre_queries') && $this->is_restricted($posts[0]->ID)) {
			...
			foreach ($posts as $post) {
				...
				if (!$this->is_restricted($post->ID)) {
					...
					if ($is_singular) {
						if (!isset($restriction['_wams_noaccess_action']) || '0' == $restriction['_wams_noaccess_action']) {
							if (WAMS()->options()->get('disable_restriction_pre_queries') || empty($restriction['_wams_access_hide_from_queries'])) {
								/**
								 * WAMS hook
								 *
								 * @type action
								 * @title wams_access_fix_external_post_content
								 * @description Hook for 3-d party content filtration
								 * @change_log
								 * ["Since: 2.0"]
								 * @usage add_action( 'wams_access_fix_external_post_content', 'function_name', 10 );
								 * @example
								 * <?php
								 * add_action( 'wams_access_fix_external_post_content', 'my_access_fix_external_post_content', 10 );
								 * function my_access_fix_external_post_content() {
								 *     // your code here
								 * }
								 * ?>
								 */
								do_action('wams_access_fix_external_post_content');

								$filtered_posts[] = $this->maybe_replace_title($post);
								continue;
							}
						} elseif ('1' == $restriction['_wams_noaccess_action']) {
							$curr = WAMS()->permalinks()->get_current_url();

							if (!isset($restriction['_wams_access_redirect']) || '0' == $restriction['_wams_access_redirect']) {

								exit(wp_redirect(esc_url_raw(add_query_arg('redirect_to', urlencode_deep($curr), wams_get_core_page('login')))));
							} elseif ('1' == $restriction['_wams_access_redirect']) {

								if (!empty($restriction['_wams_access_redirect_url'])) {
									$redirect = $restriction['_wams_access_redirect_url'];
								} else {
									$redirect = esc_url_raw(add_query_arg('redirect_to', urlencode_deep($curr), wams_get_core_page('login')));
								}
						...
						if (empty($restriction['_wams_access_hide_from_queries']) || WAMS()->options()->get('disable_restriction_pre_queries')) {
							$filtered_posts[] = $this->maybe_replace_title($post);
							continue;
						}
					}
		...
		function template_redirect()
		{
			...
			//also skip if we currently at WAMS Register|Login|Reset Password pages
			if (
				wams_is_core_post($post, 'register') ||
				wams_is_core_post($post, 'password-reset') ||
				wams_is_core_post($post, 'login')
			) {
				return;
			}
			 ...
			 *
			 * @type action
			 * @title wams_access_check_blog_page_settings
			 * @description Check blog page individual restrict options
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_access_check_blog_page_settings', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_access_check_blog_page_settings', 'my_access_check_blog_page_settings', 10 );
			 * function my_access_check_blog_page_settings() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_access_check_blog_page_settings');
			 ...
			 *
			 * @type action
			 * @title wams_roles_add_meta_boxes_wams_role_meta
			 * @description Check terms individual restrict options
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_access_check_individual_term_settings', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_access_check_individual_term_settings', 'my_access_check_individual_term_settings', 10 );
			 * function my_access_check_individual_term_settings() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_access_check_individual_term_settings');
			 ...
			 *
			 * @type action
			 * @title wams_access_check_global_settings
			 * @description Check global restrict content options
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_action( 'wams_access_check_global_settings', 'function_name', 10 );
			 * @example
			 * <?php
			 * add_action( 'wams_access_check_global_settings', 'my_access_check_global_settings', 10 );
			 * function my_access_check_global_settings() {
			 *     // your code here
			 * }
			 * ?>
			 */
			do_action('wams_access_check_global_settings');
		 ...
		 */
		function wams_access_check_blog_page_settings()
		{
			global $wp_query;
			...
			if (!empty($wp_query->is_home) && !empty($wp_query->is_posts_page)) {
				...
				if (!empty($post_id)) {
					...
					if ($restriction_result) {
						$restriction = get_post_meta($post_id, 'wams_content_restriction', true);
						if ('1' == $restriction['_wams_noaccess_action']) {
							$curr = WAMS()->permalinks()->get_current_url();

							if (!isset($restriction['_wams_access_redirect']) || '0' == $restriction['_wams_access_redirect']) {

								$this->redirect_handler = $this->set_referer(esc_url_raw(add_query_arg('redirect_to', urlencode_deep($curr), wams_get_core_page('login'))), 'individual_term');
							} elseif ('1' == $restriction['_wams_access_redirect']) {
		 ...
		 */
		function wams_access_check_individual_term_settings()
		{
			...
			if ($this->is_restricted_term($term_id, true, true)) {
				$restriction = get_term_meta($term_id, 'wams_content_restriction', true);
				if ('1' == $restriction['_wams_noaccess_action']) {
					$curr = WAMS()->permalinks()->get_current_url();
		...
		{
			return WAMS()->locate_template('restricted-taxonomy.php');
		}
		...
		{
			return WAMS()->locate_template('restricted-blog.php');
		}
		 ...
		 */
		function wams_access_check_global_settings()
		{
			global $post;

			$curr = WAMS()->permalinks()->get_current_url();
			$ms_empty_role_access = is_multisite() && is_user_logged_in() && !WAMS()->roles()->get_priority_user_role(wams_user('ID'));

			if (is_front_page()) {
				if (is_user_logged_in() && !$ms_empty_role_access) {

					$user_default_homepage = wams_user('default_homepage');
					if (!empty($user_default_homepage)) {
						return;
					}

					$redirect_homepage = wams_user('redirect_homepage');
					/**
					 * WAMS hook
					 *
					 * @type filter
					 * @title wams_custom_homepage_redirect_url
					 ...
					 * ["Since: 2.0"]
					 * @usage
					 * <?php add_filter( 'wams_custom_homepage_redirect_url', 'function_name', 10, 2 ); ?>
					 * @example
					 * <?php
					 * add_filter( 'wams_custom_homepage_redirect_url', 'my_custom_homepage_redirect_url', 10, 2 );
					 ...
					 * ?>
					 */
					$redirect_homepage = apply_filters('wams_custom_homepage_redirect_url', $redirect_homepage, wams_user('ID'));
					$redirect_to = !empty($redirect_homepage) ? $redirect_homepage : wams_get_core_page('user');
					...
					$access = WAMS()->options()->get('accessible');

					if ($access == 2) {
						//global settings for accessible home page
						$home_page_accessible = WAMS()->options()->get('home_page_accessible');

						if ($home_page_accessible == 0) {
							//get redirect URL if not set get login page by default
							$redirect = WAMS()->options()->get('access_redirect');
							if (!$redirect) {
								$redirect = wams_get_core_page('login');
							}
				...
				if (!is_user_logged_in() || $ms_empty_role_access) {
					...
					if ($access == 2) {
						//global settings for accessible home page
						$category_page_accessible = WAMS()->options()->get('category_page_accessible');
			...
			if ($access == 2 && (!is_user_logged_in() || $ms_empty_role_access)) {
				...
				$redirects = array();
				$redirects[] = trim(untrailingslashit(WAMS()->options()->get('access_redirect')));
				...
				$current_url = WAMS()->permalinks()->get_current_url(WAMS()->is_permalinks);
		...
		function set_referer($url, $referer)
		{
			 ...
			 *
			 * @type filter
			 * @title wams_access_enable_referer
			 * @description Access Referrer Enable/Disable
			 * @input_vars
			 * [{"var":"$referrer","type":"bool","desc":"Access referrer"}]
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_access_enable_referer', 'function_name', 10, 1 );
			 * @example
			 * <?php
			 * add_filter( 'wams_access_enable_referer', 'my_access_enable_referer', 10, 1 );
			 ...
			 * ?>
			 */
			$enable_referer = apply_filters('wams_access_enable_referer', false);
			if (!$enable_referer) {
				return $url;
			}

			$url = add_query_arg('wams_ref', $referer, $url);
			return $url;
		}
		...
		public function get_post_privacy_settings($post)
		{
			...
			$exclude = false;
			//exclude from privacy WAMS default pages (except Members list and User(Profile) page)
			if (!empty($post->post_type) && $post->post_type === 'page') {

				if (
					wams_is_core_post($post, 'login') || wams_is_core_post($post, 'register') ||
					wams_is_core_post($post, 'account') || wams_is_core_post($post, 'logout') ||
					wams_is_core_post($post, 'password-reset') || (is_user_logged_in() && wams_is_core_post($post, 'user'))
				)
					$exclude = true;
			}

			$exclude = apply_filters('wams_exclude_posts_from_privacy', $exclude, $post);
			if ($exclude) {
				$cache[$cache_key] = false;
				return false;
			}
			...
			if (!empty($post->post_type) && !empty($restricted_posts[$post->post_type])) {
				...
				if (!empty($restriction['_wams_custom_access_settings'])) {
					if (!isset($restriction['_wams_accessible'])) {
						$restricted_taxonomies = WAMS()->options()->get('restricted_access_taxonomy_metabox');
						...
						if (!empty($taxonomies)) {
							foreach ($taxonomies as $taxonomy) {
								...
								$terms = array_merge($terms, wp_get_post_terms($post->ID, $taxonomy, array('fields' => 'ids', 'wams_ignore_exclude' => true,)));
							}
						}
						...
						if (!is_user_logged_in() && wams_is_core_post($post, 'user') && $restriction['_wams_accessible'] == '2' && $restriction['_wams_noaccess_action'] == '0') {
							if (isset($restriction['_wams_access_roles'])) {
								$restriction = array(
									'_wams_accessible'            => '2',
									'_wams_access_roles'          => $restriction['_wams_access_roles'],
									'_wams_noaccess_action'       => '1',
									'_wams_access_redirect'       => '1',
									'_wams_access_redirect_url'   => get_home_url(get_current_blog_id())
								);
							} else {
								$restriction = array(
									'_wams_accessible'            => '2',
						...
						$restriction = apply_filters('wams_post_content_restriction_settings', $restriction, $post);
		 ...
		 * @return bool
		 */
		function wams_custom_restriction($restriction)
		{
			/**
			 * WAMS hook
			 *
			 * @type filter
			 * @title wams_custom_restriction
			 ...
			 * @change_log
			 * ["Since: 2.0"]
			 * @usage add_filter( 'wams_custom_restriction', 'function_name', 10, 2 );
			 * @example
			 * <?php
			 * add_filter( 'wams_custom_restriction', 'my_custom_restriction', 10, 2 );
			 ...
			 * ?>
			 */
			return apply_filters('wams_custom_restriction', true, $restriction);
		}
		...
		function is_restricted($post_id, $on_single_page = false, $ignore_cache = false)
		{
			...
			if (!$restriction) {
				...
				if ('0' == $restriction['_wams_accessible']) {
					//post is private
					$restricted = false;
					if ($on_single_page) {
						$this->allow_access = true;
					}
				} elseif ('1' == $restriction['_wams_accessible']) {
				...
				} elseif ('2' == $restriction['_wams_accessible']) {
					//if post for logged in users and user is not logged in
					if (is_user_logged_in()) {
						$custom_restrict = $this->wams_custom_restriction($restriction);

						if (empty($restriction['_wams_access_roles']) || false === array_search('1', $restriction['_wams_access_roles'])) {
							if ($custom_restrict) {
								$restricted = false;
								if ($on_single_page) {
									$this->allow_access = true;
							...
							$user_can = $this->user_can(get_current_user_id(), $restriction['_wams_access_roles']);
			...
			$restricted = apply_filters('wams_is_restricted_post', $restricted, $post_id, $on_single_page);
		...
		function is_restricted_term($term_id, $on_term_page = false, $ignore_cache = false)
		{
			...
			$restricted = apply_filters('wams_is_restricted_term', $restricted, $term_id, $on_term_page);
